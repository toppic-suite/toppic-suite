...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
  ...
  <xsl:template match="protein">
    <div id="p{sequence_id}">
      ...
      <xsl:apply-templates select="compatible_proteoform/prsm">
      </xsl:apply-templates>
      <br/>
    </div>
  </xsl:template>
  ...
  <xsl:template match="compatible_proteoform/prsm">
    <xsl:if test="position()=1">
      <xsl:choose>
        <xsl:when test="count(../prsm) = 1">There is only <a href="prsms/prsm{prsm_id}.html">1 PrSM</a> with an E-value <xsl:value-of select="e_value"/>.</xsl:when>
        <xsl:otherwise>
