...
<html>
<head>
<title>Protein-Spectrum-Match for Spectrum #<xsl:value-of select="ms/ms_header/id"/></title>
...
function showMatchedPeaks() {
	for (i = 0; peaksCount>i; i++) {
		if (document.getElementById('peak' + i + 'B') != null){
			document.getElementById('peak' + i + 'B').style.display = "";
		}
...
function showNotMatchedPeaks() {
	for (i = 0; peaksCount>i; i++) {
		if (document.getElementById('peak' + i + 'B') != null){
			document.getElementById('peak' + i + 'B').style.display = "none";
		}
		if (document.getElementById('peak' + i + 'Y') != null){
...
function showIonPeaks(ids) {
	...
	for (i = 0; s.length>i; i++) {
		...
		document.getElementById('peak' + s[i]).style.background  =  "#BEECFF";
	}
	$('div.dataTables_scrollBody').height(400);
}
...
<div class="container">
	...
	<h2>Protein-Spectrum-Match #<xsl:value-of select="prsm_id"/> for Spectrum #<xsl:value-of select="ms/ms_header/id"/></h2>
	...
	<tr>
		...
		<td>Precursor mass:</td><td> <xsl:value-of select="ms/ms_header/precursor_mass"/> </td>
...
<tr>
<th width="25">ID</th>
<th width="100">Monoisotopic mass</th>
<th width="100">Monoisotopic m/z</th>
<th width="80" style="vertical-align:middle">Intensity</th>
<th width="70" style="vertical-align:middle">Charge</th>
<th width="103">Theoretical mass</th>
<th width="50" style="vertical-align:middle">Ion</th>
<th width="70" style="vertical-align:middle">position</th>
...
<xsl:variable name="peakID" select="id" as="xs:integer"/>
