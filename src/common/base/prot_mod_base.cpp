...
void ProtModBase::initBase(const std::string& base_dir) {
  ...
  std::string prot_mod_base_file_name =
      base_dir + file_util::getFileSeparator() + "prot_mod_base.xml";
  std::string prot_mod_base_data = file_util::readFile(prot_mod_base_file_name);
  XmlDOMDocument doc(parser->parseStr(prot_mod_base_data));
  ...
  if (prot_mod_ptr_NONE_ == nullptr || prot_mod_ptr_M_ACETYLATION_ == nullptr ||
      prot_mod_ptr_NME_ == nullptr) {
    LOG_ERROR("Protein modification configuration file is incomplete!");
    throw std::runtime_error(
        "Protein modification configuration file is incomplete!");
  }
}
