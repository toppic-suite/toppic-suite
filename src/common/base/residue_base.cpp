...
void ResidueBase::initBase(const std::string& base_dir) {
  ...
  std::string residue_base_file_name =
      base_dir + file_util::getFileSeparator() + "residue_base.xml";
  std::string residue_base_data = file_util::readFile(residue_base_file_name);
  XmlDOMDocument doc(parser->parseStr(residue_base_data));
  ...
  for (int i = 0; i < residue_num; i++) {
    ...
    if (residue_ptr->getAminoAcidPtr() ==
            AminoAcidBase::getEmptyAminoAcidPtr() &&
...
ResiduePtr ResidueBase::getBaseResiduePtr(const AminoAcidPtr& acid_ptr) {
  ResiduePtr residue_ptr =
      std::make_shared<Residue>(acid_ptr, PtmBase::getEmptyPtmPtr());
  return getBaseResiduePtr(residue_ptr);
}
