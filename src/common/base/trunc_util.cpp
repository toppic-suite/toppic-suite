...
bool isValidTrunc(const TruncPtr& trunc_ptr, const ResiduePtrVec& res_ptr_vec) {
  ...
  for (int i = 0; i < trunc_ptr->getTruncLen(); i++) {
    ...
    if (trunc_residue_ptr_vec[i]->getAminoAcidPtr() !=
        res_ptr_vec[i]->getAminoAcidPtr()) {
      return false;
    }
  }
  // check the second letter for NME
  ResiduePtrVec allow_first_remain_residues =
      trunc_ptr->getAllowFirstRemainResiduePtrs();
