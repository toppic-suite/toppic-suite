...
std::unordered_map<std::string, NeutralLossPtr>
    NeutralLossBase::neutral_loss_name_map_;
...
void NeutralLossBase::initBase(const std::string& base_dir) {
  ...
  std::string neutral_loss_base_file_name =
      base_dir + file_util::getFileSeparator() + "neutral_loss_base.xml";
  std::string neutral_loss_base_data =
      file_util::readFile(neutral_loss_base_file_name);
  XmlDOMDocument doc(parser->parseStr(neutral_loss_base_data));
  XmlDOMElement parent = doc.getDocumentElement();
  std::string element_name = NeutralLoss::getXmlElementName();
  int neutral_loss_num =
  ...
  if (neutral_loss_ptr_NONE_ == nullptr) {
    LOG_ERROR("Neutral loss configuration file is incomplete!");
    throw std::runtime_error("Neutral loss configuration file is incomplete!");
  }
}
