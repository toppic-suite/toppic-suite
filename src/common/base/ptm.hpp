...
class Ptm {
 public:
  Ptm(const std::string& name, const std::string& abbr_name, double mono_mass,
      int unimod_id);
  ...
  void appendAbbrNameMassToXml(XmlDOMDocument* xml_doc,
  ...
  bool isSame(const PtmPtr& ptm_ptr) const {
    return abbr_name_ == ptm_ptr->getAbbrName();
  }
