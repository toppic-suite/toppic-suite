...
std::unordered_map<std::string, ActivationPtr>
    ActivationBase::activation_name_map_;
...
void ActivationBase::initBase(const std::string& base_dir) {
  ...
  std::string activation_base_file_name =
      base_dir + file_util::getFileSeparator() + "activation_base.xml";
  std::string activation_base_data =
      file_util::readFile(activation_base_file_name);
  XmlDOMDocument doc(parser->parseStr(activation_base_data));
