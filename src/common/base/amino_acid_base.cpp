...
std::unordered_map<std::string, AminoAcidPtr>
    AminoAcidBase::amino_acid_one_letter_map_;

std::unordered_map<std::string, AminoAcidPtr>
    AminoAcidBase::amino_acid_three_letter_map_;

std::unordered_map<std::string, AminoAcidPtr>
    AminoAcidBase::amino_acid_name_map_;
...
void AminoAcidBase::initBase(const std::string& base_dir) {
  ...
  std::string amino_acid_base_file_name =
      base_dir + file_util::getFileSeparator() + "amino_acid_base.xml";
  std::string amino_acid_base_data =
      file_util::readFile(amino_acid_base_file_name);

  XmlDOMDocument doc(parser->parseStr(amino_acid_base_data));
  ...
  if (empty_amino_acid_ptr_ == nullptr) {
    ...
    throw std::runtime_error(
        "Empty amino acid cannot be found in amino acid base data!");
  }
}
...
AminoAcidPtr AminoAcidBase::getAminoAcidPtrByOneLetter(
...
AminoAcidPtr AminoAcidBase::getAminoAcidPtrByThreeLetter(
...
bool AminoAcidBase::containsName(const std::string& name) {
  return amino_acid_name_map_.find(name) != amino_acid_name_map_.end();
}

bool AminoAcidBase::containsOneLetter(const std::string& one_letter) {
  return amino_acid_one_letter_map_.find(one_letter) !=
         amino_acid_one_letter_map_.end();
}

bool AminoAcidBase::containsThreeLetter(const std::string& three_letter) {
  return amino_acid_three_letter_map_.find(three_letter) !=
         amino_acid_three_letter_map_.end();
}
