...
Trunc::Trunc(const std::string& name, int trunc_len,
  ...
  trunc_residue_ptr_vec_ =
      residue_util::convertStrToResiduePtrVec(trunc_residues);
...
Trunc::Trunc(XmlDOMElement element) {
  ...
  std::string trunc_residues =
      xml_dom_util::getChildValue(element, "trunc_residues", 0);
  LOG_DEBUG("name " << name_ << " str " << trunc_residues << " trunc len "
                    << trunc_len_);
  trunc_residue_ptr_vec_ =
      residue_util::convertStrToResiduePtrVec(trunc_residues);
