...
std::unique_ptr<pugi::xml_document> XmlDOMParser::parse(const std::string &xml_file) {
...
std::unique_ptr<pugi::xml_document> XmlDOMParser::parseStr(const std::string &xml_str) {
  auto doc = std::make_unique<pugi::xml_document>();
  pugi::xml_parse_result result = doc->load_buffer(xml_str.data(), xml_str.size());
