...
#include "common/seq/proteoform.hpp"
...
#include "common/seq/alter_type.hpp"
#include "common/seq/proteoform_factory.hpp"
...
std::string Proteoform::getMIScore() {
  ...
  for (size_t i = 0; i < mass_shift_vec.size(); i++) {
    ...
    for (int j = left_db_bp; j < right_db_bp; j++) {
      ...
      mi_score = mi_score + acid_letter + std::to_string(j - start_pos_ + 1) + ":";
      std::stringstream ss;
