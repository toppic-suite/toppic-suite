...
ResSeqPtr ResidueSeq::getSubResidueSeq(int bgn, int end) {
  ...
  // from bgn to end, the number of residues should be end - bgn + 1
  std::copy(residues_.begin() + bgn, residues_.begin() + end + 1,
            std::back_inserter(sub_residues));
  // a sub sequence keeps the N-terminal modification only when it starts at
  // the beginning of the original sequence.
  ModPtr n_mod_ptr = ModBase::getNTermNoneModPtr();
  if (bgn == 0) {
    n_mod_ptr = getNModPtr();
  }
  return std::make_shared<ResidueSeq>(sub_residues, n_mod_ptr);
}
...
std::string ResidueSeq::toAcidString() const {
  std::stringstream s;
  for (size_t i = 0; i < residues_.size(); i++) {
