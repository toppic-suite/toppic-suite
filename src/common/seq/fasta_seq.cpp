...
#include "common/seq/fasta_seq.hpp"
...
std::string FastaSeq::getSubSeq(int start, int end) const {
  std::string result;
...
std::string FastaSeq::getNameFromXml(XmlDOMElement element) {
  return xml_dom_util::getChildValue(element, "seq_name", 0);
}

std::string FastaSeq::getDescFromXml(XmlDOMElement element) {
  return xml_dom_util::getChildValue(element, "seq_desc", 0);
}

std::string FastaSeq::getAcidReplaceStr(int bgn, int end) const {
  std::string result;
