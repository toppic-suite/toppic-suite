...
inline constexpr int LOG_LEVEL_ERROR = 5;
    ...
    defined(__MINGW64__)
#define SRC_FILENAME__ \
  (strstr(__FILE__, "\\src") ? strstr(__FILE__, "\\src") + 1 : __FILE__)
#else
#define SRC_FILENAME__ \
  (strstr(__FILE__, "/src") ? strstr(__FILE__, "/src") + 1 : __FILE__)
#endif
...
#define LOG_TRACE(X)                                                  \
  do {                                                                \
    if (logger::log_level <= LOG_LEVEL_TRACE) {                       \
      std::cout << "LOG TRACE: " << SRC_FILENAME__ << "[" << __LINE__ \
                << "]: " << X << '\n';                                \
    }                                                                 \
  } while (0)
...
#define LOG_DEBUG(X)                                                  \
  do {                                                                \
    if (logger::log_level <= LOG_LEVEL_DEBUG) {                       \
      std::cout << "LOG DEBUG: " << SRC_FILENAME__ << "[" << __LINE__ \
                << "]: " << X << '\n';                                \
    }                                                                 \
  } while (0)
...
#define LOG_INFO(X)                                                  \
  do {                                                               \
    if (logger::log_level <= LOG_LEVEL_INFO) {                       \
      std::cout << "LOG INFO: " << SRC_FILENAME__ << "[" << __LINE__ \
                << "]: " << X << '\n';                               \
    }                                                                \
  } while (0)
...
#define LOG_WARN(X)                                                  \
  do {                                                               \
    if (logger::log_level <= LOG_LEVEL_WARN) {                       \
      std::cerr << "LOG WARN: " << SRC_FILENAME__ << "[" << __LINE__ \
                << "]: " << X << '\n';                               \
    }                                                                \
  } while (0)
...
#define LOG_ERROR(X)                                                  \
  do {                                                                \
    if (logger::log_level <= LOG_LEVEL_ERROR) {                       \
      std::cerr << "LOG ERROR: " << SRC_FILENAME__ << "[" << __LINE__ \
                << "]: " << X << '\n';                                \
    }                                                                 \
  } while (0)
}  // namespace toppic
