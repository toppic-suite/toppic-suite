...
void scrFilter(std::vector<double>& scr, int& bgn, int& end, double& conf,
               double threshold) {
  conf = 0.0;
...
PtmPtrVec getPtmPtrVecByMass(double mass, double err,
                             const PtmPtrVec& ptm_vec) {
  PtmPtrVec res;
  for (size_t i = 0; i < ptm_vec.size(); i++) {
    if (std::abs(ptm_vec[i]->getMonoMass() - mass) < err ||
        std::abs(std::abs(ptm_vec[i]->getMonoMass() - mass) -
                 mass_constant::getIsotopeMass()) < err) {
...
PtmPairVec getPtmPairVecByMass(double mass, double err,
                               const PtmPairVec& ptm_pair_vec) {
  PtmPairVec res;
  for (size_t i = 0; i < ptm_pair_vec.size(); i++) {
    double pair_mass = ptm_pair_vec[i].first->getMonoMass() +
                       ptm_pair_vec[i].second->getMonoMass();

    if (std::abs(pair_mass - mass) < err ||
        std::abs(std::abs(pair_mass - mass) - mass_constant::getIsotopeMass()) <
            err)
...
void compMTable(const std::vector<double>& ori_theo_masses, double shift,
                ...
                const PeakTolerancePtr& tole_ptr, std::vector<int>& m_table) {
...
void compOnePtmSTable(std::vector<int>& s_table, const ProteoformPtr& form_ptr,
                      const ExtendMsPtrVec& extend_ms_ptr_vec,
                      const PtmPtr& ptm_ptr, const LocalMngPtr& mng_ptr) {
  ...
  for (size_t i = 0; i < extend_ms_ptr_vec.size(); i++) {
    std::vector<double> ms_masses =
        extend_ms_util::getExtendMassVec(extend_ms_ptr_vec[i]);
    // updated Match table using prm masses
    double n_shift = extend_ms_ptr_vec[i]
                         ->getMsHeaderPtr()
                         ->getActivationPtr()
                         ->getN_BYShift();
    ...
    local_util::compMTable(prm_masses, n_shift + shift_mass, ms_masses,
                           tole_ptr, row_2_n);
    local_util::addTwoVectors(row_2, row_2_n);

    // updated Match table using srm masses
    double c_shift = extend_ms_ptr_vec[i]
                         ->getMsHeaderPtr()
                         ->getActivationPtr()
                         ->getC_BYShift() +
                     mass_constant::getWaterMass();
    // ActivationPtr act_ptr =
    ...
    LOG_DEBUG(std::setprecision(8) << "c shift " << c_shift);
    local_util::compMTable(srm_masses, c_shift + shift_mass, ms_masses,
                           tole_ptr, row_1_c);
...
void compTwoPtmMTable(std::vector<std::vector<int>>& m_table,
                      const ProteoformPtr& form_ptr,
                      const ExtendMsPtrVec& extend_ms_ptr_vec,
  ...
  for (size_t i = 0; i < extend_ms_ptr_vec.size(); i++) {
    ...
    compMTable(prm_masses, n_shift + mass_1 + mass_2, ms_masses, tole_ptr,
               row_3_n);
    ...
    double c_shift = extend_ms_ptr_vec[i]
                     ...
                     mass_constant::getWaterMass();
    std::vector<int> row_1_c(len + 1, 0);
    compMTable(srm_masses, c_shift + mass_1 + mass_2, ms_masses, tole_ptr,
               row_1_c);
  ...
  for (int j = 0; j < len+1; j++) {
    LOG_DEBUG(std::setprecision(8) << " m_table[0]  " << j << " " <<
  prm_masses[j] << " "  << m_table[0][j]);
  }

  for (int j = 0; j < len+1; j++) {
    LOG_DEBUG(std::setprecision(8) << " m_table[1]  " << j << " " <<
  prm_masses[j] << " "  << m_table[1][j]);
  }

  for (int j = 0; j < len+1; j++) {
    LOG_DEBUG(std::setprecision(8) << " m_table[2]  " << j << " " <<
  prm_masses[j] << " "  << m_table[2][j]);
  }
  */
}
...
int compMatchFragNum(const ProteoformPtr& proteoform_ptr,
                     const ExtendMsPtrVec& ms_ptr_vec, double min_mass) {
  return static_cast<int>(
      peak_ion_pair_util::genePeakIonPairs(proteoform_ptr, ms_ptr_vec, min_mass)
          .size());
}
