...
void getNtermTruncRange(const ProteoformPtr& proteoform,
                        const LocalMngPtr& mng_ptr, int& min, int& max) {
...
void getCtermTruncRange(const ProteoformPtr& proteoform,
                        const LocalMngPtr& mng_ptr, int& min, int& max) {
  int ori_start = proteoform->getStartPos();
  int ori_end = proteoform->getEndPos();
  ...
  int raw_seq_len =
      static_cast<int>(proteoform->getFastaSeqPtr()->getRawSeq().length());
...
ProteoformPtrVec getCandidateForm(const FastaSeqPtr& seq_ptr, int ori_start_pos,
  ...
  for (size_t i = 0; i < exp_shift_ptr_vec.size(); i++) {
    ...
    if (shift_start_pos >= form_start_pos && shift_end_pos <= form_end_pos) {
      int left_bp_pos =
          shift_ptr->getLeftBpPos() + ori_start_pos - form_start_pos;
      int right_bp_pos =
          shift_ptr->getRightBpPos() + ori_start_pos - form_start_pos;
      AlterPtr alter_ptr = std::make_shared<Alter>(
          left_bp_pos, right_bp_pos, shift_ptr->getTypePtr(),
          shift_ptr->getMassShift(), shift_ptr->getAlterPtr(0)->getModPtr());
  ...
  std::string sub_seq =
      whole_seq.substr(form_start_pos, form_end_pos - form_start_pos + 1);
  ModPtrVec fixed_mod_ptr_vec = mng_ptr->prsm_para_ptr_->getFixModPtrVec();
  ResSeqPtr db_res_seq_ptr = std::make_shared<ResidueSeq>(
      residue_util::convertStrToResiduePtrVec(whole_seq, fixed_mod_ptr_vec));
  ResSeqPtr sub_res_seq_ptr = std::make_shared<ResidueSeq>(
      residue_util::convertStrToResiduePtrVec(sub_seq, fixed_mod_ptr_vec));

  // obtain protein mod
  ProtModPtr prot_mod_ptr = ProtModBase::getProtModPtr_NONE();
  if (form_start_pos == 1) {
    ProtModPtr nme = ProtModBase::getProtModPtr_NME();
    if (prot_mod_util::containMod(mng_ptr->prsm_para_ptr_->getProtModPtrVec(),
                                  nme) &&
  ...
  ProteoformPtr form_ptr = std::make_shared<Proteoform>(
      seq_ptr, prot_mod_ptr, form_start_pos, form_end_pos, sub_res_seq_ptr,
      valid_shift_ptr_vec);
  ...
  if (form_start_pos == 0) {
    if (prot_mod_util::containMod(prot_mod_list, m_actyl) &&
      ...
      AlterPtr alter_ptr =
          std::make_shared<Alter>(pos, pos + 1, AlterType::PROTEIN_VARIABLE,
      ...
      form_ptr = std::make_shared<Proteoform>(seq_ptr, prot_mod_ptr,
                                              form_start_pos, form_end_pos,
                                              new_seq_ptr, valid_shift_ptr_vec);
      result_vec.push_back(form_ptr);
    }
...
ProteoformPtrVec getAllCandidateForms(const ProteoformPtr& ori_form_ptr,
  ...
  MassShiftPtrVec tmp_vec =
      local_util::massShiftFilter(ori_shift_ptr_vec, AlterType::UNEXPECTED);
  MassShiftPtrVec exp_shift_ptr_vec =
      local_util::massShiftFilter(tmp_vec, AlterType::PROTEIN_VARIABLE);
  ...
  ProteoformPtrVec base_form_ptr_vec =
      getCandidateForm(fasta_seq_ptr, ori_start_pos, ori_start_pos, ori_end_pos,
                       exp_shift_ptr_vec, mng_ptr);
  result_form_vec.insert(std::end(result_form_vec),
                         std::begin(base_form_ptr_vec),
                         std::end(base_form_ptr_vec));
  ...
  for (int i = start_pos_min; i <= start_pos_max; i++) {
    ...
    ProteoformPtrVec form_ptr_vec =
        getCandidateForm(fasta_seq_ptr, ori_start_pos, i, ori_end_pos,
                         exp_shift_ptr_vec, mng_ptr);
    result_form_vec.insert(std::end(result_form_vec), std::begin(form_ptr_vec),
                           std::end(form_ptr_vec));
  }
  ...
  for (int i = end_pos_min; i <= end_pos_max; i++) {
    ...
    ProteoformPtrVec form_ptr_vec =
        getCandidateForm(fasta_seq_ptr, ori_start_pos, ori_start_pos, i,
                         exp_shift_ptr_vec, mng_ptr);
    result_form_vec.insert(std::end(result_form_vec), std::begin(form_ptr_vec),
                           std::end(form_ptr_vec));
  }
...
ProteoformPtr createProteoformPtr(const ProteoformPtr& base_form_ptr,
                                  int match_score, std::vector<double> scr_vec,
  ...
  ModPtr mod_ptr = std::make_shared<Mod>(
      ResidueBase::getEmptyResiduePtr(),
      ResidueBase::getBaseResiduePtr(empty_aa_ptr, ptm_ptr),
      ModType::SIDE_CHAIN);

  AlterPtr alter = std::make_shared<Alter>(
      anno->getLeftBpPos(), anno->getRightBpPos() + 1, AlterType::VARIABLE,
      ptm_ptr->getMonoMass(), mod_ptr);
  ...
  std::sort(all_shift_ptr_vec.begin(), all_shift_ptr_vec.end(),
            MassShift::cmpPosInc);

  ProteoformPtr shift_form_ptr = std::make_shared<Proteoform>(
      base_form_ptr->getFastaSeqPtr(), base_form_ptr->getProtModPtr(),
      base_form_ptr->getStartPos(), base_form_ptr->getEndPos(),
      base_form_ptr->getResSeqPtr(), all_shift_ptr_vec);
...
ProteoformPtr createProteoformPtr(const ProteoformPtr& base_form_ptr,
  ...
  LocalAnnoPtr anno_1 = std::make_shared<LocalAnno>(
      bgn_1, end_1, conf_1, scr_vec_1, match_score, ptm_ptr_1);

  AminoAcidPtr empty_aa_ptr = AminoAcidBase::getEmptyAminoAcidPtr();
  ModPtr mod_ptr_1 = std::make_shared<Mod>(
      ResidueBase::getEmptyResiduePtr(),
      ResidueBase::getBaseResiduePtr(empty_aa_ptr, ptm_ptr_1),
      ModType::SIDE_CHAIN);

  AlterPtr alter_1 = std::make_shared<Alter>(
      anno_1->getLeftBpPos(), break_pos + 1, AlterType::VARIABLE,
      ptm_ptr_1->getMonoMass(), mod_ptr_1);

  alter_1->setLocalAnno(anno_1);

  LocalAnnoPtr anno_2 = std::make_shared<LocalAnno>(
      bgn_2, end_2, conf_2, scr_vec_2, match_score, ptm_ptr_2);

  ModPtr mod_ptr_2 = std::make_shared<Mod>(
      ResidueBase::getEmptyResiduePtr(),
      ResidueBase::getBaseResiduePtr(empty_aa_ptr, ptm_ptr_2),
      ModType::SIDE_CHAIN);
  AlterPtr alter_2 = std::make_shared<Alter>(
      break_pos + 1, anno_2->getRightBpPos() + 1, AlterType::VARIABLE,
      ptm_ptr_2->getMonoMass(), mod_ptr_2);
