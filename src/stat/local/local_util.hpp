...
#ifndef TOPPIC_STAT_LOCAL_LOCAL_UTIL_HPP_
#define TOPPIC_STAT_LOCAL_LOCAL_UTIL_HPP_
...
MassShiftPtrVec massShiftFilter(const MassShiftPtrVec & mass_shift_vec, const AlterTypePtr &type);
...
void compTwoPtmMTable(std::vector<std::vector<int>> &s_table, const ProteoformPtr &form_ptr, 
                      const ExtendMsPtrVec & extend_ms_ptr_vec,
                      const PtmPtr &ptm_ptr_1, const PtmPtr &ptm_ptr_2, const LocalMngPtr &mng_ptr); 

void normalize(std::vector<double> & scr);

int compMatchFragNum(const ProteoformPtr &proteoform_ptr, const ExtendMsPtrVec &ms_ptr_vec, double min_mass);
