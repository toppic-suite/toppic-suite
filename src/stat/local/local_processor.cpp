...
LocalProcessor::LocalProcessor(const LocalMngPtr& mng_ptr) : mng_ptr_(mng_ptr) {
  init();
}
...
void LocalProcessor::init() {
  ...
  std::vector<ModPtrVec> mod_ptr_vec2d =
      mod_util::readModTxt(mng_ptr_->residueModFileName_);
...
void LocalProcessor::process() {
  ...
  std::string input_file_name =
      file_util::basename(spec_file_name) + "." + mng_ptr_->input_file_ext_;
  std::string output_file_name =
      file_util::basename(spec_file_name) + "." + mng_ptr_->output_file_ext_;
  PrsmXmlWriterPtr prsm_writer =
      std::make_shared<PrsmXmlWriter>(output_file_name);
  std::string db_file_name =
      mng_ptr_->prsm_para_ptr_->getSearchDbFileNameWithFolder();
  ...
  MsAlignReaderPtr reader_ptr = std::make_shared<MsAlignReader>(
      spec_file_name, group_spec_num, sp_para_ptr->getActivationPtr());

  SpectrumSetPtr spec_set_ptr;

  int spectrum_num =
      msalign_util::getSpNum(mng_ptr_->prsm_para_ptr_->getSpectrumFileName());
    ...
    if (header_ptr->containsPrec()) {
      ...
      if (spec_set_ptr->isValid()) {
        ...
        while (prsm_ptr != nullptr && prsm_ptr->getSpectrumId() == spec_id) {
          ...
          if (prsm_ptr->getProteoformPtr()->getAlterNum(AlterType::UNEXPECTED) >
              0) {
...
PrsmPtr LocalProcessor::processOnePrsm(const PrsmPtr& prsm) {
  int mass_shift_num =
      prsm->getProteoformPtr()->getAlterNum(AlterType::UNEXPECTED);
  double err_tole = mng_ptr_->peak_tole_ptr_->compStrictErrorTole(
      prsm->getAdjustedPrecMass());
  if (mass_shift_num == 1) {
    double mass = prsm->getProteoformPtr()
                      ->getMassShiftPtrVec(AlterType::UNEXPECTED)[0]
                      ->getMassShift();
...
PrsmPtr LocalProcessor::processOneMassShift(const PrsmPtr& prsm) {
  int ori_num_match_ion = local_util::compMatchFragNum(
      prsm->getProteoformPtr(), prsm->getRefineMsPtrVec(), mng_ptr_->min_mass_);

  // we will get a nullptr if the mass shift can't be explained by a known
  // variable ptm
  ProteoformPtr one_known_proteoform = processOneKnownPtm(prsm);

  if (one_known_proteoform != nullptr) {
    int new_num_match_ion = local_util::compMatchFragNum(
        one_known_proteoform, prsm->getRefineMsPtrVec(), mng_ptr_->min_mass_);
  ...
  if (two_known_proteoform != nullptr) {
    double new_num_match_ion =
  local_util::compMatchFragNum(two_known_proteoform, prsm->getRefineMsPtrVec(),
                                                            mng_ptr_->min_mass_);
    //LOG_DEBUG("orig score " << ori_num_match_ion << " new match num " <<
  new_num_match_ion);
    //LOG_DEBUG("two known form " <<
  two_known_proteoform->getProteinMatchSeq()); if (new_num_match_ion >
  ori_num_match_ion - mng_ptr_->DESC_MATCH_LIMIT_
        && new_num_match_ion > ori_num_match_ion * mng_ptr_->desc_ratio_) {
  ...
  mng_ptr_->prsm_para_ptr_->getSpParaPtr()); return prsm;
    }
  }
  */
  return prsm;
...
PrsmPtr LocalProcessor::processTwoMassShifts(const PrsmPtr& prsm) {
  int ori_num_match_ion = local_util::compMatchFragNum(
      prsm->getProteoformPtr(), prsm->getRefineMsPtrVec(), mng_ptr_->min_mass_);
  ...
  if (two_known_proteoform != nullptr) {
    int new_num_match_ion = local_util::compMatchFragNum(
        two_known_proteoform, prsm->getRefineMsPtrVec(), mng_ptr_->min_mass_);
    if (new_num_match_ion > ori_num_match_ion - mng_ptr_->DESC_MATCH_LIMIT_ &&
        new_num_match_ion > ori_num_match_ion * mng_ptr_->desc_ratio_) {
...
bool LocalProcessor::modifiable(const ProteoformPtr& proteoform_ptr, int i,
                                const PtmPtr& ptm_ptr) {
  if (ptm_ptr == nullptr) return true;

  MassShiftPtrVec fixed_shift_vec =
      proteoform_ptr->getMassShiftPtrVec(AlterType::FIXED);

  for (size_t k = 0; k < fixed_shift_vec.size(); k++) {
    if (fixed_shift_vec[k]->getLeftBpPos() <= i &&
        i < fixed_shift_vec[k]->getRightBpPos()) {
      return false;
    }
  }
...
ProteoformPtr LocalProcessor::processOneKnownPtm(const PrsmPtr& prsm_ptr) {
  ...
  ProteoformPtrVec cand_form_vec =
      local_proteoform::getAllCandidateForms(ori_form_ptr, mng_ptr_);

  ExtendMsPtrVec extend_ms_ptr_vec = prsm_ptr->getRefineMsPtrVec();
  double adjust_prec_mass = prsm_ptr->getAdjustedPrecMass();
  double err_tole =
      mng_ptr_->peak_tole_ptr_->compStrictErrorTole(adjust_prec_mass);
  ...
  for (size_t i = 0; i < cand_form_vec.size(); i++) {
    ...
    LOG_DEBUG(std::setprecision(10)
              << "adjust prec mass " << adjust_prec_mass << " form mass "
              << cand_form_ptr->getMass());
    // Get candidate Ptms with similar mass shifts
    PtmPtrVec match_ptm_ptr_vec = local_util::getPtmPtrVecByMass(
        unexp_shift_mass, err_tole, ptm_ptr_vec_);
    LOG_DEBUG("ptm number " << match_ptm_ptr_vec.size() << " shift "
                            << unexp_shift_mass);
...
int LocalProcessor::compOnePtmScr(const ProteoformPtr& base_form_ptr,
  ...
  local_util::compOnePtmSTable(s_table, base_form_ptr, extend_ms_ptr_vec,
                               ptm_ptr, mng_ptr_);
...
ProteoformPtr LocalProcessor::onePtmLocalize(
    const ProteoformPtr& base_form_ptr, const ExtendMsPtrVec& extend_ms_ptr_vec,
    double shift_mass, int match_score, const PtmPtr& ptm_ptr) {
  int len = base_form_ptr->getLen();
  // score table with size len
  std::vector<int> s_table;
  local_util::compOnePtmSTable(s_table, base_form_ptr, extend_ms_ptr_vec,
  ...
  ProteoformPtr local_form_ptr = local_proteoform::createProteoformPtr(
      base_form_ptr, match_score, scr_vec, ptm_ptr, mng_ptr_);
  return local_form_ptr;
}

bool massShiftOverlap(const ProteoformPtr& form_ptr_1,
                      const ProteoformPtr& form_ptr_2) {
  MassShiftPtrVec shift_ptr_vec_1 =
      form_ptr_1->getMassShiftPtrVec(AlterType::UNEXPECTED);
  std::sort(shift_ptr_vec_1.begin(), shift_ptr_vec_1.end(),
            MassShift::cmpPosInc);
  MassShiftPtrVec shift_ptr_vec_2 =
      form_ptr_2->getMassShiftPtrVec(AlterType::VARIABLE);
  std::sort(shift_ptr_vec_2.begin(), shift_ptr_vec_2.end(),
            MassShift::cmpPosInc);
...
ProteoformPtr LocalProcessor::processTwoKnownPtms(const PrsmPtr& prsm_ptr) {
  ProteoformPtr ori_form_ptr = prsm_ptr->getProteoformPtr();
  ProteoformPtrVec cand_form_vec =
      local_proteoform::getAllCandidateForms(ori_form_ptr, mng_ptr_);
  ...
  for (size_t i = 0; i < cand_form_vec.size(); i++) {
    ...
    // sum of all expected shift values, in most cases, there is only one
    // unexpected shift
    double unexp_shift_mass = adjust_prec_mass - cand_form_ptr->getMass();
    // LOG_DEBUG("Get PTM by Mass");
    //  Get candidate Ptm pairs with similar mass shifts
    PtmPairVec ptm_pair_vec = local_util::getPtmPairVecByMass(
        unexp_shift_mass, err_tole, ptm_pair_vec_);
  ...
  if (best_match_score <= 0 ||
    ...
    ProteoformPtr result_ptr =
        twoPtmLocalize(best_form_ptr, extend_ms_ptr_vec, best_match_score,
                       best_ptm_pair.first, best_ptm_pair.second);
    // check if the PTM localization results are overlapping with the original
    // mass shifts

    // result_ptr can be null when the proteoform with ptm yields low score than
    // filtering threshold in that case, running massShiftOverlap causes seg
    // fault
...
int LocalProcessor::compTwoPtmScr(const ProteoformPtr& form_ptr,
                                  const ExtendMsPtrVec& extend_ms_ptr_vec,
                                  const PtmPtr& ptm_ptr_1,
                                  const PtmPtr& ptm_ptr_2) {
  ...
  local_util::compTwoPtmMTable(m_table, form_ptr, extend_ms_ptr_vec, ptm_ptr_1,
                               ptm_ptr_2, mng_ptr_);
  ...
  for (int i = 1; i <= len; i++) {
    ...
    if (modifiable(form_ptr, i - 1, ptm_ptr_1) &&
        d_table[0][i - 1] > d_table[1][i - 1]) {
      // LOG_DEBUG("MODIFY" << i );
  ...
  for (int i = 1; i <= len; i++) {
    ...
    if (modifiable(form_ptr, i - 1, ptm_ptr_2) &&
        d_table[1][i - 1] > d_table[2][i - 1]) {
...
ProteoformPtr LocalProcessor::twoPtmLocalize(
    const ProteoformPtr& form_ptr, const ExtendMsPtrVec& extend_ms_ptr_vec,
  ...
  std::vector<std::vector<int>> mid_scores(len + 1,
  ...
  for (size_t i = 0; i < ptm_1_pos_list.size(); i++) {
    ...
    for (size_t j = 0; j < ptm_2_pos_list.size(); j++) {
      ...
      if (pos_2 > pos_1) {
        int score = n_term_scores[pos_1] + c_term_scores[pos_2 + 1] +
                    mid_scores[pos_1 + 1][pos_2];
        b_table[pos_1][pos_2] = std::pow(mng_ptr_->prob_ratio_, score);
        total_prob = total_prob + b_table[pos_1][pos_2];
        // LOG_DEBUG("pos " << pos_1 << " " << pos_2 << " " << score << " " <<
        // b_table[pos_1][pos_2] << " " << total_prob);
      }
    }
  }
  ...
  int best_break_pos =
      std::distance(break_scores.begin(),
                    std::max_element(break_scores.begin(), break_scores.end()));
  ...
  for (size_t i = 0; i < ptm_1_pos_list.size(); i++) {
    ...
    for (int j = ptm_2_pos_list.size() - 1; j >= 0; j--) {
      int pos_2 = ptm_2_pos_list[j];
      // LOG_DEBUG("pos 1 " << pos_1 << " pos 2 " << pos_2 << " " <<
      // b_table[pos_1][pos_2] << " " << pos_scr);
  ...
  ProteoformPtr local_form_ptr = local_proteoform::createProteoformPtr(
      form_ptr, match_score, best_break_pos, scr_vec_1, scr_vec_2, ptm_ptr_1,
      ptm_ptr_2, mng_ptr_);
