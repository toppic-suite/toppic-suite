...
std::map<int, std::vector<std::string> > readMassTable(
...
std::map<int, std::vector<std::string> > geneMassTable(
    const MCMCMngPtr& mng_ptr) {
...
std::map<int, std::vector<std::string> > geneMassTableFixMod(
    const MCMCMngPtr& mng_ptr) {
  double convert_ratio = mng_ptr->convert_ratio_;

  ResiduePtrVec res_vec = residue_util::convertStrToResiduePtrVec(
      "GASPVTCLNDQKEMHFURYWO", mng_ptr->prsm_para_ptr_->getFixModPtrVec());
  ...
  for (size_t i = 0; i < res_vec.size(); i++) {
    for (size_t j = 0; j < res_vec.size(); j++) {
      for (size_t k = 0; k < res_vec.size(); k++) {
        ...
        int m = std::round((res_vec[i]->getMass() + res_vec[j]->getMass() +
                            res_vec[k]->getMass()) *
                           convert_ratio);
        std::string str_tmp;
  ...
  for (auto it = aa_pair_mass_map_.begin(); it != aa_pair_mass_map_.end();
       it++) {
  ...
  for (size_t i = 0; i < aa_pair_mass_vec.size(); i++) {
    ...
    for (int k = i - 1; k >= 0; k--) {
      ...
      seq_vec.insert(seq_vec.end(), aa_pair_mass_vec[k].second.begin(),
                     aa_pair_mass_vec[k].second.end());
    }
