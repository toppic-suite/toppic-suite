...
void DprProcessor::init() {
  ...
  for (size_t i = 0; i < var_mod_ptr_vec.size(); i++) {
    ptm_residue_map_[var_mod_ptr_vec[i]->getModResiduePtr()->getPtmPtr()]
        .push_back(var_mod_ptr_vec[i]->getOriResiduePtr());
  }

  mass_table_ = mass_table_util::geneMassTable(mng_ptr_);

  // TdgfMngPtr tdgf_mng_ptr
  //     = std::make_shared<TdgfMng>(mng_ptr_->prsm_para_ptr_, 0, 0.0, false,
  //     false, 1, "", "");

  test_num_ptr_ = std::make_shared<CountTestNum>(
      mng_ptr_->convert_ratio_, mng_ptr_->max_ptm_mass_,
      mng_ptr_->max_prec_mass_, mng_ptr_->prsm_para_ptr_);

  ptm_mass_vec2d_ = compPtmComb();
}

std::vector<std::vector<double> > DprProcessor::compPtmComb() {
  std::vector<std::vector<int> > mass_ptm_vec2d_int(mng_ptr_->max_known_mods_ +
                                                    1);
  std::vector<std::vector<double> > ptm_mass_vec2d(mng_ptr_->max_known_mods_ +
                                                   1);
  ...
  for (size_t i = 0; i < ptm_vec_.size(); i++) {
    ...
    bool found = false;
    for (auto it = mass_ptm_vec2d_int[1].begin();
         it != mass_ptm_vec2d_int[1].end(); it++) {
  ...
  for (int k = 2; k <= mng_ptr_->max_known_mods_; k++) {
    ...
    for (auto it = mass_ptm_vec2d_int[k - 1].begin();
         it != mass_ptm_vec2d_int[k - 1].end(); it++) {
      cur_vec_mass.push_back(*it);
    }

    for (size_t i = 0; i < ptm_vec_.size(); i++) {
      int mass =
          std::ceil(ptm_vec_[i]->getMonoMass() * mng_ptr_->convert_ratio_);
  ...
  for (size_t i = 0; i < mass_ptm_vec2d_int.size(); i++) {
    for (size_t k = 0; k < mass_ptm_vec2d_int[i].size(); k++) {
      ptm_mass_vec2d[i].push_back(mass_ptm_vec2d_int[i][k] /
...
void DprProcessor::process() {
  ...
  PrsmXmlWriterPtr prsm_writer = std::make_shared<PrsmXmlWriter>(
      output_file_name + "_" + std::to_string(mng_ptr_->thread_num_));

  writer_ptr_vec_ = prsm_xml_writer_util::geneWriterPtrVec(
  ...
  PrsmPtr prsm_ptr = prsm_reader->readOnePrsm(fasta_reader_ptr,
  ...
  while (deconv_ms_ptr_vec.size() != 0) {
    ...
    if (header_ptr->containsPrec()) {
      ...
      double prec_mono_mass = header_ptr->getFirstPrecMonoMass() -
                              sp_para_ptr_->getNTermLabelMass();
      SpectrumSetPtr spec_set_ptr = spectrum_set_factory::geneSpectrumSetPtr(
          deconv_ms_ptr_vec, sp_para_ptr_, prec_mono_mass);
      ...
      if (spec_set_ptr->isValid()) {
        ...
        while (prsm_ptr != nullptr && prsm_ptr->getSpectrumId() == spec_id) {
          ...
          ExtendMsPtrVec refine_ms_ptr_vec =
              extend_ms_factory::geneMsThreePtrVec(
                  deconv_ms_ptr_vec, sp_para_ptr_,
                  prsm_ptr->getAdjustedPrecMass());

          processOnePrsm(prsm_ptr, spec_set_ptr, prsm_writer);

          prsm_ptr = prsm_reader->readOnePrsm(fasta_reader_ptr,
                                              prsm_para_ptr->getFixModPtrVec());
        }
      }
    }
    std::cout << std::flush << "E-value computation - processing " << cnt
  ...
  PrsmStrMergePtr merge_ptr = std::make_shared<PrsmStrMerge>(
      sp_file_name, input_exts, mng_ptr_->output_file_ext_, prsm_top_num, norm,
      remove_dup);
  merge_ptr->process();
  merge_ptr = nullptr;
...
std::function<void()> geneTask(
    const SpectrumSetPtr& spec_set_ptr, const PrsmPtr& prsm_ptr,
    const MCMCMngPtr& mng_ptr,
    const std::map<PtmPtr, std::vector<ResiduePtr> >& ptm_residue_map,
    const std::map<int, std::vector<std::string> >& mass_table,
    const CountTestNumPtr& test_num_ptr,
  ...
  return [spec_set_ptr, prsm_ptr, mng_ptr, ptm_residue_map, mass_table,
          test_num_ptr, ptm_mass_vec2d, pool_ptr, writer_ptr_vec]() {
    CompPValueMCMCPtr comp_mcmc_ptr = std::make_shared<toppic::CompPValueMCMC>(
        mng_ptr, ptm_residue_map, mass_table);
    DeconvMsPtrVec deconv_ms_ptr_vec = spec_set_ptr->getDeconvMsPtrVec();
    ExtendMsPtrVec refine_ms_ptr_vec = extend_ms_factory::geneMsThreePtrVec(
        deconv_ms_ptr_vec, mng_ptr->prsm_para_ptr_->getSpParaPtr(),
        prsm_ptr->getAdjustedPrecMass());

    double ppo = mng_ptr->prsm_para_ptr_->getSpParaPtr()
                     ->getPeakTolerancePtr()
                     ->getPpo();

    double tolerance =
        refine_ms_ptr_vec[0]->getMsHeaderPtr()->getFirstPrecErrorTolerance(ppo);

    std::vector<double> ms_masses =
        extend_ms_util::getExtendMassVec(refine_ms_ptr_vec[0]);
    ...
    for (size_t k = 0; k < ms_masses.size(); k++) {
      ms_mass_int[k] =
          static_cast<int>(ms_masses[k] * mng_ptr->convert_ratio_) >> 5;
    }

    std::sort(ms_mass_int.begin(), ms_mass_int.end());

    ms_mass_int.erase(std::unique(ms_mass_int.begin(), ms_mass_int.end()),
                      ms_mass_int.end());

    double one_prob =
        comp_mcmc_ptr->compOneProbMCMC(prsm_ptr, act, ms_mass_int);

    double cand_num = 0;

    ProteoformTypePtr type_ptr =
        prsm_ptr->getProteoformPtr()->getProteoformType();

    int unexpect_shift_num =
        prsm_ptr->getProteoformPtr()->getAlterNum(AlterType::UNEXPECTED);

    if (prsm_ptr->getProteoformPtr()->getAlterNum(AlterType::VARIABLE) == 0) {
      cand_num = test_num_ptr->compCandNum(
          type_ptr, unexpect_shift_num,
          prsm_ptr->getAdjustedPrecMass() - mass_constant::getWaterMass(),
          tolerance);
    } else {
      std::vector<double> mass_ptm_vec =
          ptm_mass_vec2d[prsm_ptr->getProteoformPtr()->getAlterNum(
              AlterType::VARIABLE)];
    ...
    ExpectedValuePtr evalue =
        std::make_shared<ExpectedValue>(one_prob, cand_num, 0.005);
...
void DprProcessor::processOnePrsm(const PrsmPtr& prsm_ptr,
  ...
  pool_ptr_->enqueue(geneTask(spec_set_ptr, prsm_ptr, mng_ptr_,
                              ptm_residue_map_, mass_table_, test_num_ptr_,
                              ptm_mass_vec2d_, pool_ptr_, writer_ptr_vec_));
}
