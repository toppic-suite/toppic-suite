...
int getMaxShift(const PrsmPtrVec& prsm_ptrs) {
  int shift = 0;
  for (size_t i = 0; i < prsm_ptrs.size(); i++) {
    if (prsm_ptrs[i]->getProteoformPtr()->getAlterNum(AlterType::UNEXPECTED) >
        shift) {
      shift =
          prsm_ptrs[i]->getProteoformPtr()->getAlterNum(AlterType::UNEXPECTED);
    }
  }
  return shift;
}

void compProbArray(const CompProbValuePtr& comp_prob_ptr,
                   const ResFreqPtrVec& n_term_residue_ptrs,
                   const PrmPeakPtrVec2D& peak_ptr_2d,
                   const PrsmPtrVec& prsm_ptrs, bool strict,
                   double prob_prec_mass, const PeakTolerancePtr& tole_ptr,
