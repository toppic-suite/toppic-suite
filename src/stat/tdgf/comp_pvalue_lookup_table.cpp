...
CompPValueLookupTable::CompPValueLookupTable(const TdgfMngPtr &mng_ptr) {
  mng_ptr_ = mng_ptr;
  initTable();
...
void CompPValueLookupTable::initTable() {
  ...
  input_.open(
              ...
              + file_util::getFileSeparator() + "ppm" + std::to_string(ppm) + "_ptm0.txt",
              std::ios::in);
  if (!input_.is_open()) {
  ...
  input_.open(
              mng_ptr_->prsm_para_ptr_->getResourceDir() + file_util::getFileSeparator() + "p_value_table"
              + file_util::getFileSeparator() + "ppm" + std::to_string(ppm) + "_ptm1.txt",
              std::ios::in);
  ...
  input_.open(
              mng_ptr_->prsm_para_ptr_->getResourceDir() + file_util::getFileSeparator() + "p_value_table"
              + file_util::getFileSeparator() + "ppm" + std::to_string(ppm) + "_ptm2.txt",
              std::ios::in);
  if (!input_.is_open()) {
    LOG_ERROR("Exception in reading the lookup table file for E-value computation!");
    exit(EXIT_FAILURE);
