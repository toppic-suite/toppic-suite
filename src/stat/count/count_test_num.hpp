...
class CountTestNum {
 public:
  CountTestNum(double convert_ratio, double max_ptm_mass, 
               double max_prec_mass, const PrsmParaPtr &para_ptr);
 ...
 private:
  ...
  double compNonPtmCandNum(const ProteoformTypePtr &type_ptr, 
                           double ori_mass, double ori_tolerance);

  double compPtmCandNum(const ProteoformTypePtr &type_ptr);

  double compPtmRestrictCandNum (const ProteoformTypePtr &type_ptr, int shift_num, double ori_mass);
