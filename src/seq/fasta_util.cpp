...
void generateDbBlock(const std::string& db_file_name, int block_size,
  ...
  while (seq_info != nullptr) {
    ...
    if (seq_len <= max_frag_len) {
      ...
      total_seq_size = total_seq_size +
                       (seq_len - max_frag_len) * max_frag_len +
...
void checkDuplicate(std::string file_name, std::string idx_folder_name) {
  ...
  for (int i = 0; i < seq_num; i++) {
    ...
    if (!name_set.insert(name).second) {
      ...
      LOG_ERROR("The protein sequence database contains duplicated sequences: "
                << name);
      exit(EXIT_FAILURE);
    }
  }
...
void dbSimplePreprocess(const std::string& ori_db_file_name,
  ...
  if (!std::filesystem::exists(idx_folder_name)) {
  ...
  if (std::filesystem::exists(db_file_name + ".fai")) {
    return;
  }
...
void dbPreprocess(const std::string& ori_db_file_name,
                  const std::string& file_name, bool decoy, int block_size,
                  int max_frag_len, int min_block_num) {
  std::string idx_folder_name = ori_db_file_name + "_idx";
  if (!std::filesystem::exists(idx_folder_name)) {
    // if _idx folder doesn't exist yet
  ...
  std::string standard_db_file_name =
  ...
  if (standard_db_file_name.size() > 200 || new_db_file_name.size() > 200) {
    LOG_ERROR(
        "Database file name is too long. Please rename it and try running "
        "again.");
    exit(1);
  }
  if (!std::filesystem::exists(standard_db_file_name)) {
    generateStandardDb(ori_db_file_name, standard_db_file_name);
  }

  if (decoy) {
    if (!std::filesystem::exists(new_db_file_name)) {
  ...
  if (!std::filesystem::exists(new_db_file_name + "_block_index") ||
      !std::filesystem::exists(new_db_file_name + "_0")) {
    generateDbBlock(new_db_file_name, block_size, max_frag_len, min_block_num);
  }

  // Generate new database file index
  if (!std::filesystem::exists(new_db_file_name + ".fai")) {
