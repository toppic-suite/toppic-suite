...
ProteoformPtr geneDbProteoformPtr(const FastaSeqPtr& fasta_seq_ptr,
  ...
  ResiduePtrVec residue_ptrs = residue_util::convertStrToResiduePtrVec(
      fasta_seq_ptr->getAcidPtmPairVec());
  ...
  for (size_t i = 0; i < residue_ptrs.size(); i++) {
    if (residue_ptrs[i]->getPtmPtr() != PtmBase::getEmptyPtmPtr()) {
      ...
      AlterPtr alter_ptr = std::make_shared<Alter>(
          i, i + 1, AlterType::INPUT, mod_ptr->getShift(), mod_ptr);
  ...
  for (size_t i = 0; i < residue_ptrs.size(); i++) {
    for (size_t j = 0; j < fix_mod_list.size(); j++) {
      if (residue_ptrs[i] == fix_mod_list[j]->getOriResiduePtr()) {
        ...
        AlterPtr alter_ptr = std::make_shared<Alter>(
            i, i + 1, AlterType::FIXED, fix_mod_list[j]->getShift(),
            fix_mod_list[j]);
        MassShiftPtr shift_ptr = std::make_shared<MassShift>(alter_ptr);
        shift_list.push_back(shift_ptr);
        break;
  ...
  return std::make_shared<Proteoform>(fasta_seq_ptr, none_prot_mod_ptr,
                                      start_pos, end_pos, res_seq_ptr,
                                      shift_list);
}
...
ProteoformPtr geneProtModProteoform(const ProteoformPtr& db_form_ptr,
  ...
  bool valid_mod =
      prot_mod_util::allowMod(prot_mod_ptr, db_res_seq_ptr->getResidues());
  if (!valid_mod) {
  ...
  // copy input changes
  const MassShiftPtrVec& ori_shift_ptrs = db_form_ptr->getMassShiftPtrVec();
  ...
  for (size_t i = 0; i < ori_shift_ptrs.size(); i++) {
    if (ori_shift_ptrs[i]->getLeftBpPos() >= start &&
      ...
      MassShiftPtr shift_ptr =
          std::make_shared<MassShift>(ori_shift_ptrs[i], start);
      shift_ptrs.push_back(shift_ptr);
    }
  }

  const ResiduePtrVec& ori_vec = db_res_seq_ptr->getResidues();
  ...
  if (!ModBase::isNTermNoneModPtr(n_term_mod_ptr)) {
    ...
    AlterPtr alter_ptr = std::make_shared<Alter>(
        new_pos, new_pos + 1, AlterType::PROTEIN_VARIABLE,
        n_term_mod_ptr->getShift(), n_term_mod_ptr);
    MassShiftPtr shift_ptr = std::make_shared<MassShift>(alter_ptr);
    shift_ptrs.push_back(shift_ptr);
  }
  ...
  return std::make_shared<Proteoform>(db_form_ptr->getFastaSeqPtr(),
                                      prot_mod_ptr, start, end, seq_ptr,
                                      shift_ptrs);
}
...
ProteoformPtr geneSubProteoform(const ProteoformPtr& proteoform_ptr,
  ...
  const ResiduePtrVec& ori_residues =
      proteoform_ptr->getResSeqPtr()->getResidues();
  ResiduePtrVec residues(ori_residues.begin() + residue_start,
                         ori_residues.begin() + residue_end + 1);
  // if the sub-proteoform starts from the first residue, keep the N-terminal
  // mod otherwise, set it to none
  ...
  const MassShiftPtrVec& ori_shift_list = proteoform_ptr->getMassShiftPtrVec();
  MassShiftPtrVec shift_list;
  ...
  for (size_t i = 0; i < ori_shift_list.size(); i++) {
    if (ori_shift_list[i]->getLeftBpPos() >= residue_start &&
      ...
      MassShiftPtr shift_ptr =
          std::make_shared<MassShift>(ori_shift_list[i], residue_start);
      shift_list.push_back(shift_ptr);
    }
  }
  ...
  return std::make_shared<Proteoform>(
      fasta_seq_ptr, prot_mod_ptr,
      residue_start + proteoform_ptr->getStartPos(),
      residue_end + proteoform_ptr->getStartPos(), seq_ptr, shift_list);
}
