...
BpSpec::BpSpec(const ResSeqPtr& res_seq_ptr) { initBreakPoints(res_seq_ptr); }
...
void BpSpec::initBreakPoints(const ResSeqPtr& res_seq_ptr) {
  ...
  for (int i = 0; i < res_seq_ptr->getLen() - 1; i++) {
    ...
    if (srm < 0) {
      LOG_WARN(
          "Prefix residue mass is larger than the total mass of the protein!");
    }
...
std::vector<double> BpSpec::getBreakPointMasses(
  ...
  if (ion_type_ptr->isNTerm()) {
    for (size_t i = 0; i < break_point_ptr_vec_.size(); i++) {
      bp_mass_vec.push_back(
          break_point_ptr_vec_[i]->getNTermMass(ion_type_ptr));
    }
  } else {
    for (size_t i = 0; i < break_point_ptr_vec_.size(); i++) {
      bp_mass_vec.push_back(
          break_point_ptr_vec_[i]->getCTermMass(ion_type_ptr));
    }
  }
...
std::vector<int> BpSpec::getScaledMass(double scale,
                                       const IonTypePtr& ion_type_ptr) const {
  std::vector<int> result;
  if (ion_type_ptr->isNTerm()) {
    for (size_t i = 0; i < break_point_ptr_vec_.size(); i++) {
      double value =
          break_point_ptr_vec_[i]->getNTermMass(ion_type_ptr) * scale;
      result.push_back(std::floor(value + 0.5));
    }
  } else {
    for (size_t i = 0; i < break_point_ptr_vec_.size(); i++) {
      double value =
          break_point_ptr_vec_[i]->getCTermMass(ion_type_ptr) * scale;
      result.push_back(std::floor(value + 0.5));
    }
  }
  return result;
