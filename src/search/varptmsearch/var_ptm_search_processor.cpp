...
ProtModPtr findMatchedProtModPtr(const ProteoformPtr& db_proteo_ptr,
...
int findMatchedPos(const ProteoformPtr& proteo_ptr, double n_term_trunc) {
...
std::function<void()> geneTask(const SpectrumSetPtr& spec_set_ptr,
                               ...
                               const PrsmXmlWriterPtrVec& writer_ptr_vec) {
  return [spec_set_ptr, simple_prsm_ptr_vec, reader_ptr, mng_ptr, type_ptr,
          pool_ptr, writer_ptr_vec]() {
    ...
    for (size_t i = 0; i < simple_prsm_ptr_vec.size(); i++) {
      if (std::abs(spec_set_ptr->getPrecMonoMass() -
                   simple_prsm_ptr_vec[i]->getPrecMass()) > std::pow(10, -4)) {
        ...
        // A small error is allowed for errors introduced in writing real
        // numbers to xml files.
        continue;
      }
      ...
      LOG_DEBUG("type " << type_ptr->getName() << " proteoform mass "
                        << db_proteo_ptr->getMass() << " n_term_shift "
                        << n_term_shift << " c term shift " << c_term_shift);
      ...
      if (type_ptr == ProteoformType::COMPLETE ||
          type_ptr == ProteoformType::PREFIX) {
        if (n_term_shift != 0) {
          // Get the proteoform with N-terminal modification
          ProtModPtr prot_mod_ptr = findMatchedProtModPtr(
              db_proteo_ptr, prot_mod_ptr_vec, n_term_shift);
          LOG_DEBUG("Db seq: " << db_proteo_ptr->getProteoformMatchSeq());
          proteo_with_prot_mod_ptr = proteoform_factory::geneProtModProteoform(
              db_proteo_ptr, prot_mod_ptr);
      ...
      if (type_ptr == ProteoformType::PREFIX ||
          type_ptr == ProteoformType::INTERNAL) {
      ...
      ProteoformPtr sub_proteo_ptr = proteoform_factory::geneSubProteoform(
          proteo_with_prot_mod_ptr, fasta_seq_ptr, local_start_pos,
          local_end_pos);
      LOG_DEBUG("sub seq: " << sub_proteo_ptr->getProteoformMatchSeq());
      LOG_DEBUG("protein mass " << sub_proteo_ptr->getMass()
                                << "proteoform mass diff "
                                << (simple_prsm_ptr->getPrecMass() -
                                    sub_proteo_ptr->getMass()));
    ...
    PrsmPtrVec selected_prsms =
        prsm_util::getTopPrsms(prsms, mng_ptr->n_report_);
...
void VarPtmSearchProcessor::process() {
  ...
  for (int i = 0; i < mng_ptr_->thread_num_; i++) {
    ...
    PrsmXmlWriterPtr comp_writer_ptr = std::make_shared<PrsmXmlWriter>(
        output_file_name + "_" + ProteoformType::COMPLETE->getName() + "_" +
        idx);
    comp_writer_ptr_vec.push_back(comp_writer_ptr);
    PrsmXmlWriterPtr pref_writer_ptr = std::make_shared<PrsmXmlWriter>(
        output_file_name + "_" + ProteoformType::PREFIX->getName() + "_" + idx);
    pref_writer_ptr_vec.push_back(pref_writer_ptr);

    PrsmXmlWriterPtr suff_writer_ptr = std::make_shared<PrsmXmlWriter>(
        output_file_name + "_" + ProteoformType::SUFFIX->getName() + "_" + idx);
    suff_writer_ptr_vec.push_back(suff_writer_ptr);

    PrsmXmlWriterPtr internal_writer_ptr = std::make_shared<PrsmXmlWriter>(
        output_file_name + "_" + ProteoformType::INTERNAL->getName() + "_" +
        idx);
    internal_writer_ptr_vec.push_back(internal_writer_ptr);
  }

  SimpleThreadPoolPtr pool_ptr =
      std::make_shared<SimpleThreadPool>(mng_ptr_->thread_num_);
  ...
  std::vector<double> prec_error_vec =
      sp_para_ptr->getVarPtmSearchPrecErrorVec();
  // LOG_ERROR("start ");
  while (deconv_ms_ptr_vec.size() > 0) {
    if (deconv_ms_ptr_vec[0]->getMsHeaderPtr()->containsPrec()) {
      std::vector<SpectrumSetPtr> spec_set_vec =
          spectrum_set_factory::geneSpectrumSetPtrVecWithPrecError(
              deconv_ms_ptr_vec, sp_para_ptr, prec_error_vec);
      ...
      if (spec_set_vec[0]->isValid()) {
        ...
        while (comp_prsm_ptr != nullptr &&
               comp_prsm_ptr->getSpectrumId() == spec_id) {
        ...
        if (comp_selected_prsm_ptrs.size() > 0) {
          ...
          for (size_t k = 0; k < spec_set_vec.size(); k++) {
            ...
            pool_ptr->enqueue(geneTask(
                spec_set_vec[k], comp_selected_prsm_ptrs, reader_ptr, mng_ptr_,
                ProteoformType::COMPLETE, pool_ptr, comp_writer_ptr_vec));
          }
        }

        // prefix
        SimplePrsmPtrVec pref_selected_prsm_ptrs;
        while (pref_prsm_ptr != nullptr &&
               pref_prsm_ptr->getSpectrumId() == spec_id) {
        ...
        if (pref_selected_prsm_ptrs.size() > 0) {
          ...
          for (size_t k = 0; k < spec_set_vec.size(); k++) {
            ...
            pool_ptr->enqueue(geneTask(
                spec_set_vec[k], pref_selected_prsm_ptrs, reader_ptr, mng_ptr_,
                ProteoformType::PREFIX, pool_ptr, pref_writer_ptr_vec));
          }
        }

        // suffix
        SimplePrsmPtrVec suff_selected_prsm_ptrs;
        while (suff_prsm_ptr != nullptr &&
               suff_prsm_ptr->getSpectrumId() == spec_id) {
        ...
        if (suff_selected_prsm_ptrs.size() > 0) {
          ...
          for (size_t k = 0; k < spec_set_vec.size(); k++) {
            ...
            pool_ptr->enqueue(geneTask(
                spec_set_vec[k], suff_selected_prsm_ptrs, reader_ptr, mng_ptr_,
                ProteoformType::SUFFIX, pool_ptr, suff_writer_ptr_vec));
          }
        }
        ...
        if (internal_selected_prsm_ptrs.size() > 0) {
          ...
          for (size_t k = 0; k < spec_set_vec.size(); k++) {
            ...
            pool_ptr->enqueue(geneTask(spec_set_vec[k],
                                       internal_selected_prsm_ptrs, reader_ptr,
                                       mng_ptr_, ProteoformType::INTERNAL,
                                       pool_ptr, internal_writer_ptr_vec));
          }
        }
      }
    }
  ...
  if (prsm_para_ptr->getGroupSpecNum() > remainder && remainder > 0) {
    // if there are spectrum remaining because they were not combined due to not
    // having enough pairs fix the message as the processing is completed. this
    // code avoids error when no combined spectra is used but a scan is
    // remaining unprocessed because then it will not satisfy the first
    // condition
    std::cout << std::flush << "Variable PTM search - processing "
              << spectrum_num << " of " << spectrum_num << " spectra.\r";
  }
  ...
  PrsmStrMergePtr merge_ptr = std::make_shared<PrsmStrMerge>(
      sp_file_name,
      mng_ptr_->output_file_ext_ + "_" + ProteoformType::COMPLETE->getName(),
  ...
  merge_ptr = std::make_shared<PrsmStrMerge>(
      sp_file_name,
      mng_ptr_->output_file_ext_ + "_" + ProteoformType::PREFIX->getName(),
      mng_ptr_->thread_num_,
      mng_ptr_->output_file_ext_ + "_" + ProteoformType::PREFIX->getName(),
      mng_ptr_->n_report_, norm, remove_dup);
  merge_ptr->process();
  ...
  merge_ptr = std::make_shared<PrsmStrMerge>(
      sp_file_name,
      mng_ptr_->output_file_ext_ + "_" + ProteoformType::SUFFIX->getName(),
      mng_ptr_->thread_num_,
      mng_ptr_->output_file_ext_ + "_" + ProteoformType::SUFFIX->getName(),
      mng_ptr_->n_report_, norm, remove_dup);
  merge_ptr->process();
  ...
  merge_ptr = std::make_shared<PrsmStrMerge>(
      sp_file_name,
      mng_ptr_->output_file_ext_ + "_" + ProteoformType::INTERNAL->getName(),
      mng_ptr_->thread_num_,
      mng_ptr_->output_file_ext_ + "_" + ProteoformType::INTERNAL->getName(),
      mng_ptr_->n_report_, norm, remove_dup);
  merge_ptr->process();
