...
VarPtmSlowMatch::VarPtmSlowMatch(const ProteoformPtr& proteo_ptr,
  ...
  // LOG_ERROR("prec mass " << prec_mono_mass_ << " protein mass " <<
  // proteo_ptr->getMonoNeutralMass());
  mng_ptr_ = mng_ptr;
  PeakTolerancePtr tole_ptr =
      mng_ptr_->prsm_para_ptr_->getSpParaPtr()->getPeakTolerancePtr();
  prec_error_tole_ =
      deconv_ms_ptr_vec_[0]->getMsHeaderPtr()->getFirstPrecErrorTolerance(
          tole_ptr->getPpo());
...
inline DiagHeaderPtrVec VarPtmSlowMatch::geneVarPtmNTermShiftHeaders() {
  ...
  for (size_t i = 0; i < shifts.size(); i++) {
    ...
    DiagHeaderPtr header_ptr = std::make_shared<DiagHeader>(
        n_shift, true, false, false, false, true, false);
    ...
    LOG_DEBUG(i << " c shift " << c_shift << " error tolerance "
                << prec_error_tole_);
...
void VarPtmSlowMatch::init() {
  ...
  DiagonalPtrVec diagonal_ptrs = diag_pair_util::geneDiagonalsWithEmptyList(
  ...
  var_ptm_align_ptr_ = std::make_shared<VarPtmAlign>(
      ms_masses, seq_masses, sub_res_seq_ptr, diagonal_ptrs, mng_ptr_);
  success_init_ = true;
}
