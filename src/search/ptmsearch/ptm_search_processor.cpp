...
void seleTopPrsms(const PrsmPtrVec& all_prsm_ptrs, PrsmPtrVec& sele_prsm_ptrs,
                  int n_report) {
...
std::function<void()> geneTask(const SpectrumSetPtr& spectrum_set_ptr,
                               const SimplePrsmPtrVec& ori_simple_prsm_ptrs,
                               const PtmSearchMngPtr& mng_ptr,
                               const SimpleThreadPoolPtr& pool_ptr,
                               const PrsmXmlWriterSetPtrVec& writer_ptr_vec) {
  return [spectrum_set_ptr, ori_simple_prsm_ptrs, mng_ptr, pool_ptr,
          writer_ptr_vec]() {
    ...
    for (size_t i = 0; i < ori_simple_prsm_ptrs.size(); i++) {
      ...
      if (std::abs(spectrum_set_ptr->getPrecMonoMass() -
                   ori_simple_prsm_ptrs[i]->getPrecMass()) < std::pow(10, -4)) {
    ...
    PtmSearchSlowFilterPtr slow_filter_ptr =
        std::make_shared<PtmSearchSlowFilter>(spectrum_set_ptr,
                                              simple_prsm_ptrs, mng_ptr);
    ...
    for (int s = 2; s <= mng_ptr->align_para_ptr_->n_unknown_shift_; s++) {
      PrsmPtrVec complete_prsm_ptrs = slow_filter_ptr->getPrsms(
          s - 2, mng_ptr->prsm_para_ptr_, ProteoformType::COMPLETE);
      ...
      seleTopPrsms(complete_prsm_ptrs, sele_complete_prsm_ptrs,
                   mng_ptr->n_report_);
      ...
      PrsmPtrVec prefix_prsm_ptrs = slow_filter_ptr->getPrsms(
          s - 2, mng_ptr->prsm_para_ptr_, ProteoformType::PREFIX);
      ...
      PrsmPtrVec suffix_prsm_ptrs = slow_filter_ptr->getPrsms(
          s - 2, mng_ptr->prsm_para_ptr_, ProteoformType::SUFFIX);
      ...
      PrsmPtrVec internal_prsm_ptrs = slow_filter_ptr->getPrsms(
          s - 2, mng_ptr->prsm_para_ptr_, ProteoformType::INTERNAL);
      std::sort(internal_prsm_ptrs.begin(), internal_prsm_ptrs.end(),
                Prsm::cmpMatchFragDecMatchPeakDecProtInc);
      PrsmPtrVec sele_internal_prsm_ptrs;
      seleTopPrsms(internal_prsm_ptrs, sele_internal_prsm_ptrs,
                   mng_ptr->n_report_);
...
PtmSearchProcessor::PtmSearchProcessor(const PtmSearchMngPtr& mng_ptr) {
  mng_ptr_ = mng_ptr;
}
...
void PtmSearchProcessor::process() {
  ...
  for (int i = 0; i < mng_ptr_->thread_num_; i++) {
    std::string writer_file_name = output_file_name + "_" + std::to_string(i);
    PrsmXmlWriterSetPtr writer_set_ptr =
        std::make_shared<PrsmXmlWriterSet>(writer_file_name, n_unknown_shift);
  ...
  std::vector<double> prec_error_vec =
      sp_para_ptr->getMultiShiftSearchPrecErrorVec();
  while (deconv_ms_ptr_vec.size() > 0) {
    ...
    cnt += group_spec_num;
    std::cout << std::flush << "Multiple unexpected shifts search - processing "
  ...
  for (int s = 2; s <= n_unknown_shift; s++) {
    ...
    PrsmStrMergePtr merge_ptr = std::make_shared<PrsmStrMerge>(
        sp_file_name, complete_input_exts, complete_output_ext, prsm_top_num,
        norm, remove_dup);
    merge_ptr->process();
    merge_ptr = nullptr;
    ...
    merge_ptr = std::make_shared<PrsmStrMerge>(sp_file_name, prefix_input_exts,
    ...
    merge_ptr = std::make_shared<PrsmStrMerge>(sp_file_name, suffix_input_exts,
    ...
    merge_ptr = std::make_shared<PrsmStrMerge>(
        sp_file_name, internal_input_exts, internal_output_ext, prsm_top_num,
        norm, remove_dup);
    merge_ptr->process();
    merge_ptr = nullptr;
