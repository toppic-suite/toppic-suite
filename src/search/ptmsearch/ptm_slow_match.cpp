...
DiagHeaderPtrVec PtmSlowMatch::getNTermShiftListCommonHeaders() {
  ...
  for (size_t i = 0; i < best_shifts.size(); i++) {
    ...
    DiagHeaderPtr header_ptr
        = std::make_shared<DiagHeader>(best_shifts[i],
...
void PtmSlowMatch::compute(const ProteoformTypePtr &type_ptr, PrsmPtrVec &prsm_ptrs) {
...
PrsmPtr PtmSlowMatch::compute(const ProteoformTypePtr &align_type_ptr, int shift_num) {
