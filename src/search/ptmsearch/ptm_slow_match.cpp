...
PtmSlowMatch::PtmSlowMatch(const ProteoformPtr& proteo_ptr,
...
DiagHeaderPtrVec PtmSlowMatch::getNTermShiftListCommonHeaders() {
  ...
  std::vector<std::pair<int, int>> sp_masses_toles =
      prm_ms_util::getIntMassErrorList(ms_six_ptr_vec_, tole_ptr, scale, true,
                                       false);
  ...
  std::vector<double> best_shifts = comp_shift_ptr->findBestShift(
      sp_masses_toles, proteo_ptr_->getBpSpecPtr()->getScaledPrmMasses(scale),
      mng_ptr_->n_top_diagonals_, mng_ptr_->min_diagonal_gap_, scale);
  ...
  for (size_t i = 0; i < best_shifts.size(); i++) {
    ...
    DiagHeaderPtr header_ptr = std::make_shared<DiagHeader>(
        best_shifts[i], true, false, false, false, false, false);
    header_ptrs.push_back(header_ptr);
  }
  return header_ptrs;
}

void PtmSlowMatch::addPrefixDiagonals(DiagHeaderPtrVec& common_header_ptrs,
  ...
  for (size_t i = 0; i < common_header_ptrs.size(); i++) {
    ...
    int best_n_pos =
        diag_header_util::findSimilarShiftPos(n_term_match_shifts, s);
    if (best_n_pos >= 0) {
      double new_shift = n_term_match_shifts[best_n_pos];
      if (!diag_header_util::isExistHeader(n_extend_header_ptrs, new_shift)) {
        // n_term strict; c_term nostrict; prot n_term no_match; prot c_term
        // no_match pep n_term match; pep c_term no_match
        DiagHeaderPtr header_ptr = std::make_shared<DiagHeader>(
            new_shift, true, false, false, false, true, false);
...
void PtmSlowMatch::addSuffixDiagonals(DiagHeaderPtrVec& common_header_ptrs,
                                      DiagHeaderPtrVec& c_extend_header_ptrs) {
  double prec_mass_minus_water =
      prec_mono_mass_ - mass_constant::getWaterMass();
  std::vector<double> prm_masses = proteo_ptr_->getBpSpecPtr()->getPrmMasses();
  ...
  for (size_t i = 0; i < common_header_ptrs.size(); i++) {
    double s = common_header_ptrs[i]->getProtNTermShift();
    int best_c_pos =
        diag_header_util::findSimilarShiftPos(c_term_match_shifts, s);
    if (best_c_pos >= 0) {
      double new_shift = c_term_match_shifts[best_c_pos];
      if (!diag_header_util::isExistHeader(c_extend_header_ptrs, new_shift)) {
        // n term nostrict, c_term strict, prot n_term no match ; prot c_term no
        // match pep n_term no match, pep c_term match
        DiagHeaderPtr header_ptr = std::make_shared<DiagHeader>(
            new_shift, false, true, false, false, false, true);
...
DiagHeaderPtrVec PtmSlowMatch::geneNTermShiftHeaders() {
  DiagHeaderPtrVec n_extend_header_ptrs;
  DiagHeaderPtrVec c_extend_header_ptrs;

  // add corner diagonals for all types of alignments
  double seq_mass = proteo_ptr_->getResSeqPtr()->getSeqMass();
  diag_header_util::addCornerDiagonals(
      n_extend_header_ptrs, c_extend_header_ptrs, seq_mass, prec_mono_mass_);

  DiagHeaderPtrVec header_ptrs;
  // if not complete alignment, find best shifts
  if (align_type_ptr_ != ProteoformType::COMPLETE) {
    DiagHeaderPtrVec common_header_ptrs = getNTermShiftListCommonHeaders();

    if (align_type_ptr_ == ProteoformType::SUFFIX ||
        align_type_ptr_ == ProteoformType::INTERNAL) {
      // add prefix masses
      addPrefixDiagonals(common_header_ptrs, n_extend_header_ptrs);
    }
    if (align_type_ptr_ == ProteoformType::PREFIX ||
        align_type_ptr_ == ProteoformType::INTERNAL) {
    ...
    if (mng_ptr_->align_para_ptr_->getUnknownShiftNum() >= 2) {
      header_ptrs.insert(header_ptrs.end(), common_header_ptrs.begin(),
                         common_header_ptrs.end());
    }
  }
  header_ptrs.insert(header_ptrs.end(), n_extend_header_ptrs.begin(),
                     n_extend_header_ptrs.end());
  header_ptrs.insert(header_ptrs.end(), c_extend_header_ptrs.begin(),
                     c_extend_header_ptrs.end());
  for (size_t i = 0; i < header_ptrs.size(); i++) {
    double n_shift = header_ptrs[i]->getProtNTermShift();
    double c_shift =
        prec_mono_mass_ - proteo_ptr_->getResSeqPtr()->getSeqMass() - n_shift;
...
void PtmSlowMatch::init() {
  DiagHeaderPtrVec n_term_shift_header_ptrs = geneNTermShiftHeaders();
  PeakTolerancePtr tole_ptr =
      mng_ptr_->prsm_para_ptr_->getSpParaPtr()->getPeakTolerancePtr();
  PrmPeakPtrVec prm_peaks =
      prm_ms_util::getPrmPeakPtrs(ms_six_ptr_vec_, tole_ptr);
  int group_spec_num = ms_six_ptr_vec_.size();
  DiagonalPtrVec diagonal_ptrs = diag_pair_util::geneDiagonalsWithoutEmptyList(
      n_term_shift_header_ptrs, prm_peaks, group_spec_num, proteo_ptr_);
  ...
  ps_align_ptr_ = std::make_shared<PsAlign>(
      ms_masses, seq_masses, diagonal_ptrs, mng_ptr_->align_para_ptr_);
}

void PtmSlowMatch::compute(const ProteoformTypePtr& type_ptr,
                           PrsmPtrVec& prsm_ptrs) {
  ps_align_ptr_->compute(type_ptr);
  for (int s = 2; s <= mng_ptr_->align_para_ptr_->getUnknownShiftNum(); s++) {
    PrsmPtr prsm_ptr =
        ps_align_ptr_->geneResult(s, proteo_ptr_, deconv_ms_ptr_vec_,
                                  ms_three_ptr_vec_, mng_ptr_->prsm_para_ptr_);
    prsm_ptrs.push_back(prsm_ptr);
  }
}

PrsmPtr PtmSlowMatch::compute(const ProteoformTypePtr& align_type_ptr,
                              int shift_num) {
