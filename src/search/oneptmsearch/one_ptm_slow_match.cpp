...
OnePtmSlowMatch::OnePtmSlowMatch(const ProteoformPtr& proteo_ptr,
...
inline void OnePtmSlowMatch::addPrefixDiagonals(
...
inline void OnePtmSlowMatch::addComplementDiagonals(
    DiagHeaderPtrVec& n_extend_header_ptrs,
...
inline void OnePtmSlowMatch::addSuffixDiagonals(
    DiagHeaderPtrVec& c_extend_header_ptrs) {
  std::vector<double> shifts =
      simple_prsm_ptr_->getNTermShiftsFromCTermShifts();
  for (size_t i = 0; i < shifts.size(); i++) {
    double new_shift = shifts[i] - proteo_ptr_->getProtModPtr()->getProtShift();
    if (!diag_header_util::isExistHeader(c_extend_header_ptrs, new_shift)) {
      // n term nostrict, c_term strict, prot n_term no match ; prot c_term no
