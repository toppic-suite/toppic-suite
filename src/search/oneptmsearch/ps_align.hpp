...
#ifndef TOPPIC_SEARCH_ONEPTMSEARCH_PS_ALIGN_HPP_
#define TOPPIC_SEARCH_ONEPTMSEARCH_PS_ALIGN_HPP_
...
class PsAlign {
 public:
  ...
  void compute(const ProteoformTypePtr &type_ptr);

  void initDpPair();

  void dp(const ProteoformTypePtr &type_ptr);
  ...
  PrsmPtr geneResult(int shift_num, const ProteoformPtr &proteo_ptr, DeconvMsPtrVec &deconv_ms_ptr_vec,
                     ExtendMsPtrVec &ms_three_ptr_vec, const PrsmParaPtr &prsm_para_ptr);
 ...
 protected:
  ...
  DpPairPtr getTruncPre(const DpPairPtr &cur_pair_ptr, int s, const ProteoformTypePtr &type_ptr);
