...
#ifndef TOPPIC_SEARCH_ONEPTMSEARCH_DP_PAIR_HPP_
#define TOPPIC_SEARCH_ONEPTMSEARCH_DP_PAIR_HPP_
...
class DpPair : public Pair{
 public:
  DpPair(int x,int y,double pair_score,double diff,
         int order,int n_shift, const DiagHeaderPtr &header_ptr);

  DpPairPtr getDiagPrevPairPtr() {return diag_prev_pair_ptr_;}

  void setDiagPrevPairPtr(const DpPairPtr &diag_prev_pair_ptr) {
  ...
  void updateTable(int s,double score,int path_type,const DpPairPtr &prev_pair);
