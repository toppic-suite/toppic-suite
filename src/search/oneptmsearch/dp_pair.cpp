...
DpPair::DpPair(int x, int y, double pair_score, double diff, int order,
               int n_shift, const DiagHeaderPtr& header_ptr)
    : Pair(x, y) {
  diff_ = diff;
...
void DpPair::updateTable(int s, double score, int path_type,
                         const DpPairPtr& prev_pair_ptr) {
  scores_[s] = score;
  types_[s] = path_type;
