...
#ifndef TOPPIC_SEARCH_GRAPHALIGN_GRAPH_ALIGN_MNG_HPP_
#define TOPPIC_SEARCH_GRAPHALIGN_GRAPH_ALIGN_MNG_HPP_
...
class GraphAlignMng {
 public:
  GraphAlignMng(const PrsmParaPtr& prsm_para_ptr,
                const std::string& var_mod_file_name, int n_unknown_shift,
                int max_known_mods, int proteo_graph_gap, int var_ptm_in_gap,
                double max_ptm_mass, int thread_num, bool whole_protein_only,
  ...
  int getIntMaxPtmSumMass() {
    return std::ceil(max_ptm_sum_mass_ * convert_ratio_);
  }

  int getIntMinConsistentDist() {
    return std::ceil(min_consistent_dist_ * convert_ratio_);
  }
