...
PrsmPtr getPrsmPtr(const DeconvMsPtrVec& deconv_ms_ptr_vec,
                   const ZpFastMatchPtr& fast_match_ptr,
                   const SpParaPtr& sp_para_ptr) {
  ProteoformPtr proteoform_ptr = proteoform_factory::geneSubProteoform(
      fast_match_ptr->getProteoformPtr(),
      fast_match_ptr->getProteoformPtr()->getFastaSeqPtr(),
      fast_match_ptr->getBegin(), fast_match_ptr->getEnd());
