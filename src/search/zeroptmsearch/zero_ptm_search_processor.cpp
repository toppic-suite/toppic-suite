...
PrsmPtrVec ZeroPtmSearchProcessor::zeroPtmSearchOneSpec(
    const SpectrumSetPtr& spec_set_ptr,
    const SimplePrsmPtrVec& simple_prsm_ptr_vec,
    const FastaIndexReaderPtr& reader_ptr, const ZeroPtmSearchMngPtr& mng_ptr,
  ...
  for (size_t i = 0; i < simple_prsm_ptr_vec.size(); i++) {
    ...
    double shift =
        spec_set_ptr->getPrecMonoMass() - simple_prsm_ptr_vec[i]->getPrecMass();
  ...
  for (size_t i = 0; i < fast_matches.size(); i++) {
    PrsmPtr prsm_ptr =
        zero_ptm_util::getPrsmPtr(deconv_ms_vec, fast_matches[i], sp_para_ptr);
...
void ZeroPtmSearchProcessor::process() {
  ...
  std::vector<double> prec_error_vec =
      sp_para_ptr->getZeroShiftSearchPrecErrorVec();
  while (deconv_ms_ptr_vec.size() > 0) {
      ...
      if (spec_set_vec[0]->isValid()) {
        ...
        if (comp_selected_prsm_ptrs.size() > 0) {
          for (size_t k = 0; k < spec_set_vec.size(); k++) {
            PrsmPtrVec prsms = zeroPtmSearchOneSpec(
        ...
        if (internal_selected_prsm_ptrs.size() > 0) {
          for (size_t k = 0; k < spec_set_vec.size(); k++) {
            PrsmPtrVec prsms = zeroPtmSearchOneSpec(
                spec_set_vec[k], internal_selected_prsm_ptrs, reader_ptr,
    ...
    cnt += group_spec_num;
    std::cout << std::flush << "Zero unexpected shift search - processing "
