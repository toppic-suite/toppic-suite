...
DiagHeader::DiagHeader(double n_term_shift, bool n_strict, bool c_strict,
...
void DiagHeader::initHeader(double c_shift, const ProteoformPtr& proteo_ptr) {
  // set protein c term shift
  prot_C_term_shift_ = c_shift;
  std::vector<double> prm_masses = proteo_ptr->getBpSpecPtr()->getPrmMasses();

  trunc_first_res_pos_ =
      prsm_algo::getFirstResPos(prot_N_term_shift_, prm_masses);

  trunc_last_res_pos_ =
      prsm_algo::getLastResPos(prot_C_term_shift_, prm_masses);
