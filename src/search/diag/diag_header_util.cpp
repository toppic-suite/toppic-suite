...
DiagHeaderPtr getTopLeftCornerHeader() {
  ...
  return std::make_shared<DiagHeader>(shift, true, false, true, false, false,
                                      false);
}
...
DiagHeaderPtr getBottomRightCornerHeader(double seq_mass, double prec_mass) {
  ...
  return std::make_shared<DiagHeader>(shift, false, true, false, true, false,
                                      false);
}

void addCornerDiagonals(DiagHeaderPtrVec& n_extend_header_ptrs,
                        DiagHeaderPtrVec& c_extend_header_ptrs, double seq_mass,
                        double prec_mass) {
  // get top-left corner header in spectral grid (shift is 0)
  DiagHeaderPtr top_left_corner_header_ptr =
      diag_header_util::getTopLeftCornerHeader();
...
MassShiftPtrVec getDiagonalMassChanges(const DiagHeaderPtrVec& header_ptrs,
  ...
  if (!header_ptrs[0]->isPepNTermMatch() &&
      !header_ptrs[0]->isProtNTermMatch()) {
    AlterPtr alter_ptr = std::make_shared<Alter>(
        0, header_ptrs[0]->getMatchFirstBpPos() - first_res_pos, type_ptr,
  ...
  for (size_t i = 0; i < header_ptrs.size() - 1; i++) {
    AlterPtr alter_ptr = std::make_shared<Alter>(
        ...
        header_ptrs[i + 1]->getMatchFirstBpPos() - first_res_pos, type_ptr,
  ...
  if (!last_header_ptr->isPepCTermMatch() &&
      !last_header_ptr->isProtCTermMatch()) {
    AlterPtr alter_ptr = std::make_shared<Alter>(
        last_header_ptr->getMatchLastBpPos() - first_res_pos,
        (last_res_pos + 1) - first_res_pos, type_ptr,
        last_header_ptr->getPepCTermShift(), none_ptr);
    MassShiftPtr shift = std::make_shared<MassShift>(alter_ptr);
    shift_list.push_back(shift);
  }
  return shift_list;
...
MassShiftPtrVec getDiagonalMassChanges(const DiagHeaderPtrVec& header_ptrs,
  ...
  if (!header_ptrs[0]->isPepNTermMatch() &&
      !header_ptrs[0]->isProtNTermMatch()) {
    AlterPtr alter_ptr = std::make_shared<Alter>(
        0, header_ptrs[0]->getMatchFirstBpPos() - first_res_pos, types[0],
        header_ptrs[0]->getPepNTermShift(), none_ptr);
  ...
  for (size_t i = 0; i < header_ptrs.size() - 1; i++) {
    AlterPtr alter_ptr = std::make_shared<Alter>(
        header_ptrs[i]->getMatchLastBpPos() - first_res_pos,
        header_ptrs[i + 1]->getMatchFirstBpPos() - first_res_pos, types[i + 1],
        header_ptrs[i + 1]->getProtNTermShift() -
            header_ptrs[i]->getProtNTermShift(),
        none_ptr);
    MassShiftPtr shift = std::make_shared<MassShift>(alter_ptr);
  ...
  if (!last_header_ptr->isPepCTermMatch() &&
    ...
    AlterPtr alter_ptr = std::make_shared<Alter>(
        last_header_ptr->getMatchLastBpPos() - first_res_pos,
        (last_res_pos + 1) - first_res_pos, AlterType::UNEXPECTED,
        last_header_ptr->getPepCTermShift(), none_ptr);
    MassShiftPtr shift = std::make_shared<MassShift>(alter_ptr);
    shift_list.push_back(shift);
  }
