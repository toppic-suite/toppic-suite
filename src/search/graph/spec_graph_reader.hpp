...
class SpecGraphReader {
 public:
  SpecGraphReader(const std::string& sp_file_name, int group_sp_num,
                  double convert_ratio, const SpParaPtr& sp_para_ptr);

  SpecGraphPtrVec getNextSpecGraphPtrVec(int error);

  SpecGraphPtrVec getNextSpecGraphPtrVec(const SpectrumSetPtr& spec_set_ptr,
                                         int error);
