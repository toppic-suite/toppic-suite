...
class SpecGraph {
 public:
  explicit SpecGraph(const SpectrumSetPtr &spec_set_ptr): spec_set_ptr_(spec_set_ptr) {}

  SpecGraph(const SpectrumSetPtr &spec_set_ptr, const PrmPeakPtrVec &peak_vec,
            const MassGraphPtr &mass_graph_ptr, double convert_ratio);
