...
void TopDIADialog::on_defaultButton_clicked() {
  ...
  ui->ms1snRatioEdit->setText(
      QString::number(topfd_para_ptr_->getMsOneSnRatio()));
  ui->ms2snRatioEdit->setText(
      QString::number(topfd_para_ptr_->getMsTwoSnRatio()));
  ui->windowSizeEdit->setText(
      QString::number(topfd_para_ptr_->getPrecWindowWidth()));
  ...
  //////////////////////////////////////
  ui->ms1EcscoreCutoffEdit->setText(
      QString::number(topfd_para_ptr_->getMs1EcscoreCutoff()));
  ui->ms2EcscoreCutoffEdit->setText(
      QString::number(topfd_para_ptr_->getMs2EcscoreCutoff()));
  ui->ms1MinScanNumEdit->setText(
      QString::number(topfd_para_ptr_->getMs1MinScanNum()));
  ui->ms2MinScanNumEdit->setText(
      QString::number(topfd_para_ptr_->getMs2MinScanNum()));
  ui->pseudoScoreCutoffEdit->setText(
      QString::number(topdia_para_ptr_->getPseudoScoreCutoff()));
  ui->pseudoMinPeakNumEdit->setText(
      QString::number(topdia_para_ptr_->getPseudoMinPeaks()));
  ui->ms1IntePccCutoffEdit->setText(
      QString::number(topdia_para_ptr_->getMs1SeedEnvInteCorrToleCutoff()));
  ui->ms2IntePccCutoffEdit->setText(
      QString::number(topdia_para_ptr_->getMs2SeedEnvInteCorrToleCutoff()));
  //////////////////////////////////////
...
void TopDIADialog::on_startButton_clicked() {
  ...
  std::string cmd = toppic::command::geneTopdiaCommand(
      topfd_para_ptr_, topdia_para_ptr_, spec_file_lst_);
...
void TopDIADialog::on_exitButton_clicked() { close(); }
...
void TopDIADialog::getParaPtr() {
  ...
  topfd_para_ptr_->setMaxCharge(
      std::stoi(ui->maxChargeEdit->text().toStdString()));
  ...
  topfd_para_ptr_->setMs1EcscoreCutoff(
      std::stod(ui->ms1EcscoreCutoffEdit->text().toStdString()));
  topfd_para_ptr_->setMs2EcscoreCutoff(
      std::stod(ui->ms2EcscoreCutoffEdit->text().toStdString()));
  topfd_para_ptr_->setMs1MinScanNum(
      std::stoi(ui->ms1MinScanNumEdit->text().toStdString()));
  topfd_para_ptr_->setMs2MinScanNum(
      std::stoi(ui->ms2MinScanNumEdit->text().toStdString()));
  topdia_para_ptr_->setPseudoScoreCutoff(
      std::stod(ui->pseudoScoreCutoffEdit->text().toStdString()));
  topdia_para_ptr_->setPseudoMinPeaks(
      std::stoi(ui->pseudoMinPeakNumEdit->text().toStdString()));
  topdia_para_ptr_->setMs1SeedEnvInteCorrToleCutoff(
      std::stod(ui->ms1IntePccCutoffEdit->text().toStdString()));
  topdia_para_ptr_->setMs2SeedEnvInteCorrToleCutoff(
      std::stod(ui->ms2IntePccCutoffEdit->text().toStdString()));
  //////////////////////////////////////

  topfd_para_ptr_->setUseSingleScanNoiseLevel(
...
bool TopDIADialog::checkError() {
  ...
  if (ui->ms1EcscoreCutoffEdit->text().isEmpty()) {
    QMessageBox::warning(this, tr("Warning"),
                         tr("MS1 ECScore cutoff is empty!"), QMessageBox::Yes);
    return true;
  }

  if (ui->ms2EcscoreCutoffEdit->text().isEmpty()) {
    QMessageBox::warning(this, tr("Warning"),
                         tr("MS2 ECScore cutoff is empty!"), QMessageBox::Yes);
    return true;
  }

  if (ui->pseudoScoreCutoffEdit->text().isEmpty()) {
    QMessageBox::warning(this, tr("Warning"),
                         tr("Pseudo Score cutoff is empty!"), QMessageBox::Yes);
    return true;
  }
  ...
  if (ui->threadNumberEdit->text().toInt() >
      toppic::mem_check::getMaxThreads("topdia")) {
