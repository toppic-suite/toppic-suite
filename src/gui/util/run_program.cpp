...
std::string RunProgram::geneCommand(std::map<std::string, std::string> arguments_, std::string app_name) {
  ...
  for (std::map<std::string, std::string>::iterator it = arguments_.begin(); it != arguments_.end(); ++it) {
    if (common_para.find(it->first) != common_para.end()) { //if one of the common parameters
