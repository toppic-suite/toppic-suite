...
void addTwoMasses(PrmPeakPtrVec& list, int spec_id,
                  const DeconvPeakPtr& deconv_peak_ptr, double prec_mono_mass,
                  double n_term_label_mass,
                  const ActivationPtr& active_type_ptr,
                  const PeakTolerancePtr& tole_ptr) {
  double peak_mass = deconv_peak_ptr->getMonoMass();
  double n_term_mass =
      peak_mass - active_type_ptr->getN_BYShift() - n_term_label_mass;
  double default_score = 1.0;
  PrmPeakPtr new_peak_ptr = std::make_shared<PrmPeak>(
      spec_id, deconv_peak_ptr, BasePeakType::ORIGINAL, n_term_mass,
      default_score);
  new_peak_ptr->setStrictTolerance(tole_ptr->compStrictErrorTole(peak_mass));
  new_peak_ptr->setNStrictCRelaxTolerance(
      tole_ptr->compStrictErrorTole(peak_mass));
  new_peak_ptr->setNRelaxCStrictTolerance(
      tole_ptr->compRelaxErrorTole(peak_mass, prec_mono_mass));
  list.push_back(new_peak_ptr);
  double reverse_mass = prec_mono_mass -
                        (peak_mass - active_type_ptr->getC_BYShift()) -
                        n_term_label_mass;
  PrmPeakPtr reverse_peak_ptr = std::make_shared<PrmPeak>(
      spec_id, deconv_peak_ptr, BasePeakType::REVERSED, reverse_mass,
      default_score);
  reverse_peak_ptr->setStrictTolerance(
      tole_ptr->compStrictErrorTole(peak_mass));
  reverse_peak_ptr->setNRelaxCStrictTolerance(
      tole_ptr->compStrictErrorTole(peak_mass));
  reverse_peak_ptr->setNStrictCRelaxTolerance(
      tole_ptr->compRelaxErrorTole(peak_mass, prec_mono_mass));
  list.push_back(reverse_peak_ptr);
}

void addSuffixTwoMasses(PrmPeakPtrVec& list, int spec_id,
                        const DeconvPeakPtr& deconv_peak_ptr,
                        double prec_mono_mass,
                        const ActivationPtr& active_type_ptr,
                        const PeakTolerancePtr& tole_ptr) {
  double default_score = 1.0;
  double peak_mass = deconv_peak_ptr->getMonoMass();
  double c_res_mass = peak_mass - active_type_ptr->getC_BYShift() -
                      mass_constant::getWaterMass();
  PrmPeakPtr new_peak_ptr = std::make_shared<PrmPeak>(
      spec_id, deconv_peak_ptr, BasePeakType::ORIGINAL, c_res_mass,
      default_score);
  new_peak_ptr->setStrictTolerance(tole_ptr->compStrictErrorTole(peak_mass));
  new_peak_ptr->setNStrictCRelaxTolerance(
      tole_ptr->compRelaxErrorTole(peak_mass, prec_mono_mass));
  ...
  list.push_back(new_peak_ptr);
  double reverse_mass = prec_mono_mass -
                        (peak_mass - active_type_ptr->getN_BYShift()) -
                        mass_constant::getWaterMass();
  PrmPeakPtr reverse_peak_ptr = std::make_shared<PrmPeak>(
      spec_id, deconv_peak_ptr, BasePeakType::REVERSED, reverse_mass,
      default_score);
...
void addSixMasses(PrmPeakPtrVec& list, int spec_id,
                  const DeconvPeakPtr& deconv_peak_ptr, double prec_mono_mass,
                  double n_term_label_mass,
                  const ActivationPtr& active_type_ptr,
                  const PeakTolerancePtr& tole_ptr,
  ...
  for (size_t i = 0; i < offsets.size(); i++) {
    double mass = peak_mass - active_type_ptr->getN_BYShift() -
                  n_term_label_mass + offsets[i];
    PrmPeakPtr peak_ptr = std::make_shared<PrmPeak>(
        spec_id, deconv_peak_ptr, BasePeakType::ORIGINAL, mass, default_score);
    peak_ptr->setStrictTolerance(tole_ptr->compStrictErrorTole(peak_mass));
    peak_ptr->setNStrictCRelaxTolerance(
        tole_ptr->compStrictErrorTole(peak_mass));
    peak_ptr->setNRelaxCStrictTolerance(
        tole_ptr->compRelaxErrorTole(peak_mass, prec_mono_mass));
    list.push_back(peak_ptr);
  }
  for (size_t i = 0; i < offsets.size(); i++) {
    double mass =
        prec_mono_mass - (peak_mass - active_type_ptr->getC_BYShift() -
                          n_term_label_mass + offsets[i]);
    PrmPeakPtr peak_ptr = std::make_shared<PrmPeak>(
        spec_id, deconv_peak_ptr, BasePeakType::REVERSED, mass, default_score);
    peak_ptr->setStrictTolerance(tole_ptr->compStrictErrorTole(peak_mass));
    peak_ptr->setNRelaxCStrictTolerance(
        tole_ptr->compStrictErrorTole(peak_mass));
    peak_ptr->setNStrictCRelaxTolerance(
...
void addSuffixSixMasses(PrmPeakPtrVec& list, int spec_id,
                        const DeconvPeakPtr& deconv_peak_ptr,
                        double prec_mono_mass,
                        const ActivationPtr& active_type_ptr,
                        const PeakTolerancePtr& tole_ptr,
  ...
  double c_res_mass = ori_mass - (active_type_ptr->getC_BYShift() +
                                  mass_constant::getWaterMass());
  for (size_t i = 0; i < offsets.size(); i++) {
    ...
    peak_ptr->setNStrictCRelaxTolerance(
        tole_ptr->compRelaxErrorTole(ori_mass, prec_mono_mass));
    peak_ptr->setNRelaxCStrictTolerance(
        tole_ptr->compStrictErrorTole(ori_mass));
    list.push_back(peak_ptr);
  }
  double reverse_mass = prec_mono_mass -
                        (ori_mass - active_type_ptr->getN_BYShift()) -
                        mass_constant::getWaterMass();
  for (size_t i = 0; i < offsets.size(); i++) {
...
void geneApproxSpecPeaks(PrmPeakPtrVec& peak_list, double prec_mono_mass,
  ...
  for (size_t i = 0; i < peak_list.size(); i++) {
    ...
    peak_list[i] = std::make_shared<PrmPeak>(
        peak_list[i]->getSpectrumId(), peak_list[i]->getBasePeakPtr(),
        peak_list[i]->getBaseTypePtr(), mass, peak_list[i]->getScore(),
...
PrmMsPtr geneMsTwoPtr(const DeconvMsPtr& deconv_ms_ptr, int spec_id,
  ...
  MsHeaderPtr header_ptr =
      MsHeader::geneMsHeaderPtr(ori_header_ptr, prec_mass_without_label);
  // get two prm peaks
  ...
  filterPeaks(list, list_filtered, prec_mass_without_label,
...
PrmMsPtr geneSuffixMsTwoPtr(const DeconvMsPtr& deconv_ms_ptr, int spec_id,
                            const SpParaPtr& sp_para_ptr, double prec_mono_mass,
                            double n_term_label_mass,
                            const std::vector<double>& mod_mass) {
  MsHeaderPtr ori_header_ptr = deconv_ms_ptr->getMsHeaderPtr();
  ...
  PrmPeakPtrVec list;
  for (size_t i = 0; i < deconv_ms_ptr->size(); i++) {
    addSuffixTwoMasses(list, spec_id, deconv_ms_ptr->getPeakPtr(i),
                       prec_mono_mass, active_type_ptr, tole_ptr);
  }
  // filter low mass peaks
  PrmPeakPtrVec list_filtered;
  filterPeaks(list, list_filtered, prec_mass_without_label,
...
PrmMsPtr geneMsSixPtr(const DeconvMsPtr& deconv_ms_ptr, int spec_id,
                      const SpParaPtr& sp_para_ptr, double prec_mono_mass,
                      double n_term_label_mass) {
  MsHeaderPtr ori_header_ptr = deconv_ms_ptr->getMsHeaderPtr();
  double prec_mass_without_label = prec_mono_mass - n_term_label_mass;
...
PrmMsPtr geneSuffixMsSixPtr(const DeconvMsPtr& deconv_ms_ptr, int spec_id,
                            const SpParaPtr& sp_para_ptr, double prec_mono_mass,
                            double n_term_label_mass) {
  MsHeaderPtr ori_header_ptr = deconv_ms_ptr->getMsHeaderPtr();
  double prec_mass_without_label = prec_mono_mass - n_term_label_mass;
  ...
  for (size_t i = 0; i < deconv_ms_ptr->size(); i++) {
    if (deconv_ms_ptr->getPeakPtr(i)->getMonoMass() <= extend_min_mass) {
      ...
      addSuffixSixMasses(list, spec_id, deconv_ms_ptr->getPeakPtr(i),
                         prec_mono_mass, active_type_ptr, tole_ptr,
                         sp_para_ptr->getExtendOffsets());
    }
  }
