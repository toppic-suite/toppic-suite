...
void addTwoMasses(PrmPeakPtrVec &list, int spec_id, const DeconvPeakPtr &deconv_peak_ptr,
                  double prec_mono_mass, double n_term_label_mass, 
                  const ActivationPtr &active_type_ptr, const PeakTolerancePtr &tole_ptr) {
  ...
  new_peak_ptr->setNStrictCRelaxTolerance(tole_ptr->compStrictErrorTole(peak_mass));
  ...
  reverse_peak_ptr->setNStrictCRelaxTolerance(tole_ptr->compRelaxErrorTole(peak_mass, prec_mono_mass));
  list.push_back(reverse_peak_ptr);
}

void addSuffixTwoMasses(PrmPeakPtrVec &list, int spec_id, const DeconvPeakPtr &deconv_peak_ptr,
                        double prec_mono_mass, const ActivationPtr &active_type_ptr,
  ...
  new_peak_ptr->setStrictTolerance(tole_ptr->compStrictErrorTole(peak_mass));
  new_peak_ptr->setNStrictCRelaxTolerance(tole_ptr->compRelaxErrorTole(peak_mass, prec_mono_mass));
  ...
  PrmPeakPtr reverse_peak_ptr
      = std::make_shared<PrmPeak>(spec_id, deconv_peak_ptr,
                                  BasePeakType::REVERSED, reverse_mass, default_score);
  reverse_peak_ptr->setStrictTolerance(tole_ptr->compStrictErrorTole(peak_mass));
  reverse_peak_ptr->setNStrictCRelaxTolerance(tole_ptr->compStrictErrorTole(peak_mass));
  reverse_peak_ptr->setNRelaxCStrictTolerance(tole_ptr->compRelaxErrorTole(peak_mass, prec_mono_mass));
  list.push_back(reverse_peak_ptr);
}

void addSixMasses(PrmPeakPtrVec &list, int spec_id, const DeconvPeakPtr &deconv_peak_ptr,
                  double prec_mono_mass, double n_term_label_mass, 
                  const ActivationPtr &active_type_ptr, const PeakTolerancePtr &tole_ptr, 
  ...
  for (size_t i = 0; i < offsets.size(); i++) {
    ...
    peak_ptr->setNStrictCRelaxTolerance(tole_ptr->compStrictErrorTole(peak_mass));
  ...
  for (size_t i = 0; i < offsets.size(); i++) {
    ...
    peak_ptr->setNStrictCRelaxTolerance(tole_ptr->compRelaxErrorTole(peak_mass, prec_mono_mass));
    list.push_back(peak_ptr);
  }
}

void addSuffixSixMasses(PrmPeakPtrVec &list, int spec_id, const DeconvPeakPtr &deconv_peak_ptr,
                        double prec_mono_mass, const ActivationPtr &active_type_ptr,
                        const PeakTolerancePtr &tole_ptr, const std::vector<double> &offsets) {
  double default_score = 1.0;
  ...
  for (size_t i = 0; i < offsets.size(); i++) {
    ...
    peak_ptr->setNStrictCRelaxTolerance(tole_ptr->compRelaxErrorTole(ori_mass, prec_mono_mass));
  ...
  for (size_t i = 0; i < offsets.size(); i++) {
    ...
    peak_ptr->setNStrictCRelaxTolerance(tole_ptr->compStrictErrorTole(ori_mass));
    list.push_back(peak_ptr);
  }
}
...
PrmMsPtr geneMsTwoPtr(const DeconvMsPtr &deconv_ms_ptr, int spec_id, const SpParaPtr &sp_para_ptr,
                      double prec_mono_mass, double n_term_label_mass, 
                      const std::vector<double> & mod_mass) {
