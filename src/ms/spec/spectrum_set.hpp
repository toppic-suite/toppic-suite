...
class SpectrumSet {
 public:
  SpectrumSet(const DeconvMsPtrVec &deconv_ms_ptr_vec,
  ...
  int getSpectrumId() const {return deconv_ms_ptr_vec_[0]->getMsHeaderPtr()->getSpecId();}

  const ExtendMsPtrVec& getMsThreePtrVec() const {return extend_ms_three_ptr_vec_;}
  ...
  const PrmMsPtrVec& getSuffixMsTwoPtrVec() const {return srm_ms_two_ptr_vec_;}
