...
namespace toppic {
  ...
  void SimpleMsAlignReader::readMsOneSpectra(const std::string &file_name, DeconvMsPtrVec &ms_ptr_vec) {
  ...
  void SimpleMsAlignReader::readMsTwoSpectra(const std::string &file_name, DeconvMsPtrVec &ms_ptr_vec) {
    SimpleMsAlignReader sp_reader(file_name);
    DeconvMsPtr ms_ptr;
    int ids = 0;
    while ((ms_ptr = sp_reader.getNextMsPtr()) != nullptr) {
