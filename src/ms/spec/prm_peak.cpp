...
PrmPeak::PrmPeak(int spec_id, const DeconvPeakPtr& base_peak_ptr,
                 const BasePeakTypePtr& base_type, double mono_mass,
                 double score, double strict_tolerance,
...
void PrmPeak::addNghbEdge(const DeconvPeakPtr& deconv_peak_ptr, double offset,
                          const SPTypePtr& peak_type_ptr, double score) {
  score_ += score;
  SupportPeakPtr support_peak_ptr = std::make_shared<SupportPeak>(
      deconv_peak_ptr, offset, score, peak_type_ptr);
  neighbor_list_.push_back(support_peak_ptr);
}

RmBreakTypePtr PrmPeak::getBreakType() const {
