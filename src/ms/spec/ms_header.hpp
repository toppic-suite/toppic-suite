...
class MsHeader {
 public:
  ...
  std::pair<int,int> getFirstPrecMonoMassMinusWaterError(double ppo, double scale) const;
  ...
  void setActivationPtr(ActivationPtr acti_ptr) {activation_ptr_ = std::move(acti_ptr);}

  // set function for precursor
  void setSinglePrecPtr(const PrecursorPtr &prec_ptr);

  void setPrecPtrVec(PrecursorPtrVec prec_ptr_vec) {prec_ptr_vec_ = std::move(prec_ptr_vec);}

  std::string toString() const;

  // Append the ms_header element under `parent` and return it.
  XmlDOMElement getHeaderXml(XmlDOMDocument* xml_doc, XmlDOMElement parent) const;
  ...
  static MsHeaderPtr geneMsHeaderPtr(const MsHeaderPtr &ori_ptr, double new_prec_mass);
