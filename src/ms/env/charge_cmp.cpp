...
std::vector<int> getSharedPeaks(const ExpEnvPtr& env_a,
                                const ExpEnvPtr& env_b) {
...
bool isSecondBetter(const PeakPtrVec& peak_list, const MatchEnvPtr& a,
                    const MatchEnvPtr& b, double tolerance) {
  // get shared peak
  std::vector<int> s_peaks =
      getSharedPeaks(a->getExpEnvPtr(), b->getExpEnvPtr());
  ...
  for (size_t i = 0; i < s_peaks.size(); i++) {
    for (size_t j = i + 1; j < s_peaks.size(); j++) {
      if (s_peaks[i] >= 0 && s_peaks[j] >= 0) {
        d = (peak_list[s_peaks[j]]->getPosition() -
             peak_list[s_peaks[i]]->getPosition()) /
            (j - i);
        dist.push_back(d);
      }
    }
...
int comp(const PeakPtrVec& peak_list, const MatchEnvPtr& a,
         const MatchEnvPtr& b, double tolerance) {
