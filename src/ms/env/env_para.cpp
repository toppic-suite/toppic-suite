...
int EnvPara::getMassGroup(double base_mass) {
  ...
  for (int i = 0; i < (int)mass_group_boundary_.size() - 1; i++) {
    if (base_mass >= mass_group_boundary_[i] &&
        base_mass < mass_group_boundary_[i + 1]) {
      group = i;
      break;
    }
  }
...
int EnvPara::compLowMassNum() {
  int low_mass_num =
      static_cast<int>(low_high_dividor_ / aa_avg_mass_ * peak_density_);
  return low_mass_num;
}

int EnvPara::compHighMassNum(double prec_mass) {
  int high_mass_num = static_cast<int>((prec_mass - low_high_dividor_) /
                                       aa_avg_mass_ * peak_density_);
  if (high_mass_num < 0) {
    high_mass_num = 0;
  }
