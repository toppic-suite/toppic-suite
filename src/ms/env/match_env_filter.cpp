...
MatchEnvPtrVec filterByAANum(MatchEnvPtrVec& ori_envs, double prec_mass,
  ...
  for (size_t i = 0; i < ori_envs.size(); i++) {
    if (ori_envs[i]->getExpEnvPtr()->getMonoNeutralMass() <=
        env_para_ptr->low_high_dividor_) {
  ...
  MatchEnvPtrVec result;
  result.insert(std::end(result), std::begin(low_mass_envs),
                std::end(low_mass_envs));
  result.insert(std::end(result), std::begin(high_mass_envs),
                std::end(high_mass_envs));
  if (use_msdeconv) {
