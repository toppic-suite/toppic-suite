...
#ifndef TOPPIC_MS_ENV_MATCH_ENV_HPP_
#define TOPPIC_MS_ENV_MATCH_ENV_HPP_
...
class MatchEnv {
 public:
  MatchEnv(int mass_group, const EnvPtr &theo_env_ptr,
  ...
  void setTheoEnvPtr(const EnvPtr &theo_env_ptr) { theo_env_ptr_ = theo_env_ptr;}

  void compMsdeconvScr(const EnvParaPtr &env_para_ptr);
 ...
 private:
  ...
  double findBestShift(const EnvParaPtr &env_para_ptr);

  double findBestRatio(const EnvParaPtr &env_para_ptr);
