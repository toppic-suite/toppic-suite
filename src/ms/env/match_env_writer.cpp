...
void write_env(std::ofstream& file, const MsHeaderPtr& header,
               const MatchEnvPtr& match_env) {
  ...
  file << "REAL_PEAK_NUM="
       << (real_env->getPeakNum() - real_env->getMissPeakNum()) << std::endl;
  ...
  for (int i = 0; i < theo_env->getPeakNum(); i++) {
    file << theo_env->getMz(i) << " " << theo_env->getInte(i) << " "
         << (real_env->isExist(i) ? "True" : "False") << " "
         << real_env->getPeakIdx(i) << " " << real_env->getMz(i) << " "
         << real_env->getInte(i) << std::endl;
  }

  file << "END ENVELOPE" << std::endl;
}

void write_env_vec(std::ofstream& file, const MsHeaderPtr& header,
...
void write(const std::string& file, const MsHeaderPtr& header,
           const MatchEnvPtrVec& envs) {
...
void writePeakList(const std::string& file, const PeakPtrVec& peak_list,
                   const MatchEnvPtrVec& envs) {
  std::ofstream of(file, std::ofstream::out);
  of << "PEAK_IDX\tORIG_MZ\tORIG_INTE\tTHEO_MONO_MZ\tTHEO_MONO_MASS\tTHEO_INTE_"
        "SUM\tTHEO_CHARGE\tENV_CNN_SCORE\tTHEO_MZ\tTHEO_NEUTRAL_MASS\tTHEO_INTE"
     << std::endl;
  ...
  for (size_t i = 0; i < envs.size(); i++) {
    ...
    for (int j = 0; j < real_env->getPeakNum(); j++) {
      if (real_env->isExist(j) > 0) {
        ...
        of << exp_peak_ptr->getIdx() << "\t" << orig_peak_ptr->getPosition()
           << "\t" << orig_peak_ptr->getIntensity() << "\t"
           << theo_env->getMonoMz() << "\t" << theo_env->getMonoNeutralMass()
           << "\t" << theo_env->compInteSum() << "\t" << theo_env->getCharge()
           << "\t" << envs[i]->getEnvcnnScore() << "\t"
           << theo_peak_ptr->getPosition() << "\t"
           << peak_util::compPeakNeutralMass(theo_peak_ptr->getPosition(),
                                             theo_env->getCharge())
           << "\t" << theo_peak_ptr->getIntensity() << std::endl;
      }
    }
  }
  of.close();
