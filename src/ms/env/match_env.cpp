...
MatchEnv::MatchEnv(int mass_group, const EnvPtr& theo_env_ptr,
                   const ExpEnvPtr& real_env_ptr)
...
double calcNormInteScr(double intensity) { return std::sqrt(intensity); }

}  // namespace

// Scoring function
// compute matching score_
void MatchEnv::compMsdeconvScr(const EnvParaPtr& env_para_ptr) {
  ...
  msdeconv_score_ =
      calcScrWithSftRatio(0, 1, env_para_ptr->score_error_tolerance_);
}

// search for best m/z shift
double MatchEnv::findBestShift(const EnvParaPtr& env_para_ptr) {
  double best_score = -std::numeric_limits<double>::infinity();
  int best_shift = 0;
  int charge = theo_env_ptr_->getCharge();
  // initialize start shift and end shift based on configuration
  int bgn_shift = (int)std::round(-env_para_ptr->getMzTolerance(charge) *
                                  env_para_ptr->shift_scale_);
  int end_shift = (int)std::round(env_para_ptr->getMzTolerance(charge) *
                                  env_para_ptr->shift_scale_);

  for (int s = bgn_shift; s <= end_shift; s++) {
    double tmp_score =
        calcScrWithSftRatio((double)s / env_para_ptr->shift_scale_, 1.0,
                            env_para_ptr->score_error_tolerance_);
...
double MatchEnv::findBestRatio(const EnvParaPtr& env_para_ptr) {
  double best_score = -std::numeric_limits<double>::infinity();
  int best_ratio = 0;
  // initialize start ratio and end ratio based on configuration
  int bgn_ratio = (int)std::round(env_para_ptr->bgn_ratio_ *
                                  env_para_ptr->inte_ratio_scale_);
  int end_ratio = (int)std::round(env_para_ptr->end_ratio_ *
                                  env_para_ptr->inte_ratio_scale_);

  for (int r = bgn_ratio; r <= end_ratio; r++) {
    double tmp_score =
        calcScrWithSftRatio(0, (double)r / env_para_ptr->inte_ratio_scale_,
                            env_para_ptr->score_error_tolerance_);
    if (tmp_score > best_score) {
      best_score = tmp_score;
      best_ratio = r;
...
double MatchEnv::calcScrWithSftRatio(double shift, double ratio,
                                     double tolerance) {
  double s = 0;
...
double MatchEnv::calcMzFactor(int id_x, double shift, double tolerance) {
  ...
  if (exp_env_ptr_->isExist(id_x)) {
    double dist = std::abs(theo_env_ptr_->getMz(id_x) + shift -
                           exp_env_ptr_->getMz(id_x));
...
void MatchEnv::appendXml(XmlDOMDocument* xml_doc, XmlDOMElement parent) const {
