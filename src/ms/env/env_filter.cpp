...
void filterEnvByRealEnv(MatchEnvPtr2D &match_envs, const EnvParaPtr &env_para_ptr) {
  for (size_t i = 0; i < match_envs.size(); i++) {
    for (size_t j = 0; j < match_envs[i].size(); j++) {
      if (match_envs[i][j] != nullptr) {
...
void filter(MatchEnvPtr2D &match_envs, const PeakPtrVec &peak_list, 
  ...
  LOG_DEBUG("Valid match envelope number " << cntValid(match_envs));
  LOG_DEBUG("Filtering by charge...");
  filterEnvByChrg(match_envs);

  LOG_DEBUG("Valid match envelope number " << cntValid(match_envs));
  LOG_DEBUG("Filtering by charge comparison...");
...
// Filtering methods 
void multipleMassFilter(MatchEnvPtr2D &match_envs, const EnvParaPtr &env_para_ptr) {
