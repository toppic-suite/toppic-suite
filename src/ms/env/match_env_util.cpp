  ...
  for (size_t i = 0; i < envs.size(); i++) {
    ...
    for (int j = 0; j < env->getExpEnvPtr()->getPeakNum(); j++) {
      ...
      if (peak >= 0) {
        ...
        double intensity = real_env->getInte(j) *
                           env->getTheoEnvPtr()->getInte(j) /
...
MatchEnvPtrVec addUnusedMasses(MatchEnvPtrVec& envs, PeakPtrVec& ms,
                               double tolerance) {
  ...
  result.insert(std::end(result), std::begin(unused_mass_envs),
                std::end(unused_mass_envs));
...
MatchEnvPtrVec addMultipleMass(MatchEnvPtrVec& envs, MatchEnvPtr2D& candidates,
  ...
  for (size_t i = 0; i < envs.size(); i++) {
    ...
    double min_score =
        charge_envs[0]->getMsdeconvScore() * env_para_ptr->multiple_min_ratio_;
    if (charge >= env_para_ptr->multiple_min_charge_) {
      double score_minus_one = 0;
      // Env for charge - 1
      if (charge > 1 && candidates[refer_peak][charge - 2] != nullptr) {
        score_minus_one =
            candidates[refer_peak][charge - 2]->getMsdeconvScore();
      }
      double score_plus_one = 0;
      // Env for charge + 1
      if (charge < static_cast<int>(candidates[0].size()) &&
          candidates[refer_peak][charge] != nullptr) {
...
DeconvMsPtr getDeconvMsPtr(const MsHeaderPtr& header_ptr,
                           MatchEnvPtrVec& envs) {
  ...
  for (size_t i = 0; i < envs.size(); i++) {
    ...
    DeconvPeakPtr peak_ptr =
        std::make_shared<DeconvPeak>(sp_id, i, pos, inte, charge, score);
    peak_list.push_back(peak_ptr);
  }
  DeconvMsPtr ms_ptr =
      std::make_shared<DeconvMs>(header_ptr, std::move(peak_list));
  return ms_ptr;
}
