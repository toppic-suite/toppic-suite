...
MzmlMsGroupReader::MzmlMsGroupReader(const std::string& file_name,
  ...
  reader_ptr_ =
      std::make_shared<PwMsReader>(file_name, isolation_window, activation);
...
MzmlMsPtr MzmlMsGroupReader::readNextMzmlMs() {
  ...
  const PeakPtrVec& peak_list = reader_ptr_->getPeakList();
...
MzmlMsGroupPtr MzmlMsGroupReader::getMs2OnlyMsGroupPtr() {
  ...
  MzmlMsGroupPtr ms_group_ptr =
      std::make_shared<MzmlMsGroup>(null_ms_one_ptr, ms_two_ptr_vec);
  return ms_group_ptr;
}
...
MzmlMsGroupPtr MzmlMsGroupReader::getMs1Ms2MsGroupPtr() {
  ...
  while (true) {
    ...
    if (cur_last_ms_two_scan_map_.count(ms1_scan) > 0 &&
        cur_last_ms_two_scan_map_[ms1_scan] >=
            last_ms_two_scan_map_[ms1_scan]) {
      if (last_ms_two_scan_map_[ms1_scan] == -1) {
        // if the ms1 scan does not have ms2, return an ms group with the ms1
        // scan only
        MzmlMsGroupPtr ms_group_ptr = std::make_shared<MzmlMsGroup>(
            ms_one_ptr_map_[ms1_scan], MzmlMsPtrVec());
        ...
        MzmlMsGroupPtr ms_group_ptr = std::make_shared<MzmlMsGroup>(
            ms_one_ptr_map_[ms1_scan], ms_two_ptr_vec_map_[ms1_scan]);
        ms_one_ptr_map_[ms1_scan] = nullptr;
...
void MzmlMsGroupReader::getMs1Map(PeakPtrVec2D& ms1_mzml_peaks,
  ...
  while (true) {
    ...
    const PeakPtrVec& peak_list = group_ptr->getMsOnePtr()->getPeakPtrVec();
    ms1_mzml_peaks.push_back(peak_list);
    const MzmlMsPtrVec& ms2_ptr_vec = group_ptr->getMsTwoPtrVec();
...
int MzmlMsGroupReader::getMs2Map(PeakPtrVec2D& ms2_mzml_peaks,
                                 double win_mz_begin) {
  reader_ptr_->resetIndexes();
  int peak_num = 0;
  while (true) {
    ...
    MsHeaderPtrVec header_vec;
    for (size_t i = 0; i < ms2_ptr_vec.size(); i++) {
      double cur_win_begin =
          ms2_ptr_vec[i]->getMsHeaderPtr()->getPrecWinBegin();
      // if (cur_win_begin == win_mz_begin) {
      // Because a small error may be introduced in curr_win_begin when we write
      // the window begin value into the msalign file, we use a tolerance of
      // 1e-4 here Using 1e-5 is also ok.
      if (std::abs(cur_win_begin - win_mz_begin) < 1e-4) {
        const PeakPtrVec& peak_list = ms2_ptr_vec[i]->getPeakPtrVec();
