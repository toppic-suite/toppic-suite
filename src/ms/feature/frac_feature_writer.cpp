...
void writeXmlFeatures(const std::string& output_file_name,
  ...
  for (std::size_t i = 0; i < features.size(); i++) {
    // pugixml has no detached nodes, so build the frac_feature element under a
    // throwaway document root and serialize just that element.
    XmlDOMDocument doc("frac_feature_list");
    XmlDOMElement root = doc.getDocumentElement();
    XmlDOMElement element = features[i]->toXmlElement(&doc, root);
