...
class FracFeature {
 public:
  ...
  FracFeature(const std::string& file_name, int frac_id, int feat_id,
              double mono_mass, double inte, int min_ms1_id, int max_ms1_id,
              double retent_begin, double retent_end, int scan_begin,
              int scan_end, int min_charge, int max_charge, double apex_time,
              int apex_scan, double apex_inte, int rep_charge,
              double rep_avg_mz, int env_num, double ec_score);
  ...
  const SingleChargeFeaturePtrVec& getSingleFeatures() const {
    return single_features_;
  }
  ...
  static bool cmpFracIncInteDec(const FracFeaturePtr& a,
