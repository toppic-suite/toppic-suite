...
void writeHeader(std::ofstream& of) {
  of << "File_name" << "\t"
     ...
     << "Precursor_charge" << "\t"
     << "Precursor_intensity" << std::endl;
}

void writeOneFeature(std::ofstream& of, const SpecFeaturePtr& feature) {
  of << feature->getFileName() << "\t" << feature->getFracId() << "\t"
     << feature->getSpecId() << "\t" << feature->getScans() << "\t"
     << feature->getMsOneId() << "\t" << feature->getMsOneScan() << "\t"
     << feature->getFracFeatureId() << "\t" << feature->getFracFeatureInte()
     << "\t" << feature->getFracFeatureScore() << "\t"
     << feature->getFracFeatureMinTime() / 60 << "\t"
     << feature->getFracFeatureMaxTime() / 60 << "\t"
     << feature->getFracFeatureApexTime() / 60 << "\t" << std::fixed
     << std::setprecision(5) << feature->getPrecMonoMass() << "\t"
     << feature->getPrecMonoMz() << "\t" << feature->getPrecAvgMz() << "\t"
     << std::setprecision(0) << feature->getPrecCharge() << "\t"
     << std::setprecision(5) << feature->getPrecInte() << std::endl;
}
