...
//		for(unsigned int i =0;i<deconv_sp->size();i++){
			SpectrumSetPtr spectrum_set = prot::getSpectrumSet(deconv_sp,0,mng_->sp_para_,prot::getProtModPtrByName(mng_->base_data->getProtModPtrVec(),"ACETYLATION")->getPepShift(),mng_->base_data->getIonTypePtrVec());
