...
//		for(unsigned int i =0;i<deconv_sp->size();i++){
			SpectrumSetPtr spectrum_set = prot::getSpectrumSet(deconv_sp,0,mng_->sp_para_,prot::getProtModPtrByName(mng_->base_data->getProtModPtrVec(),"ACETYLATION")->getPepShift(),IonTypeFactory::getBaseIonTypePtrVec());
...
void PtmFastFilterProcessor::combineBlock(std::string sp_file_name){
	...
	for(int i=0;i<filter_->getBlockSize();i++){
		std::stringstream block_s;
		block_s<<i;
		std::string block_file_name = mng_->spectrum_file_name_+ ".FILTER" + mng_->output_file_ext_+"_"+block_s.str();
		matches.push_back(prot::readSimplePrSM(block_file_name.c_str()));
	}

	MsAlignReader reader(sp_file_name.c_str());
	std::string output_file_name = mng_->spectrum_file_name_ + ".FILTER" + mng_->output_file_ext_+"_COMBINED";
	SimplePrSMWriter writer(output_file_name.c_str());
	DeconvMsPtr deconv_sp;
	while((deconv_sp = reader.getNextMs()) != nullptr){
