...
VarPtmFilterMng::VarPtmFilterMng(const PrsmParaPtr& prsm_para_ptr,
                                 const std::string& index_file_para,
                                 const std::string& var_ptm_file_name,
                                 int var_ptm_num, int thread_num,
...
std::vector<int> VarPtmFilterMng::computeShifts(
  ...
  for (int i = 0; i < var_ptm_num; i++) {
    ...
    for (size_t j = 0; j < cur_list_len; j++) {
      for (size_t k = 0; k < round_single_shift_list.size(); k++) {
        ...
        if (std::find(round_shift_list.begin(), round_shift_list.end(),
                      new_shift) == round_shift_list.end()) {
