...
void geneOnePtmIndexFile(const ProteoformPtrVec& proteo_ptrs,
  ...
  std::vector<std::vector<double>> shift_2d = proteoform_util::getNTermShift2D(
  ...
  std::vector<std::vector<double>> n_term_acet_2d =
      proteoform_util::getNTermAcet2D(
          proteo_ptrs, mng_ptr->prsm_para_ptr_->getProtModPtrVec());
  MassMatchPtr term_index_ptr = mass_match_factory::getPrmTermMassMatchPtr(
  ...
  MassMatchPtr diag_index_ptr = mass_match_factory::getPrmDiagMassMatchPtr(
  ...
  MassMatchPtr rev_term_index_ptr = mass_match_factory::getSrmTermMassMatchPtr(
      proteo_ptrs, rev_shift_2d, n_term_acet_2d, mng_ptr->max_proteoform_mass_,
      mng_ptr->filter_scale_);
  {
    std::lock_guard<std::mutex> lock(serial_mutex);
  ...
  MassMatchPtr rev_diag_index_ptr = mass_match_factory::getSrmDiagMassMatchPtr(
...
void createIndexFiles(const ProteoformPtrVec& raw_forms, int block_idx,
                      const OnePtmFilterMngPtr& mng_ptr) {
...
void process(const OnePtmFilterMngPtr& mng_ptr) {
  ...
  for (int i = 0; i < block_num; i++) {
    ...
    std::cout << "One shift index files - processing " << (i + 1) << " of "
              << block_num << " files." << std::endl;
    pool_ptr->enqueue(
        geneIndexTask(db_block_ptr_vec[i]->getBlockIdx(), mng_ptr));
  }
  pool_ptr->shutDown();
