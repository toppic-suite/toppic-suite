...
inline std::vector<int> getScaledSrmMasses(const ProteoformPtr& proteo_ptr,
                                           std::vector<double>& n_ace_shifts,
                                           double scale) {
  std::vector<int> masses =
      proteo_ptr->getBpSpecPtr()->getScaledPrmMasses(scale);
...
inline MassMatchPtr getMassMatchPtr(const ProteoformPtrVec& proteo_ptrs,
  ...
  MassMatchPtr index_ptr = std::make_shared<MassMatch>(
      proteo_minus_water_masses, mass_2d, float_shift_2d, pos_2d,
...
MassMatchPtr getPrmDiagMassMatchPtr(const ProteoformPtrVec& proteo_ptrs,
  ...
  for (size_t i = 0; i < proteo_ptrs.size(); i++) {
    std::vector<int> masses =
        proteo_ptrs[i]->getBpSpecPtr()->getScaledPrmMasses(scale);
...
MassMatchPtr getSrmDiagMassMatchPtr(
  ...
  for (size_t i = 0; i < proteo_ptrs.size(); i++) {
    std::vector<int> masses =
        getScaledSrmMasses(proteo_ptrs[i], n_ace_shift_2d[i], scale);
    mass_2d.push_back(masses);
  }
  LOG_DEBUG("mass 2d ver 1 complete");
  bool prm = false;
...
MassMatchPtr getPrmTermMassMatchPtr(
    const ProteoformPtrVec& proteo_ptrs,
    std::vector<std::vector<double>>& float_shift_2d,
    double max_proteoform_mass, double scale) {
  std::vector<double> proteo_minus_water_masses;
...
MassMatchPtr getSrmTermMassMatchPtr(
    const ProteoformPtrVec& proteo_ptrs,
    std::vector<std::vector<double>>& float_shift_2d,
    std::vector<std::vector<double>>& n_ace_shift_2d,
    double max_proteoform_mass, double scale) {
