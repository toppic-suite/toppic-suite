...
// initialize ps_align
inline void OnePtmSlowMatch::initPsAlign(CompShiftLowMemPtr comp_shift_ptr){
...
inline DiagonalHeaderPtrVec OnePtmSlowMatch::getNTermShiftHeaders(
...
void OnePtmSlowMatch::compute(AlignTypePtr type_ptr, PrsmPtrVec &prsm_ptrs) {
    ps_align_ptr_->compute(type_ptr);
    for (int s = 1; s <= mng_ptr_->align_para_ptr_->getUnknownShiftNum(); s++) {
