...
void PrsmCoverage::compOneCoverage(std::ofstream &file, const PrsmPtr &prsm_ptr,
  ...
  for (size_t i = 0; i < deconv_ms_ptr_vec.size(); i++) {
    spec_ids = spec_ids + std::to_string(deconv_ms_ptr_vec[i]->getMsHeaderPtr()->getSpecId()) + " ";
...
void PrsmCoverage::outputMatchPeaks(const PrsmPtr &prsm_ptr, const PrsmParaPtr &prsm_para_ptr) {
...
void PrsmCoverage::processTwoPrsms(std::ofstream &file, const PrsmPtr &prsm_ptr_1, const PrsmPtr &prsm_ptr_2,
                                   const PrsmParaPtr &prsm_para_ptr) {
  double min_mass = prsm_para_ptr_->getSpParaPtr()->getMinMass();
