...
PrsmCoverage::PrsmCoverage(const PrsmParaPtr& prsm_para_ptr,
...
void PrsmCoverage::processSingleCoverage() {
  ...
  std::string input_file_name =
      file_util::basename(sp_file_name) + "." + input_file_ext_;
  ...
  MsAlignReaderPtr ms_reader_ptr = std::make_shared<MsAlignReader>(
      sp_file_name, group_spec_num,
      prsm_para_ptr_->getSpParaPtr()->getActivationPtr());
  int cnt = 0;
  SpectrumSetPtr spec_set_ptr;

  std::string output_file_name =
      file_util::basename(sp_file_name) + "." + output_file_ext_;
  std::ofstream out_stream;
  out_stream.open(output_file_name.c_str());
  // write title
  printTitle(out_stream);
  SpParaPtr sp_para_ptr = prsm_para_ptr_->getSpParaPtr();
  while ((spec_set_ptr = spectrum_set_factory::readNextSpectrumSetPtr(
              ms_reader_ptr, sp_para_ptr)) != nullptr) {
    cnt += group_spec_num;
    ...
    std::cout << std::flush << "PrSM coverage is processing " << cnt << " of "
...
void PrsmCoverage::printTitle(std::ofstream& file) {
  file << "Data_file_name" << "\t"
       ...
       << "right_Both_term_ion_coverage" << "\t" << std::endl;
}
...
void PrsmCoverage::computeCoverage(std::ofstream& file, const PrsmPtr& prsm_ptr,
                                   PeakIonPairPtrVec& pair_ptrs,
                                   const PrsmParaPtr& prsm_para_ptr) {
  int len = prsm_ptr->getProteoformPtr()->getResSeqPtr()->getLen() - 1;
  int begin = 1;
  int end = len - 1;
  double n_full_coverage = peak_ion_pair_util::computePairCoverage(
      pair_ptrs, begin, end, RmBreakType::N_TERM);
  double c_full_coverage = peak_ion_pair_util::computePairCoverage(
      pair_ptrs, begin, end, RmBreakType::C_TERM);
  double both_full_coverage = peak_ion_pair_util::computePairCoverage(
      pair_ptrs, begin, end, RmBreakType::BOTH);
  int one_third = len / 3;
  end = one_third;
  double left_n_full_coverage = peak_ion_pair_util::computePairCoverage(
      pair_ptrs, begin, end, RmBreakType::N_TERM);
  double left_c_full_coverage = peak_ion_pair_util::computePairCoverage(
      pair_ptrs, begin, end, RmBreakType::C_TERM);
  double left_both_full_coverage = peak_ion_pair_util::computePairCoverage(
      pair_ptrs, begin, end, RmBreakType::BOTH);
  begin = one_third + 1;
  int two_thirds = len / 3 * 2;
  end = two_thirds;
  double middle_n_full_coverage = peak_ion_pair_util::computePairCoverage(
      pair_ptrs, begin, end, RmBreakType::N_TERM);
  double middle_c_full_coverage = peak_ion_pair_util::computePairCoverage(
      pair_ptrs, begin, end, RmBreakType::C_TERM);
  double middle_both_full_coverage = peak_ion_pair_util::computePairCoverage(
      pair_ptrs, begin, end, RmBreakType::BOTH);
  begin = two_thirds + 1;
  end = len - 1;
  double right_n_full_coverage = peak_ion_pair_util::computePairCoverage(
      pair_ptrs, begin, end, RmBreakType::N_TERM);
  double right_c_full_coverage = peak_ion_pair_util::computePairCoverage(
      pair_ptrs, begin, end, RmBreakType::C_TERM);
  double right_both_full_coverage = peak_ion_pair_util::computePairCoverage(
      pair_ptrs, begin, end, RmBreakType::BOTH);
  file << n_full_coverage << "\t" << c_full_coverage << "\t"
       << both_full_coverage << "\t" << left_n_full_coverage << "\t"
       << left_c_full_coverage << "\t" << left_both_full_coverage << "\t"
       << middle_n_full_coverage << "\t" << middle_c_full_coverage << "\t"
       << middle_both_full_coverage << "\t" << right_n_full_coverage << "\t"
       << right_c_full_coverage << "\t" << right_both_full_coverage << "\t";
}

void PrsmCoverage::compOneCoverage(std::ofstream& file, const PrsmPtr& prsm_ptr,
                                   PeakIonPairPtrVec& pair_ptrs,
                                   const PrsmParaPtr& prsm_para_ptr) {
  std::string spec_ids;
  ...
  file << prsm_para_ptr_->getSpectrumFileName() << "\t" << prsm_ptr->getPrsmId()
...
void PrsmCoverage::compTwoCoverage(std::ofstream& file, const PrsmPtr& prsm_ptr,
...
void PrsmCoverage::outputMatchPeaks(const PrsmPtr& prsm_ptr,
                                    const PrsmParaPtr& prsm_para_ptr) {
  double min_mass = prsm_para_ptr_->getSpParaPtr()->getMinMass();
  PeakIonPairPtrVec pair_ptrs = peak_ion_pair_util::genePeakIonPairs(
      prsm_ptr->getProteoformPtr(), prsm_ptr->getRefineMsPtrVec(), min_mass);
  std::string file_name =
      "scan_" +
      prsm_ptr->getDeconvMsPtrVec()[0]->getMsHeaderPtr()->getScansString() +
      ".peak";
...
void PrsmCoverage::processOnePrsm(std::ofstream& file, const PrsmPtr& prsm_ptr,
                                  const PrsmParaPtr& prsm_para_ptr) {
  double min_mass = prsm_para_ptr_->getSpParaPtr()->getMinMass();
  PeakIonPairPtrVec pair_ptrs = peak_ion_pair_util::genePeakIonPairs(
      prsm_ptr->getProteoformPtr(), prsm_ptr->getRefineMsPtrVec(), min_mass);
...
void PrsmCoverage::processTwoPrsms(std::ofstream& file,
                                   const PrsmPtr& prsm_ptr_1,
                                   const PrsmPtr& prsm_ptr_2,
                                   const PrsmParaPtr& prsm_para_ptr) {
  double min_mass = prsm_para_ptr_->getSpParaPtr()->getMinMass();
  PeakIonPairPtrVec pair_ptrs_11;
  if (prsm_ptr_1 != nullptr) {
    pair_ptrs_11 = peak_ion_pair_util::genePeakIonPairs(
        prsm_ptr_1->getProteoformPtr(), prsm_ptr_1->getRefineMsPtrVec(),
        min_mass);
  }

  PeakIonPairPtrVec pair_ptrs_12;
  if (prsm_ptr_1 != nullptr && prsm_ptr_2 != nullptr) {
    pair_ptrs_12 = peak_ion_pair_util::genePeakIonPairs(
        prsm_ptr_1->getProteoformPtr(), prsm_ptr_2->getRefineMsPtrVec(),
        min_mass);
  }

  PeakIonPairPtrVec pair_ptrs_1;
  pair_ptrs_1.insert(pair_ptrs_1.begin(), pair_ptrs_11.begin(),
                     pair_ptrs_11.end());
  pair_ptrs_1.insert(pair_ptrs_1.begin(), pair_ptrs_12.begin(),
                     pair_ptrs_12.end());
  if (prsm_ptr_1 != nullptr) {
    compTwoCoverage(file, prsm_ptr_1, pair_ptrs_11, pair_ptrs_12, pair_ptrs_1,
                    prsm_para_ptr);
  }

  PeakIonPairPtrVec pair_ptrs_21;
  if (prsm_ptr_1 != nullptr && prsm_ptr_2 != nullptr) {
    pair_ptrs_21 = peak_ion_pair_util::genePeakIonPairs(
        prsm_ptr_2->getProteoformPtr(), prsm_ptr_1->getRefineMsPtrVec(),
        min_mass);
  }

  PeakIonPairPtrVec pair_ptrs_22;
  if (prsm_ptr_2 != nullptr) {
    pair_ptrs_22 = peak_ion_pair_util::genePeakIonPairs(
        prsm_ptr_2->getProteoformPtr(), prsm_ptr_2->getRefineMsPtrVec(),
        min_mass);
  }

  PeakIonPairPtrVec pair_ptrs_2;
  pair_ptrs_2.insert(pair_ptrs_2.begin(), pair_ptrs_21.begin(),
                     pair_ptrs_21.end());
  pair_ptrs_2.insert(pair_ptrs_2.begin(), pair_ptrs_22.begin(),
                     pair_ptrs_22.end());
  if (prsm_ptr_2 != nullptr) {
    compTwoCoverage(file, prsm_ptr_2, pair_ptrs_21, pair_ptrs_22, pair_ptrs_2,
                    prsm_para_ptr);
  }
}
}  // namespace toppic
