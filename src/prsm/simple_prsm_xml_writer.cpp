...
void SimplePrsmXmlWriter::write(const SimplePrsmStrPtr &prsm_str_ptr) {
...
void SimplePrsmXmlWriter::write(const SimplePrsmPtr &simple_prsm_ptr) {
  // pugixml has no detached nodes, so build the simple_prsm element under a
  // throwaway document root and serialize just that element.
  XmlDOMDocument doc("simple_prsm_list");
  XmlDOMElement root = doc.getDocumentElement();
  XmlDOMElement element = simple_prsm_ptr->toXml(&doc, root);
