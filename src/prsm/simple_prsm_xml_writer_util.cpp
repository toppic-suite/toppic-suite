...
SimplePrsmXmlWriterPtrVec geneWriterPtrVec(std::string& file_name,
                                           int thread_num) {
  SimplePrsmXmlWriterPtrVec writer_ptr_vec;
  for (int i = 0; i < thread_num; i++) {
    std::string writer_file_name = file_name + "_" + std::to_string(i);
    SimplePrsmXmlWriterPtr writer_ptr =
        std::make_shared<SimplePrsmXmlWriter>(writer_file_name);
