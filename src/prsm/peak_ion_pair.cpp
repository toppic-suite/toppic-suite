...
PeakIonPair::PeakIonPair(int spec_id, ExtendPeakPtr real_peak_ptr,
    ...
    : spec_id_(spec_id),
      real_peak_ptr_(std::move(real_peak_ptr)),
      theo_peak_ptr_(std::move(theo_peak_ptr)) {}

void PeakIonPair::appendRealPeakToXml(XmlDOMDocument* xml_doc,
                                      XmlDOMElement parent) {
  XmlDOMElement element = xml_doc->addElement(parent, "matched_peak");
  std::string str = theo_peak_ptr_->getIonPtr()->getIonTypePtr()->getName();
  xml_doc->addElement(element, "ion_type", str.c_str());
  str = std::to_string(theo_peak_ptr_->getIonPtr()->getPos());
  xml_doc->addElement(element, "ion_position", str.c_str());
  str = std::to_string(theo_peak_ptr_->getIonPtr()->getDisplayPos());
  xml_doc->addElement(element, "ion_display_position", str.c_str());
  str = std::to_string(spec_id_);
  xml_doc->addElement(element, "spec_id", str.c_str());
  str = std::to_string(real_peak_ptr_->getBasePeakPtr()->getPeakId());
  xml_doc->addElement(element, "peak_id", str.c_str());
  str = std::to_string(real_peak_ptr_->getBasePeakPtr()->getCharge());
...
void PeakIonPair::appendTheoPeakToXml(XmlDOMDocument* xml_doc,
                                      XmlDOMElement parent) {
  int precision = 4;
  XmlDOMElement element = xml_doc->addElement(parent, "matched_ion");
  std::string str = theo_peak_ptr_->getIonPtr()->getIonTypePtr()->getName();
  xml_doc->addElement(element, "ion_type", str.c_str());
  str = str_util::toString(theo_peak_ptr_->getShift());
  xml_doc->addElement(element, "match_shift", str.c_str());
  str = str_util::fixedToString(theo_peak_ptr_->getModMass(), precision);
  ...
  std::string disp_pos =
      std::to_string(theo_peak_ptr_->getIonPtr()->getDisplayPos());
  ...
  str = str_util::fixedToString(error, precision);
  xml_doc->addElement(element, "mass_error", str.c_str());
  str = str_util::fixedToString(error * 1000000 / real_peak_ptr_->getMonoMass(),
                                precision - 2);
...
bool PeakIonPair::cmpTheoPeakPosInc(const PeakIonPairPtr& a,
                                    const PeakIonPairPtr& b) {
