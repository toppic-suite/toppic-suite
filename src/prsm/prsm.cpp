...
double Prsm::getNormMatchFragNum() {
  int var_change_num = proteoform_ptr_->getChangeNum(ChangeType::VARIABLE);
  int unexp_change_num = proteoform_ptr_->getChangeNum(ChangeType::UNEXPECTED);
  ...
  if (end_pos == proteoform_ptr_->getFastaSeqPtr()->getLen() - 1) {
    score = score + 2;
  }
  return score;
}
