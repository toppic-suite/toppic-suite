...
PrsmXmlWriterSet::PrsmXmlWriterSet(const std::string& output_file_name,
  ...
  for (int s = 2; s <= n_mass_shift_; s++) {
    ...
    std::string file_name =
        output_file_name + "_" + ProteoformType::COMPLETE->getName() + end_str;
    PrsmXmlWriterPtr complete_writer_ptr =
        std::make_shared<PrsmXmlWriter>(file_name);
    complete_writer_ptrs_.push_back(complete_writer_ptr);
    file_name =
        output_file_name + "_" + ProteoformType::PREFIX->getName() + end_str;
    PrsmXmlWriterPtr prefix_writer_ptr =
        std::make_shared<PrsmXmlWriter>(file_name);
    prefix_writer_ptrs_.push_back(prefix_writer_ptr);
    file_name =
        output_file_name + "_" + ProteoformType::SUFFIX->getName() + end_str;
    PrsmXmlWriterPtr suffix_writer_ptr =
        std::make_shared<PrsmXmlWriter>(file_name);
    suffix_writer_ptrs_.push_back(suffix_writer_ptr);
    file_name =
        output_file_name + "_" + ProteoformType::INTERNAL->getName() + end_str;
    PrsmXmlWriterPtr internal_writer_ptr =
        std::make_shared<PrsmXmlWriter>(file_name);
