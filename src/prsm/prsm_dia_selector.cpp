...
bool PrsmDiaSelector::containsSameFastaSeq(const PrsmStrPtrVec prsm_ptrs,
                                           const PrsmStrPtr& target_prsm_ptr) {
...
void PrsmDiaSelector::process() {
  ...
  while (prsm_str_ptr != nullptr) {
    ...
    while (prsm_str_ptr != nullptr &&
           prsm_str_ptr->getSpectrumScan() == scan_num) {
      PrsmStrPtrVec cur_str_ptrs;
      while (prsm_str_ptr != nullptr &&
             prsm_str_ptr->getSpectrumId() == spec_id) {
      ...
      scan_result_ptrs.insert(scan_result_ptrs.end(), result_ptrs.begin(),
                              result_ptrs.end());
      spec_id++;
    }
