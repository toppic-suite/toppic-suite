...
void setValueStr(std::vector<std::string>& str_vec, const std::string& property,
                 std::string val) {
...
std::vector<int> getClusterIds(const PrsmPtrVec& prsm_ptrs,
  ...
  for (size_t i = 0; i < prsm_ptrs.size(); i++) {
    if (prsm_ptrs[i]->getProteoformPtr()->getSeqName() == seq_name)
      cluster_id_set.insert(
          prsm_ptrs[i]->getProteoformPtr()->getProteoClusterId());
  }
  std::copy(cluster_id_set.begin(), cluster_id_set.end(),
...
bool isMatchMs(const PrsmPtr& prsm_ptr, const MsHeaderPtr& header_ptr) {
  int id = header_ptr->getSpecId();
  std::string scan = header_ptr->getScansString();
  int prec_id = header_ptr->getFirstPrecId();
  if (prsm_ptr->getSpectrumId() == id &&
      prsm_ptr->getPrecursorId() == prec_id) {
...
void addSpectrumPtrsToPrsms(PrsmPtrVec& prsm_ptrs,
                            const PrsmParaPtr& prsm_para_ptr) {
  SpParaPtr sp_para_ptr = prsm_para_ptr->getSpParaPtr();
  MsAlignReaderPtr ms_reader_ptr = std::make_shared<MsAlignReader>(
      prsm_para_ptr->getSpectrumFileName(), prsm_para_ptr->getGroupSpecNum(),
    ...
    if (spec_set_ptr->isValid()) {
      ...
      for (size_t i = start_prsm; i < prsm_ptrs.size(); i++) {
        if (isMatchMs(prsm_ptrs[i], deconv_ms_ptr_vec[0]->getMsHeaderPtr())) {
          ...
          prsm_ptrs[i]->setRefineMsVec(extend_ms_factory::geneMsThreePtrVec(
              deconv_ms_ptr_vec, prsm_para_ptr->getSpParaPtr(), new_prec_mass));
        }
...
void addFeatureInfoToPrsms(PrsmStrPtrVec& prsm_ptrs,
...
void mergePrsmFiles(const std::vector<std::string>& prsm_file_lst,
                    int max_spec_num_per_file, int max_feat_num_per_file,
  ...
  for (size_t i = 0; i < prsm_file_lst.size(); i++) {
    ...
    while (prsm != nullptr) {
      ...
      prsm->setFracFeatureId(max_feat_num_per_file * i +
                             prsm->getFracFeatureId());
...
double compClusterInte(const PrsmStrPtrVec& prsm_list) {
  double inte = 0;
