...
int countAcidLeftCoverage(const ResSeqPtr &res_seq_ptr, const AminoAcidPtr &acid_ptr,
...
int countAcidRightCoverage(const ResSeqPtr &res_seq_ptr, const AminoAcidPtr &acid_ptr,
                           const std::vector<bool> &match_ion_vec) {
  int count = 0;
  for (int i = 0; i < res_seq_ptr->getLen(); i++) {
...
void writePrsm(std::ofstream &file, const PrsmPtr &prsm_ptr, const PrsmParaPtr &prsm_para_ptr) {
  std::string spec_ids;
