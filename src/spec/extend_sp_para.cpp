...
ExtendSpPara::ExtendSpPara(double extend_min_mass, std::vector<double> ext_offsets) {
...
ExtendSpPara::ExtendSpPara(xercesc::DOMElement* element) {
  ...
  xercesc::DOMElement* list_element = getChildElement(element, "extend_offset_list", 0);
  int offset_num =  getChildCount(list_element, "extend_offset");
  for (int i = 0; i < offset_num; i++) {
    ext_offsets_.push_back(getDoubleChildValue(list_element, "extend_offset", i));
  }
}
