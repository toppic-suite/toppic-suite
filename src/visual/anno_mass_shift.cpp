...
void AnnoMassShift::appendXml(XmlDOMDocument* xml_doc, XmlDOMElement parent) {
  XmlDOMElement element = xml_doc->addElement(parent, "mass_shift");
  std::string str = std::to_string(id_);
  xml_doc->addElement(element, "id", str.c_str());
  str = std::to_string(left_pos_);
