...
XmlGenerator::XmlGenerator(const PrsmParaPtr &prsm_para_ptr,
      ...
      if (spec_set_ptr->isValid()) {
        ...
        while (prsm_ptr != nullptr && prsm_ptr->getSpectrumId() == spec_id) {
          ...
          std::string file_name = mng_ptr_->xml_path_+ file_util::getFileSeparator() +
            ...
            std::to_string(prsm_ptr->getPrsmId()) + ".xml";
          XmlWriter writer(file_name, "");
          writer.writeAndRelease(anno_prsm::geneAnnoPrsm(writer.getDoc(), writer.getDoc()->getDocumentElement(), prsm_ptr, mng_ptr_));
          writer.close();
          ...
          file_info.push_back(mng_ptr_->html_path_ 
                              ...
                              + "prsm" + std::to_string(prsm_ptr->getPrsmId()) + ".js");
...
void XmlGenerator::outputAllPrsms() {
  ...
  XmlDOMElement prot_elements = writer.getDoc()->addElement(writer.getDoc()->getDocumentElement(), "prsms");

  for (std::size_t i = 0; i < prsm_ptrs.size(); i++) {
    cnt++;
    std::cout << std::flush << "Generating XML files - combining " << cnt << " single PrSMs.\r";
    anno_prsm::geneAnnoPrsmBrief(writer.getDoc(), prot_elements, prsm_ptrs[i], mng_ptr_, true, false);
  }
  std::cout << std::endl;
...
void XmlGenerator::outputProteoforms(){
  ...
  for (size_t i = 0; i < cluster_ids_.size(); i++) {
    ...
    std::string input_file_name = file_util::basename(spectrum_file_name) 
        + ".proteoform_" + std::to_string(cluster_ids_[i]);
    ...
    if (select_prsm_ptrs.size() > 0) {
      std::string file_name = mng_ptr_->xml_path_+ file_util::getFileSeparator() 
          ...
          + "proteoform" + std::to_string(cluster_ids_[i]) + ".xml";
      XmlWriter writer(file_name, "");
      std::sort(select_prsm_ptrs.begin(), select_prsm_ptrs.end(),
                Prsm::cmpEValueIncProtInc);
      bool detail = true; 
      bool add_ms = true;
      writer.writeAndRelease(anno_xml_util::geneXmlForProteoform(writer.getDoc(),
                                                                 writer.getDoc()->getDocumentElement(),
                                                                 select_prsm_ptrs,
      ...
      LOG_DEBUG("output proteoform completed " << i);
        ...
        file_info.push_back(mng_ptr_->html_path_ 
                            ...
                            + file_util::getFileSeparator()
                            + "proteoform"+std::to_string(cluster_ids_[i])+".js");
        anno_file_list_ptr_->file_list_.push_back(file_info);
    }
  }
  std::cout << std::endl;
  ...
  for (size_t i = 0; i < prot_ids_.size(); i++) {
    ...
    if (!cluster.empty()) {
      std::string file_name = mng_ptr_->xml_path_ + file_util::getFileSeparator() + "proteins" 
          + file_util::getFileSeparator() + "protein" + std::to_string(prot_id) + ".xml";
      ...
      file_info.push_back(mng_ptr_->html_path_ 
                            ...
                            + file_util::getFileSeparator() 
                            + "protein"+std::to_string(prot_id)+".js");
      anno_file_list_ptr_->file_list_.push_back(file_info);
    }
  }
}
...
void XmlGenerator::outputAllProteins() {
  ...
  for (size_t i = 0; i < prot_ids_.size(); i++) {
    ...
    std::string input_file_name = file_util::basename(spectrum_file_name) 
        + ".prot_" + std::to_string(prot_ids_[i]);
    PrsmPtrVec prsm_ptrs 
        = prsm_reader_util::readAllPrsms(input_file_name, fasta_reader_ptr, fix_mod_ptr_vec);
    for (size_t k = 0; k < prsm_ptrs.size(); k++) {
      prsm_ptrs[k]->setDeconvMsPtrVec(
  ...
  XmlDOMElement prot_elements = writer.getDoc()->addElement(writer.getDoc()->getDocumentElement(), "proteins");
  ...
  for (size_t i = 0; i < best_prsm_vec.size(); i++) {
    ...
    std::vector<int> cluster = prsm_util::getClusterIds(prsm_ptrs);
    if (!cluster.empty()) {
      anno_xml_util::geneXmlForProteinList(writer.getDoc(), prot_elements, prsm_ptrs, prot_id, cluster,
                                           mng_ptr_, false);
    }
  }
  std::cout << std::endl;
...
void XmlGenerator::outputFileList() {
  ...
  XmlWriter writer(file_name, "");
  writer.writeAndRelease(anno_file_list_ptr_->geneFileList(writer.getDoc(), writer.getDoc()->getDocumentElement()));
  writer.close();
}
...
void XmlGenerator::process() {
  ...
  if (std::filesystem::exists(mng_ptr_->xml_path_)) {
