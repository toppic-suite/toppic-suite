...
void AnnoCleavage::appendXml(XmlDOMDocument* xml_doc, XmlDOMElement parent) {
  XmlDOMElement element = xml_doc->addElement(parent, "cleavage");
  std::string str = std::to_string(pos_);
  ...
  XmlDOMElement peaks = xml_doc->addElement(element, "matched_peaks");
  for (size_t i = 0; i < pairs_.size(); i++) {
    pairs_[i]->appendRealPeakToXml(xml_doc, peaks);
  }
}

AnnoCleavagePtrVec AnnoCleavage::getProteoCleavage(const PrsmPtr &prsm_ptr, double min_mass) {
