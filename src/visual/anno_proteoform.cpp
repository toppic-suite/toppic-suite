...
void addSummary(XmlDOMDocument* xml_doc, XmlDOMElement prot_element,
                const ProteoformPtr &proteoform_ptr, const PrsmViewMngPtr &mng_ptr) {
  std::string str = std::to_string(proteoform_ptr->getProtId());
  xml_doc->addElement(prot_element, "sequence_id", str.c_str());

  str = std::to_string(proteoform_ptr->getProteoClusterId());
  ...
  str = std::to_string(unexpected_change_number);
  xml_doc->addElement(prot_element, "unexpected_shift_number", str.c_str());
}

void addSummaryBrief(XmlDOMDocument* xml_doc, XmlDOMElement prot_element,
                const ProteoformPtr &proteoform_ptr, const PrsmViewMngPtr &mng_ptr) {
...
void addAnnoHeader(XmlDOMDocument* xml_doc, XmlDOMElement anno_element,
                   const ProteoformPtr &proteoform_ptr) {
  std::string str = std::to_string(proteoform_ptr->getFastaSeqPtr()->getAcidPtmPairLen());
  xml_doc->addElement(anno_element, "protein_length", str.c_str());

  str = std::to_string(proteoform_ptr->getStartPos());
  xml_doc->addElement(anno_element, "first_residue_position", str.c_str());

  str = std::to_string(proteoform_ptr->getEndPos());
...
void addAnnoHeaderBrief(XmlDOMDocument* xml_doc, XmlDOMElement anno_element,
                   const ProteoformPtr &proteoform_ptr) {
...
void addAnnoPtms(const ProteoformPtr &proteoform_ptr, MassShiftPtrVec & shift_ptrs,
...
void addAnnoVarPtms(const ProteoformPtr &proteoform_ptr, MassShiftPtrVec & shift_ptrs,
...
void addAnnoMassShifts(const ProteoformPtr &proteoform_ptr, MassShiftPtrVec & shift_ptrs,
...
/*
void addInsertion(int left_db_bp, int right_db_bp, const MassShiftPtr &shift_ptr,
...
AnnoMassShiftPtrVec getMassShifts(const ProteoformPtr &proteoform_ptr, MassShiftPtrVec & shift_ptrs) {
...
void addMod(const ProteoformPtr &proteoform_ptr, int left_db_bp, int right_db_bp,
            const MassShiftPtr &shift_ptr, AnnoSegmentPtrVec & segment_ptrs) {
...
XmlDOMElement geneAnnoProteoform(XmlDOMDocument* xml_doc, XmlDOMElement parent,
  ...
  XmlDOMElement prot_element = xml_doc->addElement(parent, "annotated_protein");
  addSummary(xml_doc, prot_element, proteoform_ptr, mng_ptr);
  XmlDOMElement anno_element = xml_doc->addElement(prot_element, "annotation");
