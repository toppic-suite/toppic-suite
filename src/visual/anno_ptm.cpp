...
AnnoPtm::AnnoPtm(const PtmPtr &ptm_ptr, const AlterTypePtr &type_ptr) {
  ptm_ptr_ = ptm_ptr;
  type_ptr_ = type_ptr;
}
...
void AnnoPtm::appendXml(XmlDOMDocument* xml_doc, XmlDOMElement parent) {
  XmlDOMElement element = xml_doc->addElement(parent, "ptm");
...
AnnoPtmPtr AnnoPtm::findPtm(const AnnoPtmPtrVec &ptm_ptrs, const PtmPtr &ptm_ptr,
