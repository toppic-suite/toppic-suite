...
void AnnoPtmPosition::appendXml(XmlDOMDocument* xml_doc, XmlDOMElement parent) {
  XmlDOMElement position_element = xml_doc->addElement(parent, "occurence");
  std::string str = std::to_string(left_pos_);
  xml_doc->addElement(position_element, "left_pos", str.c_str());
  str = std::to_string(right_pos_);
