...
void processOneFileWithFaims(const TopfdParaPtr& topfd_para_ptr,
  ...
  DeconvMs2ProcessPtr ms2_proc_ptr =
      std::make_shared<DeconvMs2Process>(topfd_para_ptr, "ms2_raw.msalign");
  ...
  GeneratePseudoSpectrumPtr pseudo_genrator_ptr =
      std::make_shared<GeneratePseudoSpectrum>(topfd_para_ptr, topdia_para_ptr);
  pseudo_genrator_ptr->process(topfd_para_ptr, topdia_para_ptr);
  pseudo_genrator_ptr = nullptr;
  std::cout << "Pseudo spectrum generation finished." << std::endl;
}

void processOneFile(const TopfdParaPtr& topfd_para_ptr,
                    const TopdiaParaPtr& topdia_para_ptr,
...
int process(const TopfdParaPtr& topfd_para_ptr,
  ...
  onnx_env_cnn::initModel(topfd_para_ptr->getResourceDir(),
                          topfd_para_ptr->getThreadNum());
  onnx_ecscore::initModel(topfd_para_ptr->getResourceDir(),
                          topfd_para_ptr->getThreadNum());
