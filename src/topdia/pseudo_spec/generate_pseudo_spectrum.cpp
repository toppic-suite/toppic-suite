...
GeneratePseudoSpectrum::GeneratePseudoSpectrum(
    const TopfdParaPtr& topfd_para_ptr, const TopdiaParaPtr& topdia_para_ptr) {
  ...
  for (auto cur_win : win_list_) {
    ...
    for (auto& ms2_data : deconv_ms2_ptr_vec) {
      ...
      rt_ms2_window.push_back(ms2_data->getMsHeaderPtr()->getRetentionTime() /
                              60);
      activation_ptr_vec.push_back(
          ms2_data->getMsHeaderPtr()->getActivationPtr());
    }
  ...
  for (std::size_t iso_win_idx = 0; iso_win_idx < win_list_.size();
       iso_win_idx++) {
...
void GeneratePseudoSpectrum::process(const TopfdParaPtr& topfd_para_ptr,
  ...
  EnvParaPtr env_para_ptr = std::make_shared<EnvPara>(
      ...
      topfd_para_ptr->isRunFilterByMz());
  std::string output_base_name = topfd_para_ptr->getOutputBaseName();
  ...
  for (std::size_t iso_win_idx = 0; iso_win_idx < win_list_.size();
    ...
    MzrtFeaturePtrVec selected_ms1_features =
        get_iso_win_ms1_features(iso_win_idx);
    std::sort(selected_ms1_features.begin(), selected_ms1_features.end(),
              compareFeaturesInte);
    std::cout << "Processing isolation window " << (iso_win_idx + 1) << " of "
              << win_list_.size() << " [" << win.first << "," << win.second
              << "] with " << selected_ms1_features.size() << " features."
              << std::endl;
    for (auto& ms1_feature : selected_ms1_features) {
      ...
      for (std::size_t ms2_feature_idx = 0;
           ms2_feature_idx < ms2_features_[iso_win_idx].size();
           ms2_feature_idx++) {
      ...
      writePseudoSpectrum(output, topfd_para_ptr, topdia_para_ptr, feature_id,
                          pseudo_spec_ptr->getMs1Feature(),
...
MzrtFeaturePtrVec GeneratePseudoSpectrum::get_iso_win_ms1_features(
    int isolation_window_base_index) {
...
double GeneratePseudoSpectrum::computeSharedArea(
  ...
  if (std::isnan(sharedArea)) {
    LOG_ERROR("computing shared area is nan " << xic1.size() << " "
                                              << xic2.size());
    for (std::size_t i = 0; i < xic1.size(); ++i) {
...
void GeneratePseudoSpectrum::writePseudoSpectrum(
    std::ofstream& output, const TopfdParaPtr& topfd_para_ptr,
    const TopdiaParaPtr& topdia_para_ptr, int ms1_feature_idx,
    const MzrtFeaturePtr& ms1_feature,
  ...
  output << "TITLE=" << "Pseudo_Scan_" + std::to_string(ms1_feature_idx)
         << std::endl;
  output << "SCANS=" << (ms1_scan_[ms1_apex_cycle] + 1 + iso_win_idx)
         << std::endl;  ///
  ...
  output << std::setprecision(6)
         << "PRECURSOR_WINDOW_BEGIN=" << ms1_feature->getWin().first
         << std::endl;
  output << "PRECURSOR_WINDOW_END=" << ms1_feature->getWin().second
         << std::endl;
  // need to add ACTIVATION information
  output << "ACTIVATION="
         << activation_ms2_[iso_win_idx][ms1_apex_cycle]->getName()
         << std::endl;
  output << "PRECURSOR_MZ=" << std::setprecision(6) << ms1_feature->getMonoMz()
         << std::endl;
  output << "PRECURSOR_CHARGE=" << ms1_feature->getCharge() << std::endl;
  output << "PRECURSOR_MASS=" << ms1_feature->getMass() << std::endl;
  output << "PRECURSOR_INTENSITY=" << std::setprecision(2)
         << ms1_feature->getIntensity() << std::endl;
