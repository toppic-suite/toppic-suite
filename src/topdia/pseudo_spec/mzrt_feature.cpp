...
namespace toppic {
    mzrtFeature::mzrtFeature(int id, int fraction_id, int env_num, double mass, double mono_mz, int charge,
                             double intensity, int mz_begin, int mz_end, double time_begin, double time_end,
                             int spec_id_begin, int spec_id_end, double time_apex, double ec_score,
    ...
    mzrtFeaturePtrVec mzrtFeature::read_record(std::string filename) {
        ...
        while (std::getline(file, line)) {
            ...
            if (tokens.size() >= 0) {
                ...
                int apex_cycle = std::distance(xic.begin(), std::max_element(xic.begin(), xic.end()));
                mzrtFeaturePtr feature = std::make_shared<mzrtFeature>(id, fraction_id, env_num, mass, mono_mz, charge, intensity, mz_begin,
                                                                       mz_end, time_begin, time_end, spec_id_begin, spec_id_end, time_apex,
                                                                       ec_score, xic, normalized_xic, envelope_mz, envelope_inte, apex_cycle);
                if (std::accumulate(xic.begin(), xic.end(), 0) == 0)
                    continue;
                data.push_back(feature);
            }
        }
        file.close();
        std::sort(data.begin(), data.end(), GeneratePseudoSpectrum::compareFeaturesInte);
        return data;
    }
    ...
    void mzrtFeature::parseEnvelope(const std::string &input, std::vector<double> &array1, std::vector<double> &array2) {
