...
//	std::cout<<trunc_last_res_pos<<std::endl;
	...
	double pep_c_term_shift = prot_c_term_shift + seq_b_masses[seq_b_masses.size()-1]-seq_b_masses[trunc_last_res_pos+1];
...
void setProtTermMod(DiagonalHeaderPtr &header,ProteoformPtr seq,PtmMngPtr mng){
	...
	ProtModPtr mod;
	if(header->isNTrunc()){
		mod = findProtTermMod(mng->allow_prot_N_mods_,trunc_len,resseq,header->getPepNTermShift(),mng->test_term_mod_error_toerance_);
	}
	header->setProtNTermAllowMod(mod);
	trunc_len = seq->getResSeqPtr()->getLen()-1 -header->getTruncLastResPos();
//	std::cout<<header->getTruncLastResPos()<<std::endl;
	resseq = resseq->getSubResidueSeq(header->getTruncLastResPos()+1,resseq->getLen()-1);
	mod = nullptr;
	if(header->isCTrunc()){
//		std::cout<<":"<<trunc_len<<":"<<header->getTruncLastResPos()+1<<":"<<(resseq->getLen()-1)<<":"<<header->getPepCTermShift()<<std::endl;
...
void setPepTermMode(DiagonalHeaderPtr &header,PtmMngPtr mng){
	PtmPtr mod;
	if(header->isNTrunc()){
		mod = findPepTermMod(mng->allow_pep_N_mods_,header->getPepNTermShift(),mng->test_term_mod_error_toerance_);
	}
	header->setPepNTermAllowMod(mod);
	mod =nullptr;
	if(header->isCTrunc()){
		mod = findPepTermMod(mng->allow_pep_C_mods_,header->getPepCTermShift(),mng->test_term_mod_error_toerance_);
	}
...
void setAlignPrefSuffic(DiagonalHeaderPtr &header,PtmMngPtr mng){
	bool is_prefix = prot::isAlignPrefix(header->getProtNTermAllowTrunc(),header->getPepNTermShift(),mng->prefix_suffix_shift_thesh_);
	header->setAlignPrefix(is_prefix);
	bool is_suffix = prot::isAlignSuffix(header->getProtCTermAllowTrunc(),header->getPepCTermShift(),mng->prefix_suffix_shift_thesh_);
...
//	std::cout<<mod<<std::endl;
	if(mod==nullptr||mod->getName().compare("ACETYLATION")==0||mod->getName().compare("NME_ACETYLATION")==0){
		return true;
	}
	return false;
}
