...
void PtmProcessor::processOneSpectrum(SpectrumSetPtr spectrum_set_ptr, 
  ...
  for (int s = 1; s <= mng_ptr_->getShiftNum(); s++) {
    ...
    PrsmPtrVec prefix_prsm_ptrs = slow_filter_ptr->getPrsms(s-1, AlignType::PREFIX);
    ...
    PrsmPtrVec suffix_prsm_ptrs = slow_filter_ptr->getPrsms(s-1, AlignType::SUFFIX);
    ...
    PrsmPtrVec internal_prsm_ptrs = slow_filter_ptr->getPrsms(s-1, AlignType::INTERNAL);
