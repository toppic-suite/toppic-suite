...
DeconvData::DeconvData(const PeakPtrVec& peak_list, double max_mass,
                       int max_charge, double win_size)
    : peak_list_(peak_list),
      max_mass_(max_mass),
      max_charge_(max_charge),
      win_size_(win_size) {
  win_num_ =
      (int)std::ceil(peak_list_util::findMaxPos(peak_list_) / win_size_) + 2;
  initWinId();
  initWinBgnEnd();
}

DeconvData::DeconvData(const PeakPtrVec& peak_list, double max_mass,
                       int max_charge, double win_size, bool estimate_min_inte,
                       double sn_ratio)
    : peak_list_(peak_list),
      max_mass_(max_mass),
      max_charge_(max_charge),
