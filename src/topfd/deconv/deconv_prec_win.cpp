...
PeakIntv initPeakIntv(const EnvParaPtr& env_para_ptr, PeakPtrVec& peak_list,
...
double initMinInte(PeakPtrVec& peak_list, PeakIntv peak_intv) {
  int peak_num = peak_intv.end - peak_intv.bgn + 1;
  if (peak_num < PRECURSOR_TOP_PEAK_NUM) {
    return 0;
  } else {
    std::vector<PeakPtr>::const_iterator first =
        peak_list.begin() + peak_intv.bgn;
    std::vector<PeakPtr>::const_iterator last =
        peak_list.begin() + peak_intv.end + 1;
...
MatchEnvPtr2D initMatchEnv(const EnvParaPtr& env_para_ptr,
                           PeakPtrVec& peak_list, PeakIntv peak_intv,
                           int peak_num, double argu_max_mass, int max_charge,
...
MatchEnvPtr deconv(double prec_win_begin, double prec_win_end,
                   PeakPtrVec& peak_list, double argu_max_mass,
                   int argu_max_charge, double base_inte, double min_ref_inte) {
  if (prec_win_begin <= 0) {
    return nullptr;
  }
  EnvParaPtr env_para_ptr = initMngPtr();
  PeakIntv peak_intv =
      initPeakIntv(env_para_ptr, peak_list, prec_win_begin, prec_win_end);
  ...
  MatchEnvPtr2D match_envs =
      initMatchEnv(env_para_ptr, peak_list, peak_intv, peak_num, argu_max_mass,
                   max_charge, base_inte, min_ref_inte);
  ...
  LOG_DEBUG("Env mass " << env_ptr->getTheoEnvPtr()->getMonoNeutralMass()
                        ...
                        << " peak number "
                        << env_ptr->getTheoEnvPtr()->getPeakNum());
  return env_ptr;
}

MatchEnvPtr deconvPrecWinForOneMs(const MzmlMsPtr& ms_one,
                                  const MzmlMsPtr& ms_two, double max_mass,
                                  int max_charge, double base_inte,
                                  double min_ref_inte) {
  ...
  MatchEnvPtr match_env_ptr =
      deconv_prec_win::deconv(prec_win_begin, prec_win_end, peak_list, max_mass,
                              max_charge, base_inte, min_ref_inte);
  if (match_env_ptr == nullptr) {
    LOG_INFO("No precursor isotopic envelope is found for scan "
             << ms_two_header->getFirstScanNum());
  }
  return match_env_ptr;
}

MatchEnvPtrVec deconvPrecWinForMsGroup(const MzmlMsGroupPtr& ms_group_ptr,
  ...
  for (size_t i = 0; i < ms_two_ptr_vec.size(); i++) {
    ...
    MatchEnvPtr match_env_ptr = deconvPrecWinForOneMs(
        ms_one_ptr, ms_two_ptr, max_mass, max_charge, base_inte, min_ref_inte);
    if (match_env_ptr != nullptr) {
