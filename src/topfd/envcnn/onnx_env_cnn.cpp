...
void initModel(const std::string& dir_name, int thread_num) {
  std::string file_name = dir_name + file_util::getFileSeparator() +
                          "envcnn_models" + file_util::getFileSeparator() +
                          "envcnn_two_block.onnx";
...
MatchEnvPtr2D getBatchEnv(const MatchEnvPtrVec& ori_envs) {
...
void extractTheoPeakData(const EnvPtr& theo_env, std::vector<double>& theo_mass,
...
void getTheoEnvData(const MatchEnvPtr& ori_env, std::vector<double>& theo_mass,
                    std::vector<double>& theo_intes, double& max_inte,
                    double& theo_mono_mz) {
...
void getExpIntervaledPeakData(const PeakPtrVec& peak_list, double real_mono_mz,
  ...
  double max_theo_mass =
      *std::max_element(std::begin(theo_mass), std::end(theo_mass));
  double min_theo_mass =
      *std::min_element(std::begin(theo_mass), std::end(theo_mass));
  for (const auto& k : peak_list) {
    double temp_mz = k->getPosition();
    if (temp_mz >= real_mono_mz - 5 && temp_mz <= real_mono_mz + 5) {
      if ((temp_mz >= min_theo_mass - 0.1) &&
          (temp_mz <= max_theo_mass + 0.1)) {
...
void getExpEnvData(const PeakPtrVec& peak_list, const MatchEnvPtr& ori_env,
                   std::vector<double>& theo_mass,
                   std::vector<double>& peak_mass,
                   std::vector<double>& peak_intes) {
  ExpEnvPtr real_env = ori_env->getExpEnvPtr();
  double real_mono_mz = real_env->getMonoMz();
  getExpIntervaledPeakData(peak_list, real_mono_mz, theo_mass, peak_mass,
                           peak_intes);
}
...
void extractFeature(const std::vector<double>& theo_masses,
                    const std::vector<double>& theo_intes,
                    const std::vector<double>& peak_masses,
                    const std::vector<double>& peak_intes, double max_inte,
                    double tolerance, size_t k, double& t_peak_mass,
                    double& t_peak_inte, double& exp_inte, double& md,
                    double inte_diff) {
  ...
  for (size_t exp_id = 0; exp_id < peak_masses.size(); exp_id++) {
    ...
    if (mass_diff <= tolerance && mass_diff >= -tolerance) {
      if (std::abs(mass_diff) < std::abs(old_mass_diff)) {
        exp_peak_idx = exp_id;
...
void addNoisePeaksToMatrix(std::vector<std::vector<float>>& matrix,
  ...
  for (size_t exp_id = 0; exp_id < peak_masses.size(); exp_id++) {
    ...
    for (int shift = -2; shift <= 2; shift++) {
      ...
      if ((shifted_idx < 300) && (shifted_idx >= 0) &&
          (matrix[0][shifted_idx] != 0)) {
        matched_peak = true;
      }
    }
...
std::vector<float> getTensor(const PeakPtrVec& peak_list,
                             const MatchEnvPtr& env) {
  ...
  for (size_t k = 0; k < theo_masses.size(); k++) {
    ...
    double inte_diff = 0;
    double mass_diff = 0;
    extractFeature(theo_masses, theo_intes, peak_masses, peak_intes, max_inte,
                   tolerance, k, t_peak_mass, t_peak_inte, exp_inte, mass_diff,
                   inte_diff);
    fillMatrix(matrix, min_mz, t_peak_mass, t_peak_inte, exp_inte, mass_diff,
               inte_diff);
  }
...
std::vector<float> getBatchTensor(const PeakPtrVec& peak_list,
                                  const MatchEnvPtrVec& envs) {
  std::vector<float> result;
  for (size_t i = 0; i < envs.size(); i++) {
    std::vector<float> env_result = getTensor(peak_list, envs[i]);
    result.insert(std::end(result), std::begin(env_result),
                  std::end(env_result));
  }
  return result;
}
...
void predict(MatchEnvPtrVec& envs, std::vector<float>& input_tensor_values) {
  ...
  for (int i = 0; i < env_num; i++) {
    ...
    LOG_DEBUG(" msdeconv " << envs[i]->getMsdeconvScore() << " predict "
                           << pred_results[i]);
  }
}

std::vector<double> predict(int env_num,
...
void computeEnvScores(const PeakPtrVec& peak_list, MatchEnvPtrVec& ori_envs) {
