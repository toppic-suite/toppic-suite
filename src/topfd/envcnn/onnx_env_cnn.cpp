...
void extractTheoPeakData(const EnvPtr &theo_env,
...
std::vector<float> getTensor(const PeakPtrVec &peak_list, const MatchEnvPtr &env) {
...
std::vector<float> getBatchTensor(const PeakPtrVec &peak_list, const MatchEnvPtrVec &envs) {
  std::vector<float> result;
