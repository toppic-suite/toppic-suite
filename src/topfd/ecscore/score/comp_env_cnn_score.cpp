...
MsMapPeakPtrVec getIntvPeakList(const MsMapPtr& matrix_ptr,
                                const EnvSetPtr& env_set_ptr, int spec_id) {
  EnvPeakPtrVec peak_list = env_set_ptr->getSeedPtr()->getPeakPtrList();
  double min_theo_peak =
      std::round(peak_list[0]->getPosition() * 1000.0) / 1000.0;
  double max_theo_peak =
      std::round(peak_list[peak_list.size() - 1]->getPosition() * 1000.0) /
      1000.0;
...
std::vector<std::vector<float>> getEnvcnnInputMatrix(
    const MsMapPtr& matrix_ptr, const EnvCollPtr& coll_ptr) {
  ...
  double inte_ratio =
      ms_map_env_util::compTopThreeInteRatio(seed_ptr, exp_dist_inte);
  std::vector<double> scaled_theo_inte;
  EnvPeakPtrVec peak_list = env_set_ptr->getSeedPtr()->getPeakPtrList();
  for (const auto& peak : peak_list)
    scaled_theo_inte.push_back(inte_ratio * peak->getIntensity());
  double max_scalled_theo_inte =
      *std::max_element(scaled_theo_inte.begin(), scaled_theo_inte.end());
  ...
  for (int idx = 0; idx < num_peaks; idx++) {
    ...
    int p_idx = getIndex(std::round(theo_mz[idx] * 1000.0) / 1000.0, theo_mz[0],
                         bin_size) +
                10;
    if (p_idx >= 300) break;
  ...
  for (int spec_id = coll_ptr->getStartSpecId();
       spec_id <= coll_ptr->getEndSpecId(); spec_id++) {
    MsMapPeakPtrVec intv_peak_list =
        getIntvPeakList(matrix_ptr, env_set_ptr, spec_id);
  ...
  for (int idx = 0; idx < num_spec; idx++) {
    ...
    for (int j_idx = 0; j_idx < num_peaks; j_idx++) {
      int p_idx =
          getIndex(
              std::round(noise_distribution_list[idx][j_idx] * 1000.0) / 1000.0,
              theo_mz[0], bin_size) +
          10;
      if (p_idx >= 300) break;
      ...
      for (int shift = -2; shift <= 2; shift++) {
        ...
        if ((shifted_idx < 300) && (shifted_idx >= 0) &&
            (data_matrix[0][shifted_idx] != 0)) {
          matched_peak = true;
        }
      }
      if (!matched_peak) {
        double max_scaled_theo_inte =
            *std::max_element(scaled_theo_inte.begin(), scaled_theo_inte.end());
        noise_arr[p_idx] =
            noise_arr[p_idx] +
            (noise_inte_distribution_list[idx][j_idx] / max_scaled_theo_inte);
      }
    }
  }
...
double compEnvcnnScore(const MsMapPtr& matrix_ptr, const EnvCollPtr& coll_ptr) {
  std::vector<std::vector<float>> matrix =
      getEnvcnnInputMatrix(matrix_ptr, coll_ptr);
