...
std::vector<double> getTheoEnvPeakInte(const EnvSetPtr& env_set_ptr) {
  EnvPeakPtrVec peaks = env_set_ptr->getSeedPtr()->getPeakPtrList();
  std::vector<double> theo_peak_intes;
  for (const auto& i : peaks) theo_peak_intes.push_back(i->getIntensity());
...
double getAggOddEvenPeakRatio(const EnvSetPtr& env_set_ptr) {
  ...
  double max_aggregate_inte =
      *std::max_element(aggregate_inte.begin(), aggregate_inte.end());
  ...
  double sum_even_peaks = 0, sum_even_peaks_theo = 0, sum_odd_peaks = 0,
  ...
  if (sum_even_peaks == 0.0) {
    ...
    inte_ratio = (sum_even_peaks / sum_even_peaks_theo) /
                 (sum_odd_peaks / sum_odd_peaks_theo);
  }
...
double countMaxConsecutivePeakNum(MsMapPeakPtrVec& peaks) {
  ...
  for (auto peak : peaks) {
    if (peak != nullptr) {
      n = n + 1;
      if (n > max_consecutive_peak_num) max_consecutive_peak_num = n;
    } else
      n = 0;
  }
...
double getConsecutivePeakPercent(const EnvSetPtr& env_set_ptr) {
  ...
  for (auto& exp_env : exp_envs) {
    MsMapPeakPtrVec peaks = exp_env->getMsMapPeakList();
    for (auto p : peaks)
      if (p != nullptr) total_peaks = total_peaks + 1;
  ...
  for (int i = 0; i < num_exp_envs; i++) {
    ...
    for (int peak_id = 0; peak_id < num_peaks; peak_id++) {
      ...
      if (peak_inte > 0) {
        ...
        if (peaks[peak_id] != nullptr) positive_peaks = positive_peaks + 1;
      }
    }
  }
...
int getTheoPeakNum(std::vector<std::vector<double>>& theo_map) {
  ...
  for (auto& scan_intes : theo_map)
    for (double peak_inte : scan_intes)
      if (peak_inte > 0) total_peaks = total_peaks + 1;
  return total_peaks;
}
...
double get3ScanCorr(const EnvSetPtr& env_set_ptr, int base_spec,
  ...
  double sp_minus_1_sum =
      std::accumulate(data_sp_minus_1.begin(), data_sp_minus_1.end(), 0.0);
  double sp_plus_1_sum =
      std::accumulate(data_sp_plus_1.begin(), data_sp_plus_1.end(), 0.0);
  if (sp_sum > 0 and sp_minus_1_sum > 0 and sp_plus_1_sum > 0) {
    ...
    double corr_sp_23 =
        seed_env_util::pearsonr(data_sp_minus_1, data_sp_plus_1);
