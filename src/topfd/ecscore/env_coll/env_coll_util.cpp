...
EnvCollPtr getEnvCollPtr(const MsMapPtr& matrix_ptr, const SeedEnvPtr& seed_ptr,
                         const EnvSetPtr& seed_env_set_ptr,
  ...
  while (charge >= para_ptr->para_min_charge_) {
    ...
    EnvSetPtr env_set_ptr = env_set_util::searchEnvSet(
        matrix_ptr, cur_seed_ptr, cur_seed_ptr->getSpecId(), start_spec_id,
        end_spec_id, para_ptr, sn_ratio);
    ...
    if (miss_num >= para_ptr->max_miss_charge_) break;
    charge = charge - 1;
  }
  ...
  EnvCollPtr env_coll_ptr =
      std::make_shared<EnvColl>(seed_ptr, env_set_list, min_charge, max_charge,
                                start_spec_id, end_spec_id);

  return env_coll_ptr;
}

EnvCollPtr findEnvColl(const MsMapPtr& matrix_ptr, const SeedEnvPtr& seed_ptr,
                       const EcscoreParaPtr& para_ptr, double sn_ratio,
                       double split_ratio) {
  EnvSetPtr env_set_ptr =
      env_set_util::searchEnvSet(matrix_ptr, seed_ptr, para_ptr, sn_ratio);
  if (env_set_ptr == nullptr) {
    return nullptr;
  }
  env_set_ptr->refineXicBoundary(split_ratio);
  ...
  double even_odd_peak_ratio =
      component_score::getAggOddEvenPeakRatio(env_set_ptr);
  SeedEnvPtr new_seed_ptr = seed_ptr;
  if (std::abs(even_odd_peak_ratio) > para_ptr->even_odd_ratio_cutoff_) {
    new_seed_ptr = seed_env_util::testHalfChargeEnv(
        seed_ptr, matrix_ptr, even_odd_peak_ratio, para_ptr, sn_ratio);
    if (new_seed_ptr == nullptr) {
      return nullptr;
    }
    EnvSetPtr tmp_env_set_ptr = env_set_util::searchEnvSet(
        matrix_ptr, new_seed_ptr, para_ptr, sn_ratio);
  ...
  EnvCollPtr env_coll_ptr = getEnvCollPtr(matrix_ptr, new_seed_ptr, env_set_ptr,
                                          para_ptr, sn_ratio, split_ratio);

  return env_coll_ptr;
}

EnvCollPtr findEnvCollWithSingleEnv(const MsMapPtr& matrix_ptr,
                                    const SeedEnvPtr& seed_ptr,
  ...
  EnvCollPtr env_coll_ptr = getEnvCollPtr(matrix_ptr, seed_ptr, env_set_ptr,
                                          para_ptr, sn_ratio, split_ratio);
  return env_coll_ptr;
}

bool checkOverlap(MsMapRowHeaderPtrVec& spectrum_list,
                  const EnvCollPtr& coll_ptr, double feature_start_rt,
...
bool checkExistingFeatures(const MsMapPtr& matrix_ptr,
                           const EnvCollPtr& env_coll_ptr,
  ...
  std::vector<double> extended_masses = {env_mass - isotope_mass, env_mass,
                                         env_mass + isotope_mass};
  ...
  for (int i = 0; i < num_env_colls; i++) {
    bool overlap = checkOverlap(spectrum_list, env_coll_list[i],
                                feature_start_rt, feature_end_rt,
    ...
    if (overlap) {
      ...
      for (auto ext_mass : extended_masses) {
        double mass_diff =
            std::abs(ext_mass - env_coll_list[i]->getMonoNeutralMass());
  ...
  if (overlap_env_coll_ptr != nullptr) {
    ...
    for (size_t i = 0; i < new_set_ptrs.size(); i++) {
      ...
      SeedEnvPtr cur_seed_ptr = std::make_shared<SeedEnv>(seed_ptr, charge);
      EnvSetPtr env_set_ptr = env_set_util::searchEnvSet(
          matrix_ptr, cur_seed_ptr, cur_seed_ptr->getSpecId(),
          merge_start_spec_id, merge_end_spec_id, para_ptr, sn_ratio);
      if (env_set_ptr != nullptr) {
...
FracFeaturePtr getFracFeature(int feat_id, DeconvMsPtrVec& ms1_ptr_vec,
                              int frac_id, std::string& file_name,
                              const EnvCollPtr& coll_ptr,
                              const MsMapPtr& matrix_ptr, double sn_ratio) {
  ...
  int ms1_scan_begin =
      ms1_ptr_vec[ms1_id_begin]->getMsHeaderPtr()->getFirstScanNum();
  int ms1_scan_end =
      ms1_ptr_vec[ms1_id_end]->getMsHeaderPtr()->getFirstScanNum();
  // get apex inte
  ...
  FracFeaturePtr feature_ptr = std::make_shared<FracFeature>(
      file_name, frac_id, feat_id, feat_mass, feat_inte, ms1_id_begin,
      ms1_id_end, ms1_time_begin, ms1_time_end, ms1_scan_begin, ms1_scan_end,
      min_charge, max_charge, apex_time, apex_scan, apex_inte, rep_charge,
      rep_avg_mz, env_num, ec_score);
  ...
  for (EnvSetPtr es : coll_ptr->getEnvSetList()) {
    ...
    double time_begin =
        ms1_ptr_vec[id_begin]->getMsHeaderPtr()->getRetentionTime();
    ...
    SingleChargeFeaturePtr single_feature =
        std::make_shared<SingleChargeFeature>(
            ...
            id_begin, id_end, feat_mass, xic, envelope_mass, aggregate_env_inte,
            mono_mz, average_mz, refer_mz, scan_list, rt_list,
