...
void process_one_window(TopfdParaPtr topfd_para_ptr, 
                        EcscoreParaPtr score_para_ptr, 
                        DeconvMsPtrVec &ms_list) {
  // read ms2 raw peaks 
  ...
  mzml_reader_ptr->getMs2Map(ms2_mzml_peaks, ms_list); 
  mzml_reader_ptr = nullptr;

  //Prepare seed envelopes
  SeedEnvPtrVec seed_ptrs;
  for (size_t idx = 0; idx < ms_list.size(); idx++) {
    DeconvMsPtr ms2_data = ms_list[idx];
    DeconvPeakPtrVec peaks = ms2_data->getPeakPtrVec();
    for (auto &peak: peaks) {
      SeedEnvPtr seed_ptr = std::make_shared<SeedEnv>(peak);
      // use idx instead of id because ms2 scans are not continuous
      seed_ptr->setSpecId(idx);
  ...
  bool use_single_scan_noise = topfd_para_ptr->isUseSingleScanNoiseLevel();
  ...
  MsMapPtr matrix_ptr = std::make_shared<MsMap>(ms2_mzml_peaks, deconv_ms2_ptr_vec,
                                                ...
                                                sn_ratio, use_single_scan_noise);
...
DeconvMsPtr2D clusterPrecWindows(DeconvMsPtrVec &deconv_ms2_ptr_vec) {
  DeconvMsPtr2D result_clusters;
  for (size_t i = 0; i < deconv_ms2_ptr_vec.size(); i++) {
    MsHeaderPtr cur_ptr = deconv_ms2_ptr_vec[i]->getMsHeaderPtr();
    bool found = false;
    for (size_t j = 0; j < result_clusters.size(); j++) {
      MsHeaderPtr ref_ptr = result_clusters[j][0]->getMsHeaderPtr();
      if (ref_ptr->getPrecWinBegin() == cur_ptr->getPrecWinBegin() 
          && ref_ptr->getPrecWinEnd() == cur_ptr->getPrecWinEnd()) {
        found = true;
        result_clusters[j].push_back(deconv_ms2_ptr_vec[i]);
        break;
      }
    }
    if (!found) {
      DeconvMsPtrVec new_cluster;
      new_cluster.push_back(deconv_ms2_ptr_vec[i]);
...
void process(TopfdParaPtr topfd_para_ptr) {
  ...
  DeconvMsPtr2D spec_clusters = clusterPrecWindows(deconv_ms2_ptr_vec);
  for (size_t i = 0; i < spec_clusters.size(); i++) {
    process_one_window(topfd_para_ptr, score_para_ptr, spec_clusters[i]);
  }
}
