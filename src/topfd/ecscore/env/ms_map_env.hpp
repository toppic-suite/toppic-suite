...
class MsMapEnv {
 public:
  ...
  void setPeakPtr(int idx, const MsMapPeakPtr &peak_ptr) { peak_list_[idx] = peak_ptr; }

  int getTopThreeMatchNum(int ref_idx);

  double compTopThreeInteSum(int ref_idx);

  void removeLowIntePeaks(const SeedEnvPtr &seed_ptr, double ratio, double min_inte);
