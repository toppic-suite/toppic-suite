...
double compTopThreeInteRatio(const SeedEnvPtr &seed_ptr, const MsMapEnvPtr &env_ptr) {
  if (env_ptr == nullptr) {
    return 0;
  }
...
double compTopThreeInteRatio(const SeedEnvPtr &seed_ptr, std::vector<double> &inte_list) {
  double seed_inte = seed_ptr->compTopThreeInteSum();
  if (seed_inte == 0) {
    LOG_INFO("Empty peak list!");
    return 0;
...
MsMapPeakPtr pickMsMapPeak(const MsMapPtr &ms_map_ptr, const EnvPeakPtr &seed_peak_ptr,
...
MsMapEnvPtr getMatchMsMapEnv(const MsMapPtr &ms_map_ptr, const SeedEnvPtr &seed_ptr,
                             int sp_id, double mass_tol) {
