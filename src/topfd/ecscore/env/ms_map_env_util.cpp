...
double compTopThreeInteRatio(const SeedEnvPtr& seed_ptr,
                             const MsMapEnvPtr& env_ptr) {
  if (env_ptr == nullptr) {
    return 0;
  }
...
MsMapPeakPtr pickMsMapPeak(const MsMapPtr& ms_map_ptr,
                           const EnvPeakPtr& seed_peak_ptr, int sp_id,
                           double mass_tol) {
...
MsMapEnvPtr getMatchMsMapEnv(const MsMapPtr& ms_map_ptr,
                             const SeedEnvPtr& seed_ptr, int sp_id,
                             double mass_tol) {
...
MsMapEnvPtr getMatchMsMapEnv(const MsMapPtr& ms_map_ptr,
                             const SeedEnvPtr& seed_ptr, int sp_id,
                             double mass_tole, double min_inte) {
  MsMapEnvPtr ms_map_env_ptr =
      getMatchMsMapEnv(ms_map_ptr, seed_ptr, sp_id, mass_tole);
  double inte_ratio =
