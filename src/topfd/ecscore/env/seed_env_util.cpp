...
SeedEnvPtr preprocessSeedEnvPtr(const SeedEnvPtr& seed_ptr,
  ...
  if (seed_ptr->getSpecId() >= ms_map_ptr->getRowNum()) {
    LOG_ERROR("spec id " + std::to_string(seed_ptr->getSpecId()) +
              " is out of range!");
    return nullptr;
  }
  // 3. Get high intensity peaks only.
  MsMapEnvPtr ms_map_env_ptr = ms_map_env_util::getMatchMsMapEnv(
      ms_map_ptr, seed_ptr, seed_ptr->getSpecId(), para_ptr->peak_mz_tole_);
  double inte_ratio =
      ms_map_env_util::compTopThreeInteRatio(seed_ptr, ms_map_env_ptr);
  double min_inte = ms_map_ptr->getBaseInte() * sn_ratio;
  EnvPeakPtrVec scaled_peak_ptr_list =
      seed_ptr->getScaledPeakPtrList(inte_ratio, min_inte);
  // 4. Check peak number
  ...
  SeedEnvPtr result_seed_env_ptr =
      std::make_shared<SeedEnv>(seed_ptr, scaled_peak_ptr_list);
...
SeedEnvPtr getHalfChargeEnvV1(const SeedEnvPtr& seed_ptr,
  ...
  DeconvPeakPtr peak_ptr =
      std::make_shared<DeconvPeak>(sp_id, peak_id, mono_mass, inte, new_charge);
  SeedEnvPtr new_seed_ptr = std::make_shared<SeedEnv>(peak_ptr);

  return new_seed_ptr;
}

SeedEnvPtr getHalfChargeEnvV2(const SeedEnvPtr& seed_ptr,
...
SeedEnvPtr testHalfChargeEnv(const SeedEnvPtr& seed_ptr,
  ...
  SeedEnvPtr half_charge_seed =
      getHalfChargeEnvV1(seed_ptr, even_odd_log_ratio);
  if (half_charge_seed == nullptr) {
    return nullptr;
  }
  SeedEnvPtr processed_seed_ptr =
      preprocessSeedEnvPtr(half_charge_seed, ms_map_ptr, para_ptr, sn_ratio);
