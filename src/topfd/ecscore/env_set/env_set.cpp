...
void EnvSet::removePeakData(const MsMapPtr& ms_map_ptr) {
  ...
  for (int env_id = 0; env_id < env_num; env_id++) {
    ...
    if (peak_num != static_cast<int>(theo_env_peak_intes.size())) {
...
void EnvSet::appendToXml(XmlDOMDocument* xml_doc, XmlDOMElement parent) {
  XmlDOMElement element = xml_doc->addElement(parent, "env_set");
  seed_ptr_->appendToXml(xml_doc, element);
  XmlDOMElement env_list = xml_doc->addElement(element, "exp_env_list");
  for (size_t i = 0; i < ms_map_env_list_.size(); i++) {
