...
DpA::DpA(const DeconvDataPtr& data_ptr, MatchEnvPtr2D& win_envs_,
...
void DpA::initGraph() {
  ...
  MatchEnvPtrVec envs;
  vertices_[0] =
      vertex_util::getVertexAList(data_ptr_, -1, envs, envs, dp_para_ptr_);
  vertices_[1] = vertex_util::getVertexAList(data_ptr_, 0, envs, win_envs_[0],
                                             dp_para_ptr_);
  for (int i = 1; i < win_num_; i++) {
    vertices_[i + 1] = vertex_util::getVertexAList(
        data_ptr_, i, win_envs_[i - 1], win_envs_[i], dp_para_ptr_);
  }
  vertices_[win_num_ + 1] = vertex_util::getVertexAList(
    ...
    for (size_t j = 0; j < vertices_[i].size(); j++) {
      ...
      for (size_t k = 0; k < vertices_[i - 1].size(); k++) {
        ...
        if (Vertex::checkConsist(prev_ver, cur_ver,
          ...
          // LOG_DEBUG("i " << i << " j " << j << " k " << k << " new score " <<
          // new_score << " cur_score " << cur_score);
