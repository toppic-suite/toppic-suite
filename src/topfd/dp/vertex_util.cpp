...
void addEmptyVertexA(const DpParaPtr& dp_para_ptr, VertexAPtrVec& result,
  ...
  VertexAPtr ptr = std::make_shared<VertexA>(
      dp_para_ptr, bgn_peak, pre_win_peak_num, cur_win_peak_num);
  ptr->trim();
...
VertexAPtrVec getVertexAList(const DeconvDataPtr& data, int cur_win,
  ...
  for (size_t i = 0; i < prev_match_envs.size(); i++) {
    ...
    for (int j = 0; j < cur_size; j++) {
      if (result[j]->getMatchEnvSize() < dp_para_ptr->max_env_num_per_vertex_) {
        ...
        if (vertex->addPreEnv(prev_match_envs[i],
                              dp_para_ptr->max_env_num_per_peak_)) {
          vertex->trim();
  ...
  for (size_t i = 0; i < cur_match_envs.size(); i++) {
    ...
    for (int j = 0; j < cur_size; j++) {
      if (result[j]->getMatchEnvSize() < dp_para_ptr->max_env_num_per_vertex_) {
        VertexAPtr vertex = std::make_shared<VertexA>(result[j]);
        if (vertex->addCurEnv(cur_match_envs[i],
                              dp_para_ptr->max_env_num_per_peak_)) {
          vertex->trim();
          result.push_back(vertex);
        }
...
void addEmptyVertexB(const DpParaPtr& dp_para_ptr, VertexBPtrVec& result,
  ...
  int cur_win_peak_num = getWinPkNum(cur_win, data);
  VertexBPtr vertex = std::make_shared<VertexB>(
      dp_para_ptr, bgn_peak, prev_win_peak_num, cur_win_peak_num, env_num);
  vertex->trim();
  result.push_back(vertex);
}
