...
class VertexB : public Vertex {
 public:
  VertexB(const DpParaPtr& dp_para_ptr, int bgn_peak, int pre_win_peak_num,
          int cur_win_peak_num, int env_num);

  VertexB(const VertexBPtr& ptr);

  bool addPreEnv(const MatchEnvPtr& env, int max_overlap) { return false; }
