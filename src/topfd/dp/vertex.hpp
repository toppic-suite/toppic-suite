...
class Vertex {
 public:
  Vertex(const DpParaPtr &dp_para_ptr, int bgn_peak, int pre_win_peak_num, int cur_win_peak_num);
  ...
  void trim();
  static bool checkConsist(const VertexPtr &pre, const VertexPtr &cur, int max_env_per_peak);
  ...
  static double getShareScr(const VertexPtr &pre, const VertexPtr &cur, double error_tole);
