...
#ifndef TOPPIC_TOPFD_DP_VERTEX_HPP_
...
class Vertex {
 public:
  Vertex(const DpParaPtr& dp_para_ptr, int bgn_peak, int pre_win_peak_num,
         int cur_win_peak_num);

  Vertex(const VertexPtr& ptr);

  Vertex() {};

  virtual ~Vertex() {};

  virtual bool addPreEnv(const MatchEnvPtr& env, int max_overlap);
  bool addCurEnv(const MatchEnvPtr& env, int max_overlap);
  void trim();
  static bool checkConsist(const VertexPtr& pre, const VertexPtr& cur,
                           int max_env_per_peak);

  int getMatchEnvSize() {
    return prec_match_envs_.size() + cur_match_envs_.size();
  }
  ...
  static double getShareScr(const VertexPtr& pre, const VertexPtr& cur,
                            double error_tole);
 ...
 protected:
  ...
  bool passDblIncrCheck(const MatchEnvPtr& env);
