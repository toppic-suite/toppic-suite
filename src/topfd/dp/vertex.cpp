...
Vertex::Vertex(const DpParaPtr &dp_para_ptr, int bgn_peak, 
...
bool Vertex::checkConsist(const VertexPtr &pre, const VertexPtr &cur, int max_env_per_peak) {
  // if same size 
...
double Vertex::getShareScr(const VertexPtr &pre, const VertexPtr &cur, double error_tole) {
  double score = 0;
