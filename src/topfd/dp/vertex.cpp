...
Vertex::Vertex(const DpParaPtr& dp_para_ptr, int bgn_peak, int pre_win_peak_num,
               int cur_win_peak_num) {
...
Vertex::Vertex(const VertexPtr& ptr) {
...
bool Vertex::passDblIncrCheck(const MatchEnvPtr& env) {
  int id = env->getId();
  int exist_id;
...
bool Vertex::addPreEnv(const MatchEnvPtr& env, int max_overlap) {
...
bool Vertex::addCurEnv(const MatchEnvPtr& env, int max_overlap) {
  if (dp_para_ptr_->check_double_increase_ && !passDblIncrCheck(env)) {
    return false;
  }
  ExpEnvPtr real_env_ptr = env->getExpEnvPtr();
...
bool Vertex::checkConsist(const VertexPtr& pre, const VertexPtr& cur,
                          int max_env_per_peak) {
  // if same size
  ...
  for (size_t i = 0; i < pre->pre_env_peak_pairs_.size(); i++) {
    if (pre->pre_env_peak_pairs_[i].size() > 0) {
      ...
      if (peak_idx >= 0 && (int)cur->peak_use_cnts_[peak_idx] +
                                   (int)pre->pre_env_peak_pairs_[i].size() >
                               max_env_per_peak) {
        return false;
      }
    }
...
double Vertex::getShareScr(const VertexPtr& pre, const VertexPtr& cur,
                           double error_tole) {
  double score = 0;
