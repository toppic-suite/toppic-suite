...
VertexA::VertexA(const DpParaPtr& dp_para_ptr, int bgn_peak,
...
VertexA::VertexA(const VertexAPtr& ptr) : Vertex(ptr) {
  this_score_ = ptr->this_score_;
  score_ = ptr->score_;
  prev_vertex_ = ptr->prev_vertex_;
}

bool VertexA::addPreEnv(const MatchEnvPtr& env, int max_overlap) {
