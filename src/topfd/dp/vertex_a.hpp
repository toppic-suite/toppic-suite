...
class VertexA;
using VertexAPtr = std::shared_ptr<VertexA>;

class VertexA : public Vertex {
 public:
  VertexA(const DpParaPtr& dp_para_ptr, int bgn_peak, int pre_win_peak_num,
          int cur_win_peak_num);

  VertexA(const VertexAPtr& ptr);

  bool addPreEnv(const MatchEnvPtr& env, int max_overlap);
...
using VertexAPtrVec = std::vector<VertexAPtr>;
using VertexAPtr2D = std::vector<VertexAPtrVec>;
