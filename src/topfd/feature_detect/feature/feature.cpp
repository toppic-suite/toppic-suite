...
namespace toppic {
  ...
  FracFeaturePtr Feature::getFeature(int feat_id, DeconvMsPtrVec &ms1_ptr_vec, int frac_id, std::string file_name,
    ...
    for (EnvSet &es: env_coll.getEnvSetList()) {
      ...
      std::vector<double> aggregateEnvelopeInte = es.getAggregateEnvelopeInte();
      std::vector<double> envelopeMass = es.get_theo_distribution_mz();
      SingleChargeFeaturePtr single_feature = std::make_shared<SingleChargeFeature>(charge, time_begin, time_end,
                                                                                    ...
                                                                                    feat_mass, xic, envelopeMass, aggregateEnvelopeInte);
...
//              << " and num of isolation windows: " << isolation_windows_num << std::endl;
  ...
  void Feature::get_empty_feature_map(DeconvMsPtrVec &ms1_ptr_vec, FracFeaturePtrVec &frac_features,
    ...
    ExpEnvelope exp_env = env_set_util::get_match_exp_env(peak_matrix, env, env.getSpecId(), para_ptr->mass_tole_);
    ...
    EnvCollection env_coll = EnvCollection(env, env_set_list, env.getCharge(), env.getCharge(), start_spec_id, end_spec_id);
