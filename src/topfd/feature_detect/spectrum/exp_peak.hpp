...
namespace toppic {
  ...
  public:
    ...
    ExpPeak(int peak_id, int spec_id, PeakPtr peak) {
      peak_id_ = peak_id;
      spec_id_ = spec_id;
      pos_ = peak->getPosition();
      inte_ = peak->getIntensity();
    ...
    ExpPeak(const ExpPeak &p) {
      peak_id_ = p.getPeakId();
      spec_id_ = p.getSpecId();
      pos_ = p.getPos();
      inte_ = p.getInte();
      ori_inte_ = p.getOriInte();
      start_idx_ = p.getStartIdx();
      end_idx_ = p.getEndIdx();
      neighbor_ = p.getNeighbor();
    }
