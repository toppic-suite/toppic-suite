...
TopDiffArgument::TopDiffArgument() { arguments_ = initArguments(); }
...
void TopDiffArgument::outputArguments(
    std::ostream& output, const std::string& sep,
    std::map<std::string, std::string> arguments) {
  int gap = 26;
  output << "******************** Parameters ********************" << std::endl;
  output << std::setw(gap) << std::left << "Error tolerance:" << sep
         << arguments["errorTolerance"] << " Dalton " << std::endl;
  output << std::setw(gap) << std::left << "Database search tool name:" << sep
         << arguments["toolName"] << std::endl;
  output << std::setw(gap) << std::left << "Output file name:" << sep
         << arguments["mergedOutputFileName"] << std::endl;
  output << std::setw(gap) << std::left << "Version:" << sep
         << arguments["version"] << std::endl;
  output << "******************** Parameters ********************" << std::endl;
}

void TopDiffArgument::showUsage(
...
bool TopDiffArgument::parse(int argc, char* argv[]) {
  ...
  try {
    ...
    display_desc.add_options()("help,h", "Print the help message.")(
        ...
        "Error tolerance of precursor masses for mapping identified "
        "proteoforms. Default: 1.2 Dalton.")(
        "tool-name,t", po::value<std::string>(&tool_name),
        "<toppic|topmg>. Database search tool name: toppic or topmg. Default: "
        "toppic.")("output,o", po::value<std::string>(&merged_output_name),
                   "Output file name. Default: sample_diff.tsv.");
    po::options_description desc("Options");

    desc.add_options()("help,h", "")("error-tolerance,e",
                                     po::value<std::string>(&error_tole), "")(
        "tool-name,t", po::value<std::string>(&tool_name), "")(
...
bool TopDiffArgument::validateArguments() {
  ...
  for (size_t k = 0; k < spectrum_file_list_.size(); k++) {
    if (!std::filesystem::exists(spectrum_file_list_[k])) {
      LOG_ERROR(spectrum_file_list_[k]
                << " does not exist!\n"
                << "Please check if the file directory or name contains "
                   "special characters such as spaces or quotation marks.");
      return false;
    }
  }
  if (spectrum_file_list_.size() < 2) {
    LOG_ERROR(
        "Only 1 spectrum file added. Please add at least 2 spectrum files to "
        "make a comparison.");
    return false;
  }
